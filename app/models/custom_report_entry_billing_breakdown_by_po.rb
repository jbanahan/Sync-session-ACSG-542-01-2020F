...
class CustomReportEntryBillingBreakdownByPo < CustomReport
  ...
  def run user, row_limit = nil
    unless user.view_broker_invoices?
      raise "User #{user.email} does not have permission to view invoices and cannot run the #{CustomReportEntryBillingBreakdownByPo.template_name} report."
    end
    ...
    invoices.each do |inv|
      iterate_invoice_lines(inv) do |_line, description|
    ...
    invoices.each do |inv|
      ...
      iterate_invoice_lines(inv) do |line, description|
        unless charge_totals[description]
          ...
          charge_totals[description][:total] = BigDecimal("0")
        end
      ...
      charge_totals.each_key do |k|
        ...
        charge_values[:remaining_invoice_amount] = BigDecimal(charge_values[:total])
      ...
      po_numbers.each_with_index do |po, i|
        ...
        total_charges_per_po = BigDecimal("0")
        po_charge_amounts = []
  ...
  private
    ...
    def fix_invoice_number inv
      ...
      if invoice_number.blank?
        ...
        invoice_number << inv.suffix if inv.suffix.present?
      end
      invoice_number
    end

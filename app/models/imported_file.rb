...
#  set_blank             :boolean          default(FALSE)
...
class ImportedFile < ActiveRecord::Base
  ...
  UPDATE_MODES = {"any" => "Add or Update", "add" => "Add Only", "update" => "Update Only"}.freeze
  ...
  attr_accessible :attached_content_type, :attached_file_name, :attached_file_size,
                  ...
                  :user_id, :user, :attached, :set_blank
  ...
  has_one :result_cache, as: :result_cacheable, dependent: :destroy # rubocop:disable Rails/InverseOf

  before_validation :set_module_type
  validates :starting_row, presence: true
  validates :starting_row, numericality: { greater_than: 0 }
  validates :starting_column, presence: true
  validates :starting_column, numericality: { greater_than: 0 }
  validates :module_type, presence: true
  validates :update_mode, presence: true
  validates :update_mode, inclusion: { in: UPDATE_MODES.keys.to_a }
  ...
  def result_keys_from
    <<-SQL
      INNER JOIN (select distinct recordable_id
                  from change_records
                    inner join (select id
                                from file_import_results
                                where imported_file_id = #{self.id}
                                order by finished_at DESC limit 1) as fir
                      ON change_records.file_import_result_id = fir.id) as change_recs on change_recs.recordable_id = #{core_module.table_name}.id
    SQL
  end

  # return keys from last file import result
  def result_keys _opts = {}
    qry = <<-SQL
      select distinct recordable_id
      from change_records inner join (select id
                                      from file_import_results
                                      where imported_file_id = #{self.id} order by finished_at DESC limit 1) as fir
                            ON change_records.file_import_result_id = fir.id
    SQL
  ...
  def core_module
    CoreModule.by_class_name self.module_type
  end
  ...
  def make_items_file search_criterions = []
    if self.attached_file_name.downcase.ends_with?("xls")
      book = XlsMaker.new.make_from_results self.last_file_import_finished.changed_objects(search_criterions),
                                            self.search_columns.order("rank asc"),
                                            self.core_module.default_module_chain,
                                            self.user,
                                            search_criterions
      ...
      CsvMaker.new.make_from_results self.last_file_import_finished.changed_objects(search_criterions),
                                     self.search_columns.order("rank ASC"), self.core_module.default_module_chain,
                                     self.user,
                                     search_criterions
    end
  end
  ...
  def email_updated_file current_user, to, cc, subject, body, opts = {}
    ...
    additional_countries = opts[:extra_country_ids].collect {|id| Country.find(id).iso_code} if opts[:extra_country_ids].present?
  ...
  def make_updated_file opts = {}
    ...
    if extra_countries.present?
      ...
      extra_countries.each do |c_id|
        ((self.starting_row - 1)..base_last_row).each_with_index do |row_number, _i|
    ...
    ((self.starting_row - 1)..client.last_row_number(0)).each do |row_number|
      ...
      unless k.first.nil?
        ...
        self.search_columns.each_with_index do |sc, _i|
          if !sc.key_column? && sc.model_field.present?
            v = values[sc.rank]
            v = "" if v.nil?
  ...
  def self.process_integration_imported_file bucket, remote_path, original_path
      ...
      user = User.find_by username: folder_list[1]
      raise "Username #{folder_list[1]} not found." unless user
      raise "User #{user.username} is locked." unless user.active?
      ss = user.search_setups.find_by(module_type: folder_list[3], name: folder_list[4])
  ...
  def make_search_criterion _core_module, column, row
    ...
    SearchCriterion.new(model_field_uid: column.model_field_uid, operator: 'eq', value: val)
  end
  ...
  def import_search_columns
    if !self.search_setup.nil? && self.search_columns.blank?
      ...
      self.search_setup.search_columns.each do |col|
        ...
        col.attributes.each do |attr, _value|
          my_col.public_send("#{attr}=".to_sym, col.public_send(attr.to_s.to_sym)) unless no_copy.include?(attr.to_sym)
        end
        my_col.save
      end
    end
  ...
  class FileImportProcessJob
    ...
    def perform
      ...
      if another_instance_running?
        ...
        if @imported_file.errors.size > 0 && MasterSetup.get.custom_feature?('LogImportedFileErrors')
          OpenMailer.send_imported_file_process_fail(@imported_file, @imported_file.search_setup.user).deliver_now
        end
      end
    end
  ...
  class FileImportProcessorListener
    ...
    def initialize(imported_file, user_id)
      @imported_file = imported_file
      @fr = @imported_file.file_import_results.build(run_by: User.find_by(id: user_id))
    end

    def process_row_count count
      @fr.update!(expected_rows: (count - @imported_file.starting_row - 1))
    end
    ...
    def process_row row_number, object, messages, failed: false, saved: false
      ...
      if messages.present?
        ...
        unless saved || failed
          crms << ChangeRecordMessage.new(change_record_id: cr.id, message: "#{CoreModule.by_object(object)&.label} was not updated because no values were modified.")
        end

        ChangeRecordMessage.import crms
        FileImportResult.where(id: @fr.id).update_all(rows_processed: row_number - (@imported_file.starting_row - 1)) # rubocop:disable Rails/SkipsModelValidations
      end

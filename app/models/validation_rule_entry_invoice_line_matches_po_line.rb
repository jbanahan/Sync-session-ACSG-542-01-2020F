class ValidationRuleEntryInvoiceLineMatchesPoLine < BusinessValidationRule
  ...
  def run_child_validation invoice_line
    ...
    lines = OrderLine.joins(:order, :product).joins("INNER JOIN custom_values ON custom_values.custom_definition_id = #{@part_no_cd.id} AND custom_values.customizable_id = products.id AND custom_values.customizable_type = 'Product'")
            ...
            .where("custom_values.string_value = ?", invoice_line.part_number)
    ...
    if lines.empty?
      ...
      return check_match_fields(invoice_line,lines)
    end
  ...
  private
  def check_match_fields invoice_line, order_lines
    msgs = []
    @match_fields.each do |mf|
      ...
      if !field_matches(sc,order_lines)
        msgs << "No matching order for PO # #{invoice_line.po_number} and Part # #{invoice_line.part_number} where #{ord_line_model_field.label(false)} #{CriterionOperator.find_by_key(mf['operator']).label} #{inv_line_model_field.label(false)} (#{sc.value})"
      end
    end

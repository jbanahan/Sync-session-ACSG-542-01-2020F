...
class SearchCriterion < ActiveRecord::Base
  ...
  def json current_user
    mf = self.model_field
    {
      mfid: self.model_field_uid,
      label: if mf.can_view?(current_user)
              mf.label
             else
              ModelField.disabled_label
             end,
      operator: self.operator,
      value: self.value,
      datatype: self.model_field.data_type,
  ...
  def where_clause sql_value
    ...
    if custom_field? && !mf.user_field? && !mf.address_field? && !mf.virtual_field?
      ...
      if boolean_field?
        ...
        clause = "#{join_identifier} IN (#{base_select} AND #{CriterionOperator.by_key(self.operator)
                                                                  .query_string("custom_values.#{cd.data_column}",
                                                                                mf.data_type, self.include_empty)})"
      end
      ...
      if boolean_field?
        ...
        clause = CriterionOperator.by_key(self.operator).query_string(mf.qualified_field_name, mf.data_type, self.include_empty)
      end
  ...
  def boolean_field?
    find_model_field.data_type == :boolean
  end

  def integer_field?
    find_model_field.data_type == :integer
  end

  def decimal_field?
    find_model_field.data_type == :decimal
  end

  def date_field?
    find_model_field.data_type == :date
  end

  def date_time_field?
    find_model_field.data_type == :datetime
  end

  def field_relative_model_field
    mf_two = nil
    if field_relative?
      mf_two = relative_model_field
    end
    mf_two
  end
  ...
  def where_value
    ...
    when "regexp"
      ...
      if self_val
        ...
        self_val = BigDecimal(self_val) if decimal_field?
      end
  ...
  def test_decimal? mf, object, _user, _opts
  ...
  def secondary_model_field
    ModelField.by_uid self.secondary_model_field_uid
  end

  def relative_model_field
    ModelField.by_uid self.value
  end
  ...
  def passes_decimal? first_field, second_field, obj
    ...
    if first_field.blank? || second_field.blank?
      ...
      case self.operator
      when 'nqfdec'
        passes = decimal_value != BigDecimal(self.value)
      when 'eqfdec'
        passes = decimal_value == BigDecimal(self.value)
      when 'gtfdec'
        passes = decimal_value < BigDecimal(self.value)
  ...
  def passes_relative? my_value, other_value, opts
    ...
    results = []
    my_values.each do |my_value| # rubocop:disable Lint/ShadowingOuterLocalVariable
      passes = false
  ...
  def decimal_where_clause
    first_mf = ModelField.by_uid(self.model_field_uid)
    second_mf = secondary_model_field
  ...
  def relative_where_clause
    ...
    other_mf = relative_model_field
    mysql_user_time_zone = Time.zone.tzinfo.name
    really_old_date = Time.zone.local("1901", "01", "01", "00", "00", "00")
  ...
  def mf_passes?(mf, value_to_test)
    ...
    return false if value_to_test.nil? && ["co", "nc", "sw", "ew", "gt", "gteq", "lt", "bda", "ada", "adf", "bdf", "pm",
                                           "bma", "ama", "amf", "bmf", "cmo", "pqu", "cqu", "pfcy", "cytd"].include?(self.operator)
    ...
    return value_to_test.present? if self.operator == "notnull"
    ...
    if [:string, :text].include? d
      ...
      if self.operator == "eq"
        ...
        value_to_test.nil? ? self.value.nil? : value_to_test.downcase.rstrip == self.value.downcase.rstrip
      elsif self.operator == "co"
        value_to_test.downcase.include?(self.value.downcase)
      elsif self.operator == "nc"
        !value_to_test.downcase.include?(self.value.downcase)
      elsif self.operator == "sw"
        value_to_test.downcase.start_with?(self.value.downcase)
      elsif self.operator == "nsw"
        !value_to_test.downcase.start_with?(self.value.downcase)
      elsif self.operator == "regexp"
        !value_to_test.to_s.match(self.value).nil?
      elsif self.operator == "notregexp"
        value_to_test.to_s.match(self.value).nil?
      elsif self.operator == "ew"
        value_to_test.downcase.end_with?(self.value.downcase)
      elsif self.operator == "new"
        !value_to_test.downcase.end_with?(self.value.downcase)
      elsif self.operator == "nq"
        # The rstrip here is to match how the inequality operator works in MySQL.
        value_to_test.nil? || value_to_test.downcase.rstrip != self.value.downcase.rstrip
        ...
        break_rows(self.value.downcase).include?(value_to_test.downcase.rstrip)
      ...
      if self.operator == "eq"
        ...
        vt < self_val.days.ago.to_date # .to_date gets rid of times that screw up comparisons
      elsif self.operator == "ada"
        vt >= self_val.days.ago.to_date
      elsif self.operator == "adf"
        vt >= self_val.days.from_now.to_date
        ...
        (vt < Time.zone.now.to_date) && (vt >= base_date) && !(vt.month == 0.seconds.ago.month && vt.year == 0.seconds.ago.year)
      elsif self.operator == "bma"
        vt < get_safe_date(d, (Time.zone.now - self_val.months).beginning_of_month.at_midnight)
        ...
        vt >= get_safe_date(d, ((Time.zone.now - self_val.months).end_of_month + 1.day).at_midnight)
        ...
        vt >= Time.zone.now.beginning_of_month && vt <= Time.zone.now.end_of_month
      elsif self.operator == "pqu"
        # Excludes items from the current quarter.
        vt >= SearchCriterion.get_previous_quarter_start_date(Time.zone.now, self_val) && vt < SearchCriterion.get_quarter_start_date(SearchCriterion.get_quarter_number(Time.zone.now), Time.zone.now.year)
      elsif self.operator == "cqu"
        # Includes items that may be in the future, provided that they occur within the current quarter.
        vt >= SearchCriterion.get_quarter_start_date(SearchCriterion.get_quarter_number(Time.zone.now), Time.zone.now.year) && vt < SearchCriterion.get_next_quarter_start_date(Time.zone.now, 1)
      elsif self.operator == "pfcy"
        # Excludes items from the current calendar year.
        vt >= Date.new(self_val.years.ago.year, 1, 1) && vt < Date.new(Time.zone.now.year, 1, 1)
        ...
        vt >= Date.new(Time.zone.now.year, 1, 1) && vt <= Time.zone.now
      end
    elsif d == :boolean
      ...
      if self.operator == "eq"
        ...
        value_to_test.to_s.start_with?(self.value.to_s)
      elsif self.operator == "nsw"
        !value_to_test.to_s.start_with?(self.value.to_s)
      elsif self.operator == "ew"
        value_to_test.to_s.end_with?(self.value.to_s)
      elsif self.operator == "new"
        !value_to_test.to_s.end_with?(self.value.to_s)
      elsif self.operator == "nq"
        value_to_test.nil? || value_to_test != self_val
      elsif self.operator == "in"
        break_rows(self.value.downcase).include?(value_to_test.to_s.rstrip)
      elsif self.operator == "notin"
        !break_rows(self.value.downcase).include?(value_to_test.to_s.rstrip)
      elsif self.operator == "regexp"
        !value_to_test.to_s.match(self.value).nil?
  ...
  def number_value data_type, value
    sval = (value.blank? || !a_number?(value)) ? "0" : value
    data_type == :integer ? sval.to_i : BigDecimal(sval.to_s)
  end

  def a_number? val
      begin Float(val); true end
  ...
  def parse_date_time value
    ...
    if value.is_a?(Time)
      ...
      # rubocop:disable Style/IfInsideElse
      if value.match %r{\A([\d\-/]+)( +([\d:]+)? *(.*)?)?\z}
          date = Regexp.last_match(1)
          time = Regexp.last_match(3).nil? ? "00:00:00" : Regexp.last_match(3)
          time_zone = Time.find_zone(Regexp.last_match(4))

module Api; module V1; module Admin; class BusinessValidationSchedulesController < Api::V1::Admin::AdminApiController
  def new
    ...
    cms.select! { |cm| CoreModule.find_by(class_name: cm).enabled_lambda.call }
    render json: {cm_list: cms}
  end

  def index
    scheds = BusinessValidationSchedule.all.map do |sch|
      field_label = ModelField.by_uid(sch.model_field_uid).label if sch.model_field_uid.present?
  ...
  def edit
    ...
    cm = CoreModule.find_by(class_name: schedule.module_type)
  ...
  def criterion_mf_hsh core_module, _schedule
  ...
  def schedule_mf_hsh core_module, user
    model_fields_list = []
    core_module.model_fields(user).each do |_mfid, mf|

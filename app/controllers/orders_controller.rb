...
class OrdersController < ApplicationController
  ...
  def set_page_title
    @page_title ||= 'Order' # rubocop:disable Naming/MemoizedInstanceVariableName
  end
  ...
  def show
      ...
      action_secure(o.can_view?(current_user), o, {lock_check: false, verb: "view", module_name: "order"}) do
        @order = o
        ...
        respond_to do |format|
            ...
            format.json do
              render json: @order.to_json(only: [:id, :order_number], include: {
  ...
  def new
    o = Order.new
    action_secure(current_user.company.master, o, {lock_check: false, verb: "create", module_name: "order"}) do
      @order = o
    end
  end
  ...
  def edit
    o = Order.find(params[:id])
    action_secure(o.can_edit?(current_user), o, {verb: "edit", module_name: "order"}) do
      @order = o
    end
  end
  ...
  def create
    ...
    action_secure(current_user.company.master, ord, {verb: "edit", module_name: "order"}) do
      success = lambda {|o|
        add_flash :notices, "Order created successfully."
        redirect_to o
      }
      failure = lambda {|o, _errors|
  ...
  def update
    o = Order.find(params[:id])
    action_secure(o.can_edit?(current_user), o, {module_name: "order"}) do
      succeed = lambda {|ord|
        add_flash :notices, "Order was updated successfully."
        redirect_to ord
      }
      failure = lambda {|ord, _errors|
  ...
  def destroy
    o = Order.find(params[:id])
    action_secure(current_user.company.master, o, {verb: "delete", module_name: "order"}) do
      @order = o
      @order.destroy
  ...
  def close
    o = Order.find params[:id]
    action_secure(o.can_close?(current_user), o, {verb: "close", module_name: "order"}) do
  ...
  def reopen
    o = Order.find params[:id]
    action_secure(o.can_close?(current_user), o, {verb: "reopen", module_name: "order"}) do
  ...
  def accept
    o = Order.find params[:id]
    action_secure(o.can_be_accepted? && o.can_accept?(current_user), o, {verb: "accept", module_name: "order"}) do
  ...
  def unaccept
    o = Order.find params[:id]
    action_secure(o.can_accept?(current_user), o, {verb: "unaccept", module_name: "order"}) do
  ...
  def bulk_send_to_sap
    ...
    rescue OpenChain::BulkAction::TooManyBulkObjectsError

module EntriesHelper
  ...
  def render_xls entry, user
    render_workbook entry, user, XlsBuilder.new
  end

  def render_xlsx entry, user
    render_workbook entry, user, XlsxBuilder.new
  end

  def render_workbook entry, user, builder  
    ...
    wb = builder
    sheet = wb.create_sheet "Entry", headers: fields[:entry].collect{|f| f.label(false)}
    wb.freeze_horizontal_rows sheet, 1
    wb.add_body_row sheet, fields[:entry].collect {|f| f.process_export(entry, user)}

    sheet = wb.create_sheet "Commercial Invoices", headers: (fields[:commercial_invoice] + fields[:commercial_invoice_line] + fields[:commercial_invoice_tariff]).collect{|f| f.label(false)}
    wb.freeze_horizontal_rows sheet, 1
    wb.add_body_row(sheet, ["No Commercial Invoice data."]) if entry.commercial_invoices.length == 0
        ...
        inv.commercial_invoice_lines.each do |line|
          ...
          if line.commercial_invoice_tariffs.size > 0
            line.commercial_invoice_tariffs.each do |tariff|
              wb.add_body_row sheet, (invoice_data + invoice_line_data + fields[:commercial_invoice_tariff].collect {|f| f.process_export(tariff, user)})
            end
          else
            wb.add_body_row sheet, (invoice_data + invoice_line_data)
          end
        end
        
      else
        wb.add_body_row sheet, invoice_data
      end
    end

    if user.view_broker_invoices?
      sheet = wb.create_sheet "Broker Invoices", headers: (fields[:broker_invoice] + fields[:broker_invoice_line]).collect{|f| f.label(false)}
      wb.freeze_horizontal_rows sheet, 1
      wb.add_body_row(sheet, ["No Broker Invoice data."]) if entry.broker_invoices.length == 0
      ...
      entry.broker_invoices.each do |inv|
        ...
        if inv.broker_invoice_lines.length > 0
          inv.broker_invoice_lines.each do |line|
            wb.add_body_row sheet, (invoice_data + fields[:broker_invoice_line].collect {|f| f.process_export(line, user)})
          end
        else
          wb.add_body_row sheet, invoice_data
        end
  ...
  private
    ...
    def build_us_field_arrays user
      fields = {}
      fields[:entry] = [:ent_entry_num,:ent_brok_ref,:ent_customer_references,:ent_location_of_goods,:ent_location_of_goods_desc,:ent_mbols,
        :ent_hbols,:ent_release_cert_message,:ent_fda_message,:ent_paperless_certification,:ent_paperless_release,:ent_census_warning,:ent_error_free_release, :ent_hold_date, :ent_hold_release_date,
        :ent_cust_name,:ent_vendor_names,:ent_po_numbers,:ent_merch_desc, :ent_export_date,:ent_docs_received_date,:ent_isf_sent_date,
        :ent_isf_accepted_date,:ent_one_usg_date,:ent_first_it_date,:ent_filed_date,:ent_first_entry_sent_date,:ent_eta_date,:ent_arrival_date,:ent_release_date,
      ...
      fields[:commercial_invoice] = [:ci_invoice_number,:ci_vendor_name,:ci_invoice_date,:ci_gross_weight,:ci_country_origin_code,:ci_invoice_value,:ci_total_charges,:ci_invoice_value_foreign,:ci_currency, :ci_total_quantity, :ci_total_quantity_uom]
      fields[:commercial_invoice_line] = [:cil_part_number,:cil_po_number,:cil_customs_line_number,:cil_mid,:cil_units,:cil_uom,:cil_value,:cil_country_origin_code,:cil_country_export_code,:cil_department,:cil_related_parties,:cil_volume,:cil_hmf,:cil_prorated_mpf,:cil_cotton_fee, :cil_contract_amount,:cil_adjusted_value,
        :cil_adjustments_amount, :cil_other_amount, :cil_cash_discount,:cil_miscellaneous_discount,:cil_add_case_number, :cil_add_bond, :cil_add_case_value, :cil_add_duty_amount, :cil_add_case_percent,:cil_cvd_case_number, :cil_cvd_bond, :cil_cvd_case_value, :cil_cvd_duty_amount, :cil_cvd_case_percent
      ]
    ...
    def build_ca_field_arrays user
      ...
      fields[:entry] = [:ent_entry_num,:ent_brok_ref,:ent_cust_num, :ent_cust_name, :ent_importer_tax_id,:ent_cargo_control_number,:ent_ca_entry_type,:ent_po_numbers,
          :ent_vendor_names, :ent_customer_references,:ent_location_of_goods,:ent_location_of_goods_desc, :ent_total_units,:ent_total_packages, :ent_total_packages_uom, :ent_gross_weight, :ent_total_invoiced_value,:ent_entered_value,:ent_total_duty,:ent_total_gst,:ent_total_duty_gst,
          :ent_direct_shipment_date,:ent_eta_date,:ent_across_sent_date,:ent_pars_ack_date,:ent_pars_reject_date,:ent_release_date,:ent_one_usg_date,:ent_cadex_sent_date,:ent_cadex_accept_date,:ent_exam_ordered_date,:ent_hold_date,:ent_hold_release_date,:ent_duty_due_date,:ent_file_logged_date,:ent_docs_received_date,:ent_first_do_issued_date, :ent_k84_receive_date, :ent_k84_due_date,

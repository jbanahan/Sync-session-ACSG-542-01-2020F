var OCSearch = (function() {
  ...
  var initBulkSelectors = function() {
    $("#result_table :checkbox:not(#chk_sel_all)").on('click',rewriteBulkForm);
    var selAllBinding = function() {
      $("#result_table :checkbox:not(#chk_sel_all)").off('click').attr('checked',(allSelected ? '' : 'checked')).on('click',rewriteBulkForm);;
      $("#chk_sel_all").unbind('click').attr('checked','').on('click',selAllBinding);
      allSelected = !allSelected;
      rewriteBulkForm();
    }
    $("#chk_sel_all").on('click',selAllBinding);
  }
  ...
  var initSelectFullList = function() {
    ...
    $('body').on('click',".sel_none",function(ev) {
      ...
      $("#result_table :checkbox").attr('checked',false);
      rewriteBulkForm();
    });
  }
  ...
  var bindValueValidations = function(tr) {
    ...
    var vCont = tr.find('.srch_crit_cont')
    var opVal = op.val();
    var tagName = vField[0].nodeName.toLowerCase();
    if((opVal=='in' || opVal=='notin') && tagName!='textarea') {
    vCont.html("<textarea class='srch_crit_value' name='"+vField.attr('name')+"' cols='30' rows='5' id='"+vField.attr('id')+"' /><div class='srch_instruction'>One Value Per Line</div>");
      return;
    } else if (opVal!='in' && opVal!='notin' && tagName=='textarea') {
      vCont.html("<input type='text' size='30' id='"+vField.attr('id')+"' name='"+vField.attr('name')+"' class='srch_crit_value' />")
    }
    var vField = tr.find(".srch_crit_value");
  ...
  var initRowDoubleClick = function() {
    $(".search_row").on('dblclick',function() {
      $(this).find(".double_click_action").each(function() {
        window.location = $(this).attr("href");
      });
    });
    $(".search_row a").on('dblclick',function(evt) {evt.stopPropagation();}) //prevent an accidental double-click on a link from bubbling up to the row
  }
  ...
  return {
    ...
    addSearchCriterion: function(parentTable,parentObject,fieldList,m,field,operator,value,id,canDelete, include_empty_checked) {
      var h = "<tr class='sp_row'><td>";
        ...
        h = h + "</select></td><td><div class='srch_crit_cont'><input size='30' type='text' id='"+parentObject+"_search_criterions_attributes_"+m+"_value' name='"+parentObject+"[search_criterions_attributes]["+m+"][value]' class='srch_crit_value' /></div></td>";
        h = h + "<td><input type='hidden' name='"+parentObject+"[search_criterions_attributes]["+m+"][include_empty]' value='0' /><input type='checkbox' id='"+parentObject+"_search_criterions_attributes_"+m+"_include_empty' name='"+parentObject+"[search_criterions_attributes]["+m+"][include_empty]' value='1'" + (include_empty_checked ? "checked='checked'" : "")+ "/> Include Empty?<td>";
        if(canDelete) {
        h = h + "<td><input class='sp_destroy' id='"+parentObject+"_search_criterions_attributes_"+m+"__destroy' name='"+parentObject+"[search_criterions_attributes]["+m+"][_destroy]' type='hidden' value='false' /><a href='#' class='sp_remove'><img src='/assets/x.png' title='Remove this parameter' /></parameter></td></tr>";
        }

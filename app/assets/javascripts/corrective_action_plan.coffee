...
capApp.controller 'CorrectiveActionPlanController', ['$scope','$http',($scope,$http) ->
  ...
  updateData = (data) ->
    ...
    if data.can_edit && (!$scope.cap.corrective_issues || $scope.cap.corrective_issues.length == 0)
      $scope.cap.corrective_action_plan = []
      $scope.addIssue()
  ...
  $scope.remove = (issue) ->
    toRem = $.inArray(issue,$scope.cap.corrective_issues)
    if toRem >= 0
      $scope.cap.corrective_issues.splice(toRem,1)
    else
      console.log(issue + "is not in array.")
]

//! ChainCommon
//! Build Time: Wed Oct 19 2016 10:39:16 GMT-0400 (EDT)
//! Git SHA: 02063acd8543dcb83c96f6ec344eb5d4c1e3203a
//! Git Branch: master
...
(function() {
  ...
  app = angular.module('ChainCommon', ['ChainCommon-Templates', 'ChainDomainer', 'angularMoment']);
...
(function() {
  ...
  app.factory('chainApiSvc', [
    '$http', '$q', '$sce', '$location', '$rootScope', '$window', function($http, $q, $sce, $location, $rootScope, $window) {
      var newAdminClient, newAttachmentClient, newBulkExecuteClient, newBusinessRuleClient, newCommentClient, newCoreModuleClient, newCountryClient, newFieldValidatorClient, newMessageClient, newSearchTableConfigClient, newSupportClient, newUserClient, newUserManualClient, publicMethods;
      publicMethods = {};
      ...
      newCoreModuleClient = function(moduleType, objectProperty, loadSuccessHandler) {
        var cache, handleServerResponse, prepSearchOpts, sanitizeColumns, sanitizeSearchCriteria, sanitizeSortOpts, setCache;
        cache = {};
        ...
        sanitizeColumns = function(opts) {
          if (opts.columns) {
            opts.fields = opts.columns.join(',');
            return delete opts.columns;
          }
        };
        prepSearchOpts = function(searchOpts) {
          var sOpts;
          sOpts = $.extend({}, searchOpts);
          sanitizeColumns(sOpts);
      ...
      publicMethods.Variant = newCoreModuleClient('variants', 'variant');
      publicMethods.Variant.vendorProductCache = {};
      publicMethods.Variant.forVendorProduct = function(vendorId, productId) {
        var d, key, vpc;
        vpc = publicMethods.Variant.vendorProductCache;
        key = vendorId.toString() + "-" + productId.toString();
        d = $q.defer();
        if (vpc[key]) {
          d.resolve(vpc[key]);
        } else {
          $http.get('/api/v1/variants/for_vendor_product/' + vendorId + '/' + productId + '.json').then(function(resp) {
            vpc[key] = resp.data.variants;
            return d.resolve(vpc[key]);
          });
        }
        return d.promise;
      };
      ...
      publicMethods.Shipment = newCoreModuleClient('shipments', 'shipment');
      publicMethods.Shipment.__innerLoad = publicMethods.Shipment.load;
      publicMethods.Shipment.load = function(id, queryOpts) {
        var defaultQueryOpts, key, value;
        defaultQueryOpts = {
          booking_lines: 'true',
          include: 'order_lines',
          shipment_lines: 'true'
        };
        for (key in queryOpts) {
          value = queryOpts[key];
          defaultQueryOpts[key] = value;
        }
        return publicMethods.Shipment.__innerLoad(id, defaultQueryOpts);
      };
      publicMethods.Shipment.__save = publicMethods.Shipment.save;
      publicMethods.Shipment.save = function(shp) {
        if (shp.shp_imp_id) {
          delete shp.shp_imp_syscode;
        }
        return publicMethods.Shipment.__save(shp);
      };
      publicMethods.Shipment.bookOrder = function(shipment, order) {
        return $http.put('/api/v1/shipments/' + shipment.id + '/book_order/' + order.id + '.json', {
          order_id: order.id
        }).then(function(resp) {
          publicMethods.Shipment.removeFromCache(shipment.id);
          return true;
        });
      };
      publicMethods.Shipment.bookOrderToNewShipment = function(order) {
        return $http.post('/api/v1/shipments/booking_from_order/' + order.id + '.json', {
          order_id: order.id
        }).then(function(resp) {
          return resp.data.shipment;
        });
      };
      publicMethods.Shipment.requestBooking = function(shipment) {
        return $http.post('/api/v1/shipments/' + shipment.id + '/request_booking.json', {
          id: shipment.id
        }).then(function(resp) {
          return resp.data;
        });
      };
      publicMethods.Shipment.reviseBooking = function(shipment) {
        return $http.post('/api/v1/shipments/' + shipment.id + '/revise_booking.json', {
          id: shipment.id
        }).then(function(resp) {
          return resp.data;
        });
      };
      publicMethods.Shipment.requestCancel = function(shipment) {
        return $http.post('/api/v1/shipments/' + shipment.id + '/request_cancel.json', {
          id: shipment.id
        }).then(function(resp) {
          return resp.data;
        });
      };
      publicMethods.Shipment.uncancel = function(shipment) {
        return $http.post('/api/v1/shipments/' + shipment.id + '/uncancel.json', {
          id: shipment.id
        }).then(function(resp) {
          return resp.data;
        });
      };
      publicMethods.Shipment.sendShipmentInstructions = function(shipment) {
        return $http.post('/api/v1/shipments/' + shipment.id + '/send_shipment_instructions.json', {
          id: shipment.id
        }).then(function(resp) {
          return resp.data;
        });
      ...
      publicMethods.Admin = newAdminClient();
      ...
      newBusinessRuleClient = function() {
        return {
          forModule: function(moduleType, objectId) {
            return $http.get('/api/v1/business_rules/for_module/' + moduleType + '/' + objectId + '.json').then(function(resp) {
              return resp.data.business_rules;
            });
          },
          refresh: function(moduleType, objectId) {
            return $http.post('/api/v1/business_rules/refresh/' + moduleType + '/' + objectId + '.json', {
      ...
      publicMethods.BusinessRule = newBusinessRuleClient();
      newAttachmentClient = function() {
        ...
        return {
          ...
          attachmentTypes: function(moduleType, objectId) {
            var defer;
            defer = $q.defer();
            if (types) {
              defer.resolve(types);
            } else {
              $http.get('/api/v1/' + moduleType + '/' + objectId + '/attachment_types.json').then(function(resp) {
                types = resp.data.attachment_types;
                return defer.resolve(types);
              });
            }
            return defer.promise;
          },
          upload: function(moduleType, objectId, file, attachmentAttributes) {
            ...
            xhr.open("POST", '/api/v1/' + moduleType + '/' + objectId + '/attachments.json', true);
  ...
  angular.module('ChainCommon').directive('chainAttachmentsPanel', [
    'chainApiSvc', function(chainApiSvc) {
      return {
        ...
        link: function(scope, el, attrs) {
          var load, prepGroupedAttachments;
          prepGroupedAttachments = function(attachments) {
            var ary, att, ga, i, len, type, typeLabel;
            ga = {};
            for (i = 0, len = attachments.length; i < len; i++) {
              att = attachments[i];
              typeLabel = att.att_attachment_type;
              if (typeLabel === null) {
                typeLabel = "[none]";
              }
              ary = ga[typeLabel];
            ...
            scope.groupedAttachments = [];
            for (type in ga) {
              ary = ga[type];
              scope.groupedAttachments.push({
                type: type,
                attachments: ary
              });
            }
            return scope.groupedAttachments.sort(function(a, b) {
              if (a.type > b.type) {
                return 1;
              }
              if (a.type < b.type) {
          ...
          scope.toggleGroup = function(attachmentGroup) {
            var att, hide, i, len, ref;
            hide = !attachmentGroup.hidden;
            ref = attachmentGroup.attachments;
            for (i = 0, len = ref.length; i < len; i++) {
              att = ref[i];
              att.hidden = hide;
            }
            return attachmentGroup.hidden = !attachmentGroup.hidden;
          };
          scope.showInfo = function(att) {
            scope.modalAtt = att;
            el.find('.modal.attachment-info').modal('show');
            return null;
          };
          load = function() {
            delete scope.attachments;
            scope.loading = 'loading';
            return chainApiSvc.Attachment.forModule(scope.moduleType, scope.parent.id).then(function(atts) {
              var a, i, len, val, withMs;
              for (i = 0, len = atts.length; i < len; i++) {
                a = atts[i];
                val = a.att_updated_at;
                if (val) {
                  withMs = (val.length === 25 ? val.slice(0, 19) + '.000' + val.slice(19) : val);
                  a.friendly_uploaded_at = moment(withMs).format('YYYY-MM-DD HH:mm ZZ');
                }
              }
              prepGroupedAttachments(atts);
              return delete scope.loading;
            });
          };
          load();
          return scope.$on('chain-attachment-added', function() {
            return load();
          });
        }
      };
...
(function() {
  angular.module('ChainCommon').directive('chainBusinessRulesBasicModal', [
    'chainApiSvc', function(chainApiSvc) {
      return {
        restrict: 'E',
        scope: {
        ...
        templateUrl: 'chain-business-rules-basic-modal.html',
        link: function(scope, el, attrs) {
          var findRuleStateAttribute;
          findRuleStateAttribute = function(obj) {
            var key, rsa, value;
            rsa = null;
            for (key in obj) {
              value = obj[key];
              if (key.match(/_rule_state$/)) {
                rsa = key;
              }
            }
            return rsa;
          };
          scope.ruleState = function(obj) {
            var r, rsa;
            r = 'Unknown';
            if (obj) {
              rsa = findRuleStateAttribute(obj);
              if (rsa) {
                r = obj[rsa];
              }
            }
          ...
          scope.stateClass = function(state) {
            ...
            lc = labelClasses[state];
            if (!lc) {
              lc = 'label-default';
            }
            return lc;
          };
          scope.labelClass = function(obj) {
            return scope.stateClass(scope.ruleState(obj));
          };
          scope.showModal = function(obj) {
            delete scope.ruleResults;
            scope.loading = 'loading';
            chainApiSvc.BusinessRule.forModule(scope.moduleType, obj.id).then(function(rr) {
              scope.ruleResults = rr;
              delete scope.loading;
              return obj[findRuleStateAttribute(obj)] = rr.state;
            });
            el.find('.modal').modal('show');
            return null;
          };
          return scope.refresh = function(obj) {
            delete scope.ruleResults;
            scope.loading = 'loading';
            return chainApiSvc.BusinessRule.refresh(scope.moduleType, obj.id).then(function(rr) {
              scope.ruleResults = rr;
              delete scope.loading;
              return obj[findRuleStateAttribute(obj)] = rr.state;
            });
...
(function() {
  ...
  cDom.factory('chainDomainerSvc', [
    '$http', 'domainerSvc', function($http, domainerSvc) {
      var domainDAOChain, domainDAOChainFunc, setupDone;
      domainDAOChainFunc = function() {
        var cachedPromise;
        cachedPromise = null;
        return {
...
(function() {
  angular.module('ChainCommon').directive('chainFileUpload', [
    'chainApiSvc', function(chainApiSvc) {
      return {
        restrict: "E",
        scope: {
        ...
        templateUrl: 'chain-file-upload.html',
        link: function(scope, el, attrs) {
          var initAttachmentTypes;
          initAttachmentTypes = function() {
            return chainApiSvc.Attachment.attachmentTypes(scope.moduleType, scope.objectId).then(function(types) {
              return scope.attachmentTypes = types;
            });
          };
          scope.upload = {};
          scope.uploadAttachment = function() {
            ...
            return promise = chainApiSvc.Attachment.upload(scope.moduleType, scope.objectId, scope.upload.file, attachment).then((function(attachment) {
              scope.$emit("chain-attachment-added", attachment);
              scope.upload = {};
              el.find('.modal').modal('hide');
              return delete scope.uploading;
            }), (function(error) {
              return delete scope.uploading;
            }), (function(progress) {
          ...
          return el.find('.modal').on('show.bs.modal', function() {
            return initAttachmentTypes();
          });
        }
      };
    }
  ...
  angular.module("ChainCommon").directive("uploadFileOnChange", function() {
    return {
      restrict: "A",
...
angular.module('ChainCommon-Templates', ['chain-attachments-panel.html', 'chain-bulk-edit-modal.html', 'chain-bulk-execute-modal.html', 'chain-business-rules-basic-modal.html', 'chain-change-password-modal.html', 'chain-comments-panel.html', 'chain-errors.html', 'chain-file-upload.html', 'chain-messages-modal.html', 'chain-search-panel.html', 'chain-search-table.html', 'chain-state-toggle-buttons.html', 'chain-support-modal.html']);
...
angular.module("chain-attachments-panel.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("chain-attachments-panel.html",
    "<div class=\"panel panel-primary\"><div class=\"panel-heading\"><h3 class=\"panel-title\">Attachments</h3></div><div class=\"panel-body\" ng-if=\"scope.loading\"><chain-loading-wrapper loading-flag=\"{{loading}}\"></chain-loading-wrapper></div><ul class=\"list-group\" ng-repeat=\"ag in groupedAttachments track by $index\"><li class=\"list-group-item clickable-list-item\" ng-click=\"toggleGroup(ag)\"><strong>{{ag.type}} ({{ag.attachments.length}})</strong></li><li ng-repeat=\"att in ag.attachments track by att.id\" class=\"list-group-item\" ng-hide=\"att.hidden\"><button class=\"btn btn-xs btn-danger pull-right\" ng-click=\"deleteAttachment(att.id)\" href=\"javascript:;\" ng-if=\"canAttach\"><i class=\"fa fa-trash\"></i></button> <a href=\"/attachments/{{att.id}}/download\" target=\"_blank\">{{att.att_file_name}}</a> <i class=\"fa fa-info-circle clickable-list-item\" ng-click=\"showInfo(att)\" title=\"Information\"></i></li></ul><div class=\"panel-footer text-right\" ng-if=\"parent.id && parent.permissions.can_attach\"><chain-file-upload module-type=\"{{moduleType}}\" object-id=\"{{parent.id}}\"></chain-file-upload></div></div><div class=\"modal fade attachment-info\" tabindex=\"-1\" role=\"dialog\" aria-labelledby=\"\" aria-hidden=\"true\"><div class=\"modal-dialog\"><div class=\"modal-content\"><div class=\"modal-header\"><button type=\"button\" class=\"close\" data-dismiss=\"modal\" aria-hidden=\"true\">&times;</button><h4 class=\"modal-title\">{{modalAtt.att_file_name}}</h4></div><div class=\"modal-body\"><div class=\"form-group\"><label>Uploaded By</label><p>{{modalAtt.att_uploaded_by_fullname}} ({{modalAtt.att_uploaded_by_username}})</p></div><div class=\"form-group\"><label>Uploaded Date</label><p>{{modalAtt.friendly_uploaded_at}}</p></div><div class=\"form-group\"><label>Size</label><p>{{modalAtt.friendly_size}}</p></div><div class=\"form-group\"><label>Type</label><p>{{modalAtt.att_attachment_type}}</p></div></div><div class=\"modal-footer\"><button type=\"button\" class=\"btn btn-default\" data-dismiss=\"modal\">Close</button></div></div></div></div>");
}]);
...
angular.module("chain-business-rules-basic-modal.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("chain-business-rules-basic-modal.html",
    "<a class=\"label {{labelClass(coreObject)}}\" ng-click=\"showModal(coreObject)\">{{ruleState(coreObject)}}</a><div class=\"modal fade\"><div class=\"modal-dialog\"><div class=\"modal-content\"><div class=\"modal-header\"><h4 class=\"modal-title\">Business Rules <span class=\"label {{labelClass(coreObject)}}\">{{ruleState(coreObject)}}</span></h4></div><div class=\"modal-body\"><chain-loading-wrapper loading-flag=\"{{loading}}\"></chain-loading-wrapper><div ng-if=\"ruleResults.state && ruleResults.bv_results.length==0\">No business rules exist.</div><div ng-repeat=\"bv in ruleResults.bv_results track by bv.id\"><table class=\"table\"><thead><tr><th colspan=\"2\">{{bv.template.name}}</th></tr></thead><tbody><tr ng-repeat=\"rr in bv.rule_results track by rr.id\"><td><abbr title=\"{{rr.rule.description}}\">{{rr.rule.name}}</abbr></td><td><span class=\"label {{stateClass(rr.state)}}\">{{rr.state}}</span></td></tr></tbody></table></div></div><div class=\"modal-footer\"><button type=\"button\" ng-click=\"refresh(coreObject)\" title=\"Refresh\" class=\"btn btn-default\"><i class=\"fa fa-refresh\"></i></button> <button type=\"button\" class=\"btn btn-default\" data-dismiss=\"modal\">Close</button></div></div></div></div>");
}]);
...
angular.module("chain-file-upload.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("chain-file-upload.html",
    "<button title=\"Attach File\" class=\"btn btn-sm btn-success\" ng-click=\"showModal()\"><i class=\"fa fa-plus\"></i></button><div class=\"modal fade text-left\" tabindex=\"-1\" role=\"dialog\" aria-labelledby=\"\" aria-hidden=\"true\"><div class=\"modal-dialog\"><div class=\"modal-content\"><div class=\"modal-header\"><button type=\"button\" class=\"close\" data-dismiss=\"modal\" aria-hidden=\"true\">&times;</button><h4 class=\"modal-title\">Attach File</h4></div><div class=\"modal-body\"><chain-loading-wrapper loading-flag=\"{{loading}}\"></chain-loading-wrapper><div ng-if=\"error\" class=\"alert alert-danger\" role=\"alert\">{{error}}</div><div ng-if=\"!uploading && !loading\"><div class=\"form-group\"><label for=\"file-{{moduleType}}-{{objectId}}-upload-file\">Add File</label><input type=\"file\" name=\"upload.file\" id=\"file-{{moduleType}}-{{objectId}}-upload-file\" upload-file-on-change=\"fileChosen\"></div><div class=\"form-group\"><label for=\"file-{{moduleType}}-{{objectId}}-upload-file-type\">File Type</label><select ng-model=\"upload.fileType\" ng-options=\"option.value as option.name for option in attachmentTypes\" name=\"upload.fileType\" class=\"form-control\" id=\"file-{{moduleType}}-{{objectId}}-upload-file-type\"></select></div></div><div ng-if=\"uploading\"><div class=\"progress progress-striped active\"><div class=\"progress-bar\" role=\"progress-bar\"><span class=\"sr-only\">Uploading</span></div></div><div>{{uploadStatus}}</div></div></div><div class=\"modal-footer\"><button type=\"button\" class=\"btn btn-default\" data-dismiss=\"modal\">Close</button> <button title=\"Upload selected file\" ng-disabled=\"!upload.file\" type=\"button\" class=\"btn btn-success\" ng-click=\"uploadAttachment()\"><i class=\"fa fa-upload\" aria-hidden=\"true\"></i></button></div></div></div></div>");
}]);

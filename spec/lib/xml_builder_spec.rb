describe OpenChain::XmlBuilder do
  ...
  subject { Class.new { include OpenChain::XmlBuilder }.new }

  let! (:root) do
    *, root = subject.build_xml_document "root"
    root
  end

  describe "build_xml_document" do
    it "builds a document with a root element" do
      doc, root = subject.build_xml_document "root"
    ...
    it "suppresses xml declaration" do
      doc, root = subject.build_xml_document "root", suppress_xml_declaration: true
  ...
  describe "add_element" do
    it "adds a child element to a parent" do
      el = subject.add_element root, "child", "content"
      expect(el.name).to eq "child"
      expect(el.text).to eq "content"
    end

    it "adds blank elements by default" do
      el = subject.add_element root, "child", ""
      expect(el.text).to eq ""
    end

    it "skips blank elements if instructed" do
      expect(subject.add_element root, "child", nil, allow_blank: false).to be_nil
    end

    it "wraps content in cdata if instructed" do
      el = subject.add_element root, "child", "cdata", cdata: true
      expect(el.children.first).to be_instance_of(REXML::CData)
      expect(el.children.first.to_s).to eq "cdata"
    end

    it "allows for passing attributes" do
      el = subject.add_element root, "child", "content", {attrs: {"attribute" => "value", "attribute2" => "value2"}}
      expect(el.text).to eq "content"
      expect(el.attributes["attribute"]).to eq "value"
      expect(el.attributes["attribute2"]).to eq "value2"
    end
  end
  ...
  describe "add_collection_element" do
    ...
    it "splits values apart and adds children/grandchildren to a parent" do
      child = subject.add_collection_element root, "Children", "Child", "Thing 1\n Thing 2"
    ...
    it "uses custom split expression" do
      child = subject.add_collection_element root, "Children", "Child", "Thing 1~Thing 2", split_expression: "~"
      expect(child.children.size).to eq 2
      expect(REXML::XPath.each(root, "Children/Child").collect {|v| v.text}).to eq ["Thing 1", "Thing 2"]
    end

    it "uses custom regex split expression" do
      child = subject.add_collection_element root, "Children", "Child", "Thing 1 ~ Thing 2", split_expression: /\s~\s/
      expect(child.children.size).to eq 2
      expect(REXML::XPath.each(root, "Children/Child").collect {|v| v.text}).to eq ["Thing 1", "Thing 2"]
    end
  ...
  describe "add_date_elements" do
    ...
    it "adds date and time elements to parent" do
      date_el = subject.add_date_elements root, Date.new(2015, 1, 1)
    ...
    it "adds datetime to parent" do
      date_el = subject.add_date_elements root, DateTime.new(2015, 1, 1, 12, 15)
      expect(date_el.text "Date").to eq "20150101"
      expect(date_el.text "Time").to eq "1215"
    end

    it "adds date and time elements to parent, prefixing children" do
      date_el = subject.add_date_elements root, Date.new(2015, 1, 1), element_prefix: "Some"
      expect(date_el.text "SomeDate").to eq "20150101"
      expect(date_el.text "SomeTime").to eq "0000"
    end

    it "accepts custom date/time formats" do
      date_el = subject.add_date_elements root, DateTime.new(2015, 1, 1, 12, 15), date_format: "%Y-%m-%d", time_format: "%H:%M"
    ...
    it "adds date and time elements to parent creating child element" do
      date_el = subject.add_date_elements root, Date.new(2015, 1, 1), child_element_name: "MyDate"
    ...
    it "reuses child element if it already exists" do
      child = subject.add_element root, "Child"

      date_el = subject.add_date_elements root, Date.new(2015, 1, 1), child_element_name: "Child"
  ...
  describe "add_entity_address_info" do
    ...
    it "adds address info to parent" do
      add = subject.add_entity_address_info root, "Entity", name: "EntityName", id: "EntityId", address_1: "Add1", address_2: "Add2", city: "AI", state: "St", zip: "123", country: "US"

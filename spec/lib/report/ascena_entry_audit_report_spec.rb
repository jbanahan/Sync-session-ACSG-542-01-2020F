...
describe OpenChain::Report::AscenaEntryAuditReport do
  ...
  describe "run_report" do
    ...
    it "generates spreadsheet based on fiscal_date" do
      ...
      stub_master_setup
      @temp = described_class.run_report(u, {'start_fiscal_year_month' => '2016-11', 'end_fiscal_year_month' => '2017-02', 'range_field' => 'fiscal_date'})
      wb = Spreadsheet.open @temp.path
      sheet = wb.worksheets[0]
      expect(sheet.rows.count).to eq 3
    end
  ...
  describe "query" do
    ...
    it "skips entries whose release date isn't in the range" do
      result = ActiveRecord::Base.connection.execute(report.query '2016-04-1', '2016-04-2', 'release_date', cdefs)
      expect(result.count).to eq 0
    end

    it "skips entries that don't have customer number 'ASCE'" do
      @ent.update_attributes(customer_number: "FOO")
      result = ActiveRecord::Base.connection.execute(report.query '2016-03-14', '2016-03-20', 'release_date', cdefs)
      expect(result.count).to eq 0
    end

describe OpenChain::Report::AscenaEntryAuditReport do
  ...
  let(:co) { with_customs_management_id create(:importer, name: "Ascena", system_code: "ASCENA"), "ASCE" }
  ...
  def create_data
    vend = create(:company, name: "vend name", system_code: "vend sys code")
    fact = create(:company, name: "fact name", system_code: "fact sys code")
    @ent = create(:entry, customer_number: 'ASCE', broker_reference: 'brok ref', entry_number: 'ent num', entry_type: 'ent type', first_release_date: date_1,
    ...
    ci = create(:commercial_invoice, entry: @ent, invoice_number: 'inv num', invoice_value: 1, non_dutiable_amount: 2)
    @cil = create(:commercial_invoice_line, commercial_invoice: ci, po_number: 'po num', part_number: "part num", product_line: 'prod line', country_origin_code: 'coo',
    ...
    @cit = create(:commercial_invoice_tariff, commercial_invoice_line: @cil, hts_code: 'hts', duty_rate: 1, spi_primary: 'spi', classification_qty_1: 2,
    ...
    @cit_2 = create(:commercial_invoice_tariff, commercial_invoice_line: @cil, hts_code: 'hts2', duty_rate: 2, spi_primary: 'spi2', classification_qty_1: 3,
                     ...
                     duty_amount: 7)
    create(:port, name: 'fenix port', cbsa_port: '0123')
    create(:port, name: 'alliance port', schedule_d_code: '0123')
    create(:container, entry: @ent)
    @prod = create(:product, unique_identifier: 'ASCENA-part num')
    @ord = create(:order, order_number: 'ASCENA-prod line-po num', vendor: vend, factory: fact)
    @ord.find_and_set_custom_value(cdefs[:ord_type], 'AGS')
    @ord.find_and_set_custom_value(cdefs[:ord_selling_agent], 'agent')
    @ord.save!
    ordln = create(:order_line, order: @ord, product: @prod, price_per_unit: 1)
  ...
  describe "run_report" do
    let(:u) { create(:user, time_zone: "Eastern Time (US & Canada)", company: co) }
    ...
    it "generates spreadsheet based on fiscal_date" do
      ...
      create(:fiscal_month, company: co, year: 2016, month_number: 11, start_date: '2016-11-05', end_date: '2016-12-10')
      create(:fiscal_month, company: co, year: 2017, month_number: 2, start_date: '2017-02-07', end_date: '2017-03-06')
      stub_master_setup

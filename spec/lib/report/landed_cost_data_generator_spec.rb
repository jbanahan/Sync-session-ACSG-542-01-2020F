describe OpenChain::Report::LandedCostDataGenerator do
  ...
  before :each do
    @entry = create(:entry, :file_logged_date=>Time.zone.now, :customer_name=>"Test", :customer_references=>"A\nB\nPO", :po_numbers=>"PO\nPO2", :importer_id=>5, :release_date=>Time.zone.now, :transport_mode_code=>"2")
    @bi = create(:broker_invoice, :entry=>@entry)

    @bi_line_brokerage = create(:broker_invoice_line, :broker_invoice => @bi, :charge_type=>"R", :charge_amount=> BigDecimal.new("100"))
    @bi_line_other = create(:broker_invoice_line, :broker_invoice => @bi, :charge_type=>"O", :charge_amount => BigDecimal.new("100"))
    @bi_line_freight = create(:broker_invoice_line, :broker_invoice => @bi, :charge_type=>"F", :charge_amount => BigDecimal.new("100"), charge_code: "0600")
    @bi_line_inland = create(:broker_invoice_line, :broker_invoice => @bi, :charge_type=>"T", :charge_amount => BigDecimal.new("100"))

    @ci = create(:commercial_invoice, :entry=>@entry, :invoice_number=>"INV1")
    @ci_line_1 = create(:commercial_invoice_line, :commercial_invoice=>@ci, :part_number=>"Part1", :quantity=>BigDecimal.new("11"), :po_number=>"PO", :mid=>"MID1", :country_origin_code=>"CN",
    ...
    @ci_line_2 = create(:commercial_invoice_line, :commercial_invoice=>@ci, :part_number=>"Part2", :quantity=>BigDecimal.new("11"), :po_number=>"PO2", :mid=>"MID2", :country_origin_code=>"TW",
                          :commercial_invoice_tariffs=>[CommercialInvoiceTariff.new(:duty_amount=>BigDecimal.new("250"), :entered_value=>BigDecimal.new("750"))])

    @ci_line_3 = create(:commercial_invoice_line, :commercial_invoice=>@ci, :part_number=>"Part3", :quantity=>BigDecimal.new("11"), :po_number=>"PO2", :mid=>"MID2", :country_origin_code=>"TW",
  ...
  context "landed_cost_data_for_entry" do
    ...
    it "should prorate international freight against a single invoice if specified" do
      ...
      ci_2 = create(:commercial_invoice, :entry=>@entry, :invoice_number=>"INV2")
      ci_2_line_1 = create(:commercial_invoice_line, :commercial_invoice=>ci_2, :part_number=>"Part4", :quantity=>BigDecimal.new("11"), :po_number=>"PO", :mid=>"MID4", :country_origin_code=>"CN", :value => BigDecimal.new("1000"),

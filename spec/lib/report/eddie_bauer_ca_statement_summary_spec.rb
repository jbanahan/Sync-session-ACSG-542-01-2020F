...
describe OpenChain::Report::EddieBauerCaStatementSummary do
  ...
  describe "run" do
    before :each do
      @entry = create(:entry, entry_number: '123456789', customer_number: "EBCC", entry_filed_date: '2014-03-01')
      @commercial_invoice = create(:commercial_invoice, entry: @entry, invoice_number: "A")
      @cil =  create(:commercial_invoice_line, commercial_invoice: @commercial_invoice, po_number: "ABC-123", hmf: 1, prorated_mpf: 2, cotton_fee: 3, country_origin_code: "CN")
      ...
      @cil2 =  create(:commercial_invoice_line, commercial_invoice: @commercial_invoice, po_number: "DEF-456", hmf: 4, prorated_mpf: 5, cotton_fee: 6, country_origin_code: "CA")
      ...
      @broker_invoice = create(:broker_invoice, entry: @entry, invoice_date: '2014-03-01')
      @broker_invoice_line1 = create(:broker_invoice_line, broker_invoice: @broker_invoice, charge_amount: 5)
      @broker_invoice_line2 = create(:broker_invoice_line, broker_invoice: @broker_invoice, charge_amount: 15, charge_type: "D")
    ...
    it "outputs statement information for all entries w/ invoice dates between specified dates" do
      @t = described_class.new.run create(:master_user, time_zone: "Eastern Time (US & Canada)"), start_date: '2014-02-28', end_date: '2014-03-02'
    ...
    it "prevents users who do not have access to the entry from seeing them" do
      @t = described_class.new.run create(:user, time_zone: "Eastern Time (US & Canada)"), start_date: '2014-02-28', end_date: '2014-03-02'
      sheet = Spreadsheet.open(@t.path).worksheet 0
      expect(sheet.rows.length).to eq 1
    end

    it "excludes invoices outside the start and end date range on an entry" do
      prior_invoice = create(:broker_invoice_line, broker_invoice: create(:broker_invoice, entry: @entry, invoice_date: '2014-02-01'), charge_amount: 5).broker_invoice
      post_invoice = create(:broker_invoice_line, broker_invoice: create(:broker_invoice, entry: @entry, invoice_date: '2014-05-01'), charge_amount: 5).broker_invoice
    ...
    it "excludes entries with invoices that zero each other out when there's no duty" do
      ...
      other_invoice = create(:broker_invoice_line, broker_invoice: create(:broker_invoice, entry: @entry, invoice_date: '2014-03-01'), charge_amount: -5).broker_invoice

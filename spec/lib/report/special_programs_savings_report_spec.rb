describe OpenChain::Report::SpecialProgramsSavingsReport do
  ...
  context "run" do
    before do
      ...
      @entry = create(:entry, :import_country=>@country, :customer_number=>"SPECIAL", :release_date=>Time.zone.now, :broker_reference=>'99123456',
      ...
      ['Invoice 1'].each do |inv_num|
        invoice = create(:commercial_invoice, :entry_id=>@entry.id, :invoice_number => inv_num)
        @invoices << invoice
      end

      ['Invoice Line 1'].each_with_index do |inv_line, inv_num|
        invoice_line = create(:commercial_invoice_line, :commercial_invoice=>@invoices[inv_num], :po_number=>inv_line,
                               :country_origin_code=>'US', :part_number=>'1234')
        @invoice_lines << invoice_line
      end

      ['Invoice Tariff 1'].each_with_index do |inv_line, inv_num|
        invoice_tariff = create(:commercial_invoice_tariff, :commercial_invoice_line=>@invoice_lines[inv_num],
      ...
      @ot = create(:official_tariff, :country=>@country, :hts_code=>'123456789', :common_rate_decimal=>0.18)
    end

describe OpenChain::FeedMonitor do
  ...
  context "Alliance" do
    ...
    it "should error if last entry is more than 2 hours ago between M-F 8:00-20:00" do
      FactoryBot(:entry, :source_system=>'Alliance', :last_exported_from_source=>@est.parse("2012-10-01 4:00"))
    ...
    it "should not error if last entry is less than 2 hours ago" do
      FactoryBot(:entry, :source_system=>'Alliance', :last_exported_from_source=>@est.parse("2012-10-01 11:00"))
      OpenChain::FeedMonitor.monitor @est.parse("2012-10-01 12:00")
      expect(ErrorLogEntry.count).to eq(0)
    end
  end
  ...
  context "Alliance Images" do
    ...
    before :each do
      @ent = FactoryBot(:entry, :source_system=>'Alliance')
  ...
  context "Fenix" do
    ...
    it "should error if last entry is more than 2 hours ago between M-F 8:00-20:00 EST" do
      FactoryBot(:entry, :source_system=>'Fenix', :last_exported_from_source=>@est.parse("2012-10-01 4:00"))
      OpenChain::FeedMonitor.monitor @est.parse("2012-10-01 12:00")
    ...
    it "should not error if last entry is less than 2 hours ago" do
      FactoryBot(:entry, :source_system=>'Fenix', :last_exported_from_source=>@est.parse("2012-10-01 11:00"))
      OpenChain::FeedMonitor.monitor @est.parse("2012-10-01 21:00")
      expect(ErrorLogEntry.count).to eq(0)
    end
  end

describe OpenChain::OfficialTariffProcessor::GenericProcessor do
  ...
  describe "process" do
    let(:country) { country = Factory(:country, iso_code:"US", european_union:false) }
  ...
  describe "parse_data_for" do
    it "should return parse info for handled countries" do
      expect(described_class.parse_data_for Factory(:country, iso_code:"US", european_union:false)).to_not be_nil
      expect(described_class.parse_data_for Factory(:country, iso_code:"CA", european_union:false)).to_not be_nil
      expect(described_class.parse_data_for Factory(:country, iso_code:"FR", european_union:true)).to_not be_nil
      expect(described_class.parse_data_for Factory(:country, iso_code:"DE", european_union:true)).to_not be_nil
      expect(described_class.parse_data_for Factory(:country, iso_code:"CL", european_union:false)).to_not be_nil
      expect(described_class.parse_data_for Factory(:country, iso_code:"CN", european_union:false)).to_not be_nil
      expect(described_class.parse_data_for Factory(:country, iso_code:"MX", european_union:false)).to_not be_nil
      expect(described_class.parse_data_for Factory(:country, iso_code:"SG", european_union:false)).to_not be_nil
    end

    it "should rase an error for an unexpected country" do
      expect { described_class.parse_data_for Factory(:country, iso_code:"A+", european_union:false) }.to raise_error("No Special Program parser configured for A+")
    end
  end

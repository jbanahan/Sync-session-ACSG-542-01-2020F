describe OpenChain::KewillSqlProxyClient do
  ...
  let (:proxy_config) { {'auth_token' => "config_auth_token", "url" => "config_url"} }
  let(:http_client) do
  ...
  describe "request_alliance_invoice_details" do
    it "requests invoice details from alliance" do
      ...
      expect(http_client).to receive(:post).with("#{proxy_config['url']}/job/invoice_details", request_body)

      subject.request_alliance_invoice_details "123", "suffix     ", request_context
    end
    ...
    it "strips blank suffixes down to blank string" do
      ...
      subject.request_alliance_invoice_details "123", "     "
    end
    ...
    it "doesn't send context if a blank one is provided" do
      ...
      subject.request_alliance_invoice_details "123", "A"
    end

    it "raises error on errored post" do
      expect(http_client).to receive(:post).and_raise "Error"
      expect {subject.request_alliance_invoice_details "123", "A"}.to raise_error "Error"
    end
  end

  describe "request_alliance_invoice_numbers_since" do
    it "requests invoice numbers since given date" do
      request_body = {'job_params' => {invoice_date: 20140101}}  # rubocop:disable Style/NumericLiterals
      expect(http_client).to receive(:post).with("#{proxy_config['url']}/job/find_invoices", request_body)

      subject.request_alliance_invoice_numbers_since Date.new(2014, 1, 1)
    end
  end

  describe "request_check_details" do
    it "requests check details" do
      request_body = {'job_params' => {file_number: 123, check_number: 456, check_date: 20141101, bank_number: 10, check_amount: 101}}  # rubocop:disable Style/NumericLiterals

      expect(http_client).to receive(:post).with("#{proxy_config['url']}/job/check_details", request_body)
      subject.request_check_details "123", "456", Date.new(2014, 11, 1), "10", BigDecimal("1.01999").to_s
    end

    it "raises error on failed json post" do
      expect(http_client).to receive(:post).and_raise "Error"
      expect {subject.request_check_details "123", "456", Date.new(2014, 11, 1), "10", BigDecimal("123")}.to raise_error "Error"
    end
  end
  ...
  describe "request_file_tracking_info" do
    it "requests files between given times" do
      ...
      request_body = {'job_params' => {start_date: start.strftime("%Y%m%d").to_i, end_date: end_t.strftime("%Y%m%d").to_i,
                                       end_time: end_t.strftime("%Y%m%d%H%M").to_i}, 'context' => {results_as_array: true}}
      expect(http_client).to receive(:post).with("#{proxy_config['url']}/job/file_tracking", request_body)

      subject.request_file_tracking_info start, end_t
    end
  end
  ...
  describe "request_updated_entry_numbers" do
    it "requests updated entry data for given time period" do
      ...
      expect(http_client).to receive(:post).with("#{proxy_config['url']}/job/updated_entries", request_body)
      subject.request_updated_entry_numbers start, end_t, ""
    end
    ...
    it "adds customer numbers to request if present" do
      ...
      expect(http_client).to receive(:post).with("#{proxy_config['url']}/job/updated_entries", request_body)
      subject.request_updated_entry_numbers start, end_t, ["CUST1", "CUST2"]
    end
    ...
    it "does not swallow errors ocurring during the request" do
      ...
      request_body = {'job_params' => {start_date: start.strftime("%Y%m%d%H%M"), end_date: end_t.strftime("%Y%m%d%H%M")}}
      expect(http_client).to receive(:post).with("#{proxy_config['url']}/job/updated_entries", request_body).and_raise "Error"

      expect {subject.request_updated_entry_numbers start, end_t, ""}.to raise_error "Error"
    end
  end
  ...
  describe "bulk_request_entry_data" do
    ...
    it "uses primary key values to request entry data" do
      ...
      subject.bulk_request_entry_data primary_keys: [entry.id]
    end

    it "skips non-alliance entries" do
      entry.update! source_system: "Not Alliance"
      expect_any_instance_of(subject).not_to receive(:request_entry_data).with "123456"
      subject.bulk_request_entry_data primary_keys: [entry.id]
    end

    it "uses s3 data to pull keys" do
      expect_any_instance_of(subject).to receive(:request_entry_data).with "123456"
      expect(OpenChain::CoreModuleProcessor).to receive(:bulk_objects).with(CoreModule::ENTRY, {primary_keys: nil,
                                                                                                primary_key_file_bucket: "bucket", primary_key_file_path: "key"})
                                                                      .and_yield(1, entry)
      subject.bulk_request_entry_data s3_bucket: "bucket", s3_key: "key"
    end
  end
  ...
  describe "delayed_bulk_entry_data" do
    ...
    it "proxies requests with search runs in them" do
      ...
      expect(subject).to receive(:bulk_request_entry_data).with(s3_bucket: "bucket", s3_key: "key")
      subject.delayed_bulk_entry_data search_run.id, nil
    end

    it "passes primary keys directly through" do
      expect(subject).to receive(:delay).and_return subject
      expect(subject).to receive(:bulk_request_entry_data).with(primary_keys: [1, 2, 3])
      subject.delayed_bulk_entry_data nil, [1, 2, 3]
    end
  end

  describe "request_mid_updates" do
    it "sends a request for mid updates" do
      expect(subject).to receive(:request).with("mid_updates", {updated_date: 20161010}, {results_as_array: true}, {swallow_error: false}) # rubocop:disable Style/NumericLiterals
  ...
  describe "request_updated_statements" do
    ...
    it "sends a request for updated statements with customer numbers" do
      my_params = nil
      expect(subject).to receive(:request) do |_path, params, _context, _opts|
        my_params = params
        nil
      end
  ...
  describe "request_monthly_statements_between" do
    it "requests statements received between given dates" do
      ...
      expect(my_params[:start_date]).to eq 20171201 # rubocop:disable Style/NumericLiterals
      expect(my_params[:end_date]).to eq 20171202 # rubocop:disable Style/NumericLiterals
    end
  ...
  describe "request_entry_data" do
    it "sends request for entry data to sql proxy system" do
      ...
      expect(subject).to receive(:request).with('entry_data_to_s3', {file_no: 12345}, {hash: :data}) # rubocop:disable Style/NumericLiterals
  ...
  describe "request_updated_tariff_classifications" do
    it "sends request for tariff data to sql proxy system" do
      expect(subject).to receive(:request).with "updated_tariffs_to_s3", {start_date: 201901301225, end_date: 201901301330},  # rubocop:disable Style/NumericLiterals
                                                {s3_bucket: "bucket", s3_path: "path", sqs_queue: "queue"}, {swallow_error: false}

      subject.request_updated_tariff_classifications(Time.zone.parse("2019-1-30 12:25"), Time.zone.parse("2019-1-30 13:30"), "bucket", "path", "queue")
    end
  end

  describe "request_updated_customers" do
    it "sends request for customer data to sql proxy system" do
      expect(subject).to receive(:request).with "updated_customers_to_s3", {start_date: "201901301225", end_date: "201901301330"},
                                                {s3_bucket: "bucket", s3_path: "path", sqs_queue: "queue"}, {swallow_error: false}

      subject.request_updated_customers(Time.zone.parse("2019-1-30 12:25"), Time.zone.parse("2019-1-30 13:30"), "bucket", "path", "queue")
    end
  end

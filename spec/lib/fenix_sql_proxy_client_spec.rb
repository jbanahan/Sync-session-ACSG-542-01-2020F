...
describe OpenChain::FenixSqlProxyClient do
  ...
  describe "request_images_added_between" do
    it "sends request using times in UTC" do
      ...
      expect(subject).to receive(:request).with('fenix_updated_documents', {start_date: "2015-09-10T02:00:00Z", end_date: "2015-09-10T02:05:00Z"}, {}, swallow_error: false)
  ...
  describe "request_images_for_transaction_number" do
    it "sends request" do
      expect(subject).to receive(:request).with('fenix_documents_for_transaction', {transaction_number: "12345"}, {}, swallow_error: false)

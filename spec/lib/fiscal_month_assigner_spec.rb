describe OpenChain::FiscalMonthAssigner do
  ...
  let(:ent) { Factory(:entry, entry_number: "entry num", importer: co, release_date: Date.new(2016,03,15), 
  ...
  describe "assign" do
    it "assigns entry and invoice fiscal_date/month/year fields" do
      ...
      expect(brok_inv.fiscal_date).to eq fm2.start_date
      expect(brok_inv.fiscal_month).to eq fm2.month_number
      expect(brok_inv.fiscal_year).to eq fm2.year
    end
    ...
    it "does nothing if company doesn't have fiscal calendar enabled" do
      ...
      expect(brok_inv.fiscal_date).to eq nil
      expect(brok_inv.fiscal_month).to eq nil
      expect(brok_inv.fiscal_year).to eq nil
    end
    ...
    it "assigns blank fiscal attributes if broker invoice date is blank" do
      ...
      described_class.assign ent
      brok_inv.reload
      expect(brok_inv.fiscal_date).to be_nil
      expect(brok_inv.fiscal_month).to be_nil
      expect(brok_inv.fiscal_year).to be_nil
    end
    ...
    it "raises exception if company's fiscal calendar is enabled but entry's date doesn't have a matching fiscal month" do
      ...
      expect{described_class.assign ent}.to raise_error "No fiscal month found for entry #entry num"
    end
    ...
    it "raises exception if company's fiscal calendar is enabled but broker invoice's invoice_date doesn't have a matching fiscal month" do
      ...
      expect{described_class.assign ent}.to raise_error "No fiscal month found for broker invoice #inv num"
    end

    it "raises exception if more than one fiscal month is found for an entry" do
      Factory(:fiscal_month, company: co, year: 2016, month_number: 5, start_date: Date.new(2016,3,2), end_date: Date.new(2016,3,20))
      expect{described_class.assign ent}.to raise_error "More than one fiscal month found for entry #entry num"
    end
    ...
    it "raises exception if more than one fiscal month is found for a broker invoice" do
      ...
      expect{described_class.assign ent}.to raise_error "More than one fiscal month found for broker invoice #inv num"
    end
  end
end
describe OpenChain::TariffFileMonitor do
  ...
  describe "run" do
    it "downloads and distributes tariff files" do
      ...
      expect(subject).to receive(:ftp_file).with(tempfile_matching_name("AU_SIMPLE_20170707.ZIP"), subject.connect_vfitrack_net("inst_1/tariff_file"))
      expect(subject).to receive(:ftp_file).with(tempfile_matching_name("CN_C9_SIMPLE_20170708.ZIP"), subject.connect_vfitrack_net("inst_1/tariff_file"))
      expect(subject).to receive(:ftp_file).with(tempfile_matching_name("IT_EU_SIMPLE_20170708.ZIP"), subject.connect_vfitrack_net("inst_1/tariff_file"))
      expect(subject).to receive(:ftp_file).with(tempfile_matching_name("AU_SIMPLE_20170707.ZIP"), subject.connect_vfitrack_net("inst_2/tariff_file"))
      expect(subject).to receive(:ftp_file).with(tempfile_matching_name("CN_SIMPLE_20170707.ZIP"), subject.connect_vfitrack_net("inst_2/tariff_file"))
      expect(subject).to receive(:ftp_file).with(tempfile_matching_name("NL_EU_SIMPLE_20170708.ZIP"), subject.connect_vfitrack_net("inst_2/tariff_file"))

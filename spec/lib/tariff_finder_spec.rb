...
describe OpenChain::TariffFinder do
  ...
  describe "find_by_style" do
    ...
    it "should find by style with nil country of origin" do
      ...
      r = described_class.new(@c1.iso_code, [@imp]).find_by_style 'p1'
    ...
    it "should find by style with country of origin filter" do
      ...
      r = described_class.new(@c1.iso_code, [@imp]).find_by_style 'p1', 'ZZ'
      expect(r.part_number).to eq 'p1'
      expect(r.country_origin_code).to eq 'ZZ'
      expect(r.mid).to eq 'q'
      expect(r.hts_code).to eq '123456789'
    ...
    it "skips entries with different origin country" do
      e = build_entry '123456789', 'p1', 'ZZ', @imp.id, @c1.id, 1.day.ago, 'q'
      expect(described_class.new("NOTANISO", [@imp]).find_by_style('p1', 'ZZ')).to be_nil
    end

    it "skips entries with wrong importer" do
      e = build_entry '123456789', 'p1', 'ZZ', @imp.id, @c1.id, 1.day.ago, 'q'
      expect(described_class.new(@c1.iso_code, [Factory(:company)]).find_by_style('p1', 'ZZ')).to be_nil
    end
  end
end

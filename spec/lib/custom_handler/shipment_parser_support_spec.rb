describe OpenChain::CustomHandler::ShipmentParserSupport::OrdersChecker do
  ...
  let(:prod1) { create(:product) }
  let(:prod2) { create(:product) }
  let(:ship1) { create(:shipment, reference: "REF1", booking_mode: "Ocean - FCL") }
  let(:ship2) { create(:shipment, reference: "REF2", booking_mode: "Ocean") }
  let(:ord1) { create(:order, order_number: "ORD1", customer_order_number: "CUSTORD1", mode: "Air", approval_status: "Accepted") }
  let(:ord2) { create(:order, order_number: "ORD2", customer_order_number: "CUSTORD2", mode: "Ocean", approval_status: "Accepted") }
  let(:ordln_1) { create(:order_line, order: ord1, product: prod1)}
  let(:ordln_2) { create(:order_line, order: ord2, product: prod2)}
  ...
  context "warnings" do
    describe "orders_with_mismatched_transport_mode" do
      it "returns customer-order numbers with mismatched transportation mode" do
        ...
        create(:order, importer: imp, order_number: "ORD3", customer_order_number: "CUSTORD3", mode: "Air")
    ...
    context "manifests" do
      let(:shipln_1) {create(:shipment_line, shipment: ship1, product: prod1)}
      let(:shipln_2) {create(:shipment_line, shipment: ship2, product: prod2)}
    ...
    context "bookings" do
      let!(:booking_1) { create(:booking_line, shipment: ship1, order_line: ordln_1, product: prod1) }
      let!(:booking_2) { create(:booking_line, shipment: ship2, order_line: ordln_2, product: prod2) }

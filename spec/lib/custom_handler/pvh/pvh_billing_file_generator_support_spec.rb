describe OpenChain::CustomHandler::Pvh::PvhBillingFileGeneratorSupport do
  ...
  let (:entry) {
    e = create(:entry, entry_number: "ENTRYNUM", broker_reference: "12345", importer_id: pvh.id, transport_mode_code: "10", customer_number: "PVH", container_numbers: "ABCD1234567890", master_bills_of_lading: "MBOL9999\n MBOL1234567890", import_country: us, one_usg_date: Date.new(2018, 11, 6))
  ...
  let (:broker_invoice_1) {
    i = create(:broker_invoice, entry: entry, customer_number: "PVH", invoice_total: BigDecimal("200"), invoice_date: Date.new(2018, 11, 7), bill_to_name: "PVH CORP")
    l = create(:broker_invoice_line, broker_invoice: i, charge_amount: BigDecimal("200"), charge_code: "0100", charge_description: "CHARGE")
    i
  }

  let (:broker_invoice_2) {
    create(:broker_invoice, entry: entry, customer_number: "PVH", bill_to_name: "PVH CORP")
  }
  ...
  describe 'generate_and_send' do
    ...
    it "sends for Canada if One USG is not present" do
      entry.update! one_usg_date: nil, import_country: create(:country, iso_code: "CA")
      expect(subject).to receive(:generate_and_send_invoice_files).exactly(2).times
  ...
  describe 'generate_and_send_reversal' do
    ...
    let (:new_invoice) {
      i = create(:broker_invoice, entry: entry, invoice_total: BigDecimal("-200"), invoice_date: Date.new(2018, 11, 7), currency: "USD")
      l = create(:broker_invoice_line, broker_invoice: i, charge_amount: BigDecimal("-200"), charge_code: "0100", charge_description: "CHARGE")
      i
    }

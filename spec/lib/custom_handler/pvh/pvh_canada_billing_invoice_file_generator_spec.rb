describe OpenChain::CustomHandler::Pvh::PvhCanadaBillingInvoiceFileGenerator do
  ...
  let (:shipment) {
    ...
    l = create(:shipment_line, shipment: s, container: c, quantity: 10, product: product, linked_order_line_id: order.order_lines.first.id, gross_kgs: 100, invoice_number: "INVOICE")
    l2 = create(:shipment_line, shipment: s, container: c, quantity: 20, product: product, linked_order_line_id: order.order_lines.second.id, gross_kgs: 200, invoice_number: "INVOICE")

    l.shipment.reload
  }

  let (:entry) {
    e = create(:entry, entry_number: "1198001123", broker_reference: "12345", importer_id: pvh.id, customer_number: "PVH", container_numbers: "ABCD1234567890", master_bills_of_lading: "MBOL9999\n MBOL1234567890", transport_mode_code: 9)
  ...
  describe "generate_and_send_duty_charges" do
    ...
    it "utilizes the first shipment line found if unit cost / quantity is same for all lines" do
      ...
      expect(captured_xml.length).to eq 1
      expect(REXML::Document.new(captured_xml.first).root).to have_xpath_value("GenericInvoices/GenericInvoice/InvoiceDetails/InvoiceLineItem[ChargeField/Type/Code = 'C530']/ItemNumber", "001")
    end
    ...
    it "handles multiple invoice lines mapped to the same PO line for split tariffs" do
      ...
      l = REXML::XPath.first(x, "GenericInvoices/GenericInvoice/InvoiceDetails/InvoiceLineItem[ChargeField/Type/Code = 'C530']")
      expect(l).not_to be_nil
  ...
  describe "generate_and_send_container_charges" do
    ...
    it "makes all unmapped charge codes use the miscellaneous GTN charge code" do
      ...
      expect(lines[0]).not_to be_nil
      expect(lines[0]).to have_xpath_value("Value", "300.0")
    end

    it "uses house bill for Air modes" do
    ...
    it "uses master bill for Truck modes" do

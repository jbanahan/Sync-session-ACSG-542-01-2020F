describe OpenChain::CustomHandler::Pvh::PvhGtnOrderXmlParser do
  ...
  let (:product) {
    p = create(:product, importer: pvh, unique_identifier: "PVH-6403", name: "MEN'S KNIT T SHIRT")
  ...
  let (:order) {
    ol = create(:order_line, product: product, line_number: 100, order: create(:order, importer: pvh, vendor: vendor, factory: factory, order_number: "PVH-RTCO69258"))
    ol.order
  }

  let (:factory) {
    f = create(:company, name: "GUPTA EXIM (INDIA) PVT. LTD.", factory: true, system_code: "PVH-create-21410002", mid: "INGUPEXI103FAR")
    create(:address, company: f, system_code: f.system_code, name: "GUPTA EXIM (INDIA) PVT. LTD.", country: india, line_1: "(PLANT II)|103 DLF INDUSTRIAL AREA PHASE1", city: "FARIDABAD")
    f
  }

  let (:vendor) {
    v = create(:company, vendor: true, name: "GUPTA EXIM(INDIA) PVT LTD", system_code: "PVH-Vendor-21410")
    create(:address, company: v, system_code: v.system_code, name: "GUPTA EXIM(INDIA) PVT LTD", country: india, line_1: "144 DLF INDUSTRIAL AREA", line_2: "|PHASE 1,FARIDABAD-121 003", city: "HARYANA", state: "07", postal_code: "122505")
    v
  }

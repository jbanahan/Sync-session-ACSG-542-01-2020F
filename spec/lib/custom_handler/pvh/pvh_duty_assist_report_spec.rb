...
describe OpenChain::CustomHandler::Pvh::PvhDutyAssistReport do
  ...
  let!(:canada) { create(:importer, name: 'PVH Canada', system_code: "PVHCANADA") }
  let!(:us) { create(:importer, name: 'PVH', system_code: 'PVH') }

  def setup_data(customer_number, company)
    entry1 = create(:entry, entry_number: 'entry1', master_bills_of_lading: "1234\n",
    ...
    entry2 = create(:entry, entry_number: 'entry2', master_bills_of_lading: "2345\n",
    ...
    create(:broker_invoice, entry: entry1)
    create(:broker_invoice, entry: entry2)
    ci1 = create(:commercial_invoice, entry: entry1, invoice_number: "invoice1", exchange_rate: "10", currency: "USD")
    ci2 = create(:commercial_invoice, entry: entry2, invoice_number: "invoice2", exchange_rate: "10", currency: "CA")
    prod1 = create(:product, unique_identifier: "#{customer_number}-abcd")
    prod2 = create(:product, unique_identifier: "#{customer_number}-efgh")
    shipment1 = create(:shipment, importer: company, master_bill_of_lading: entry1.master_bills_of_lading.gsub(/\n/, ''), mode: "Ocean")
    shipment2 = create(:shipment, importer: company, master_bill_of_lading: entry2.master_bills_of_lading.gsub(/\n/, ''), mode: "Ocean")
    sl1 = create(:shipment_line, shipment: shipment1, product: prod1)
    sl2 = create(:shipment_line, shipment: shipment2, product: prod2)
    container1 = create(:container, entry: entry1, shipment_lines: [sl1], container_number: "Container1")
    container2 = create(:container, entry: entry2, shipment_lines: [sl2], container_number: "Container2")
    shipment1.containers << container1
    shipment1.save!
    shipment2.containers << container2
    shipment2.save!
    cil1 = create(:commercial_invoice_line, commercial_invoice: ci1, part_number: 'abcd', country_origin_code: 'Line1CO',
    ...
    cil2 = create(:commercial_invoice_line, commercial_invoice: ci2, part_number: 'efgh', country_origin_code: 'Line2CO',
    ...
    create(:commercial_invoice_tariff, commercial_invoice_line: cil1, tariff_description: "Tariff One",
                                        entered_value: "1.00", duty_rate: "3", hts_code: "123.12.123")
    create(:commercial_invoice_tariff, commercial_invoice_line: cil2, tariff_description: "Tariff Two",
    ...
    o1 = create(:order, importer: company, order_number: "#{customer_number}-PO1")
    o2 = create(:order, importer: company, order_number: "#{customer_number}-PO2")
    ol1 = create(:order_line, order: o1, product: prod1, line_number: 1, price_per_unit: 100)
    ol2 = create(:order_line, order: o2, product: prod2, line_number: 2, price_per_unit: 200)
  ...
  describe "permission?" do
    ...
    it "allows www users who can view entries and belong to the pvh_duty_discount_report group" do
      ...
      create(:group, system_code: "pvh_duty_discount_report").users << user
      expect(described_class.permission?(user)).to eq true
    end
  ...
  describe "run_report" do
    ...
    before do
      create(:fiscal_month, company: canada, start_date: Date.new(2020, 3, 2), end_date: Date.new(2020, 4, 5), year: 2020, month_number: 2)
      create(:fiscal_month, company: canada, start_date: Date.new(2020, 2, 3), end_date: Date.new(2020, 3, 1), year: 2020, month_number: 1)
      create(:fiscal_month, company: us, start_date: Date.new(2020, 3, 2), end_date: Date.new(2020, 4, 5), year: 2020, month_number: 2)
      create(:fiscal_month, company: us, start_date: Date.new(2020, 2, 3), end_date: Date.new(2020, 3, 1), year: 2020, month_number: 1)
    end

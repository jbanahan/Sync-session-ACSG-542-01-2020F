...
describe OpenChain::CustomHandler::Generic::LaceySimplifiedOrderXmlParser do
  describe '#parse' do
    it 'should delegate to #parse_dom' do
      ...
      expect(dc).to receive(:parse_dom).with(dom)
  ...
  describe '#parse_dom' do
    ...
    before :each do
      @importer = Factory(:company,system_code:'CLIENTCODE',importer:true)
      @china = Factory(:country,iso_code:'CN')
      @us = Factory(:country,iso_code:'US')
    end
    it "should map order data" do
      expect{described_class.new.parse_dom(base_dom)}.to change(Order,:count).from(0).to(1)
      o = Order.first
      ...
      sf = o.ship_from
      expect(sf.name).to eq 'ILS Factory #1'
    ...
    it "should update existing order" do
      ord = Factory(:order,order_number:'ABC12345',importer:@importer)
      expect{described_class.new.parse_dom(base_dom)}.to_not change(Order,:count)
      ord.reload
    ...
    it "should use existing ship from based on hash" do
      ven = Factory(:company,vendor:true,system_code:'ILSI')
      ...
      expect{described_class.new.parse_dom(base_dom)}.to_not change(ven.addresses,:count)
      expect(Order.first.ship_from).to eq sf
    end
    it "should use existing product" do
      p = Factory(:product,unique_identifier:'BRCHERFL')
      described_class.new.parse_dom(base_dom)
      ord = Order.first
    ...
    it "should use existing ship to based on hash" do
      ...
      expect{described_class.new.parse_dom(base_dom)}.to_not change(@importer.addresses,:count)
      expect(Order.first.order_lines.first.ship_to).to eq st
    end
    it "should fail if root element is wrong" do
      base_xml.gsub!('Order>','Other>')
      expect{described_class.new.parse_dom(base_dom)}.to raise_error(/invalid root element/)
      expect(Order.count).to eq 0
    end
    it "should fail if Importer isn't found" do
      base_xml.gsub!('CLIENTCODE','OTHERCODE')
      expect{described_class.new.parse_dom(base_dom)}.to raise_error(/Importer was not found/)
      expect(Order.count).to eq 0
    end
    ...
    it "should delete other lines" do
      ord = Factory(:order,order_number:'ABC12345',importer:@importer)
      ol = Factory(:order_line,order:ord,line_number:999)
      expect{described_class.new.parse_dom(base_dom)}.to change(OrderLine,:count).from(1).to(2)
    ...
    it "should fail if summary total lines doesn't match" do
      ...
      expect{described_class.new.parse_dom(base_dom)}.to raise_error(/TotalLines/)
    end
    it "should fail if summary total quantity doesn't match" do
      base_xml.gsub!('Quantity>708.9','Quantity>709.9')
      expect{described_class.new.parse_dom(base_dom)}.to raise_error(/Quantity/)
    end
    it "should not reprocess old files" do
      ord = Factory(:order,order_number:'ABC12345',importer:@importer,customer_order_number:'OTHER',last_exported_from_source:100.years.from_now)
      expect{described_class.new.parse_dom(base_dom)}.to_not change(OrderLine,:count)
      ord.reload
    ...
    context 'required fields' do
      it "should require ImporterId" do
        ...
        expect{described_class.new.parse_dom(base_dom)}.to raise_error(/ImporterId is required/)
        expect(Order.count).to eq 0
      end
      it "should require VendorNumber" do
        base_xml.gsub!('VendorNumber>ILSI','VendorNumber>')
        expect{described_class.new.parse_dom(base_dom)}.to raise_error(/VendorNumber is required/)
        expect(Order.count).to eq 0
      end
      it "should require OrderNumber" do
        base_xml.gsub!('OrderNumber>ABC12345','OrderNumber>')
        expect{described_class.new.parse_dom(base_dom)}.to raise_error(/OrderNumber is required/)
        expect(Order.count).to eq 0
      end
      it "should require CustomerOrderNumber" do
        base_xml.gsub!('CustomerOrderNumber>ABC-12345','CustomerOrderNumber>')
        expect{described_class.new.parse_dom(base_dom)}.to raise_error(/CustomerOrderNumber is required/)
        expect(Order.count).to eq 0
      end
      it "should require OrderDate" do
        base_xml.gsub!('OrderDate>2015-07-31','OrderDate>')
        expect{described_class.new.parse_dom(base_dom)}.to raise_error(/OrderDate is required/)
        expect(Order.count).to eq 0
      end
      it "should require SystemExtractDate" do
        base_xml.gsub!('SystemExtractDate>2015-08-31T13:26:00-04:00','SystemExtractDate>')
        expect{described_class.new.parse_dom(base_dom)}.to raise_error(/SystemExtractDate is required/)
        expect(Order.count).to eq 0
      end
      context 'line' do
        it "should require LineNumber" do
          base_xml.gsub!('LineNumber>1','LineNumber>')
          expect{described_class.new.parse_dom(base_dom)}.to raise_error(/LineNumber is required/)
          expect(Order.count).to eq 0
        end
        it "should require OrderedQuantity" do
          base_xml.gsub!('OrderedQuantity>307.4','OrderedQuantity>')
          expect{described_class.new.parse_dom(base_dom)}.to raise_error(/OrderedQuantity is required/)
          expect(Order.count).to eq 0
        end
        it "should require PricePerUnit" do
          base_xml.gsub!('PricePerUnit>1.21','PricePerUnit>')
          expect{described_class.new.parse_dom(base_dom)}.to raise_error(/PricePerUnit is required/)
          expect(Order.count).to eq 0
        end
        it "should require UnitOfMeasure" do
          base_xml.gsub!('UnitOfMeasure>FT2','UnitOfMeasure>')
          expect{described_class.new.parse_dom(base_dom)}.to raise_error(/UnitOfMeasure is required/)
          expect(Order.count).to eq 0
        end
        it "should require UniqueIdentifier" do
          base_xml.gsub!('UniqueIdentifier>BRCHERFL','UniqueIdentifier>')
          expect{described_class.new.parse_dom(base_dom)}.to raise_error(/UniqueIdentifier is required/)
          expect(Order.count).to eq 0
        end
      end
    end

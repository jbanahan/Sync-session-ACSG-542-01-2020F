describe OpenChain::CustomHandler::Ascena::AscenaDutySavingsReport do
  ...
  describe "fiscal_month" do
    let (:fiscal_month) { create(:fiscal_month, company: ascena, year: 2017, month_number: 3) }
  ...
  describe "run_report" do
    let! (:fiscal_month) { create(:fiscal_month, company: ascena, year: 2017, month_number: 3, start_date: Date.new(2017, 3, 1), end_date: Date.new(2017, 4, 1)) }
    ...
    context "with actual data" do
      let! (:entry) do
        e = create(:entry, importer: ascena, import_country: create(:country), customer_number: "ASCE", source_system: "Alliance", fiscal_date: Date.new(2017, 3, 1), broker_reference: "REF", transport_mode_code: "40", fiscal_year: 2017, fiscal_month: 3, release_date: DateTime.new(2017, 3, 1, 5, 0))
      ...
      let! (:order) do
        order = create(:order, order_number: "ASCENA-JST-PO")
  ...
  describe "DutySavingsCalculator" do
    ...
    describe "calculate_spi" do
      let(:ot) { create(:official_tariff, common_rate_decimal: 0.3) }
      before do
  ...
  describe "FieldFiller" do
    ...
    describe "fill_missing_fields" do
      ...
      let!(:ot) { create(:official_tariff, country: us, hts_code: "123456789", general_rate: "2%")}
      let(:results) do
        row = described_class::Wrapper.new []
        row[:e_id] = 1
        row[:cil_id] = 2
    ...
    context "EntFieldHelper" do
      ...
      let(:ent) do
        e = create(:entry, entry_number: "123456789")
        e.unlading_port =  create(:port, name: "unl port", schedule_d_code: "1234")
        e.us_entry_port = create(:port, name: "ent port", schedule_d_code: "4321")
        e.save!
        e
      end
      let(:results) do
    ...
    context "InvFieldHelper" do
      ...
      let(:cil) do
        cil = create(:commercial_invoice_line, contract_amount: 5, quantity: 10, value: 1)
        create(:commercial_invoice_tariff, commercial_invoice_line: cil, duty_amount: 2)
        create(:commercial_invoice_tariff, commercial_invoice_line: cil, duty_amount: 6)
        cil
      end
      let(:results) do
        row = described_class::Wrapper.new []
    ...
    context "TariffFieldHelper" do
      ...
      let!(:ot_1) { create(:official_tariff, hts_code: "123456789", country: us )}
      let!(:ot_2) { create(:official_tariff, hts_code: "987654321", country: us )}
      let!(:ot_3) { create(:official_tariff, hts_code: "246810121", country: ca )}
      let(:results) do
    ...
    context "non-MF helpers" do
      ...
      context "OfficialTariff calculations" do
        let(:co) { create(:country)}
        let!(:ot) { create(:official_tariff, country: co, hts_code: "1111", general_rate: "2%")}
  ...
  describe "Query" do
    ...
    let!(:ann) { create(:importer, alliance_customer_number: "ATAYLOR") }
    let!(:us) { create(:country)}

    let!(:e_asce) { create(:entry, importer: ascena, import_country_id: us.id, customer_name: "Ascena", customer_number: "ASCE", source_system: "Alliance", broker_reference: "ascena broker ref", transport_mode_code: "40", fiscal_date: "2018-03-16", fiscal_month: 1, release_date: "2018-03-10", entry_number: "asce ent number", arrival_date: "2018-03-08", import_date: "2018-03-06", unlading_port_code:"1234", entry_port_code: "4321", fish_and_wildlife_transmitted_date: "2018-03-06")}
    let!(:ci_asce) { create(:commercial_invoice, entry: e_asce, invoice_number: "inv num asce") }
    let!(:cil_asce) { create(:commercial_invoice_line, commercial_invoice: ci_asce, contract_amount: 5, related_parties: true, customs_line_number: 1, part_number: "part asce", po_number: "po asce", entered_value_7501: 5, product_line: "brand asce", country_origin_code: "AM", country_export_code: "country export asce", unit_price: 1, quantity: 2, unit_of_measure: "uom asce", value: 1, non_dutiable_amount: 2) }
    let!(:cit_asce) { create(:commercial_invoice_tariff, commercial_invoice_line: cil_asce, hts_code: "hts code", duty_rate: 1.1, tariff_description: "tar descr asce", duty_amount: 2, entered_value_7501: 4, spi_primary: "spi asce", special_tariff:true) }
    let!(:vend_asce) { create(:vendor, name: "asce vend") }
    let!(:fact_asce) { create(:factory, name: "asce fact") }
    let!(:ord_asce) do
      order = create(:order, order_number: "ASCENA-brand asce-po asce", vendor: vend_asce, factory: fact_asce)
      order.update_custom_value! cdefs[:ord_type], "ord type asce"
      order
    end

    let!(:official_tariff) { create(:official_tariff, country: us, hts_code: "hts code", general_rate: "2%") }
    let!(:port_arrival) { create(:port, schedule_d_code: "1234", name: "arrival port")}
    let!(:port_entry) { create(:port, schedule_d_code: "4321", name: "entry port")}

    let!(:e_ann) { create(:entry, importer: ann, import_country_id: us.id, customer_name: "Ann", customer_number: "ATAYLOR", source_system: "Alliance", broker_reference: "ann broker ref", transport_mode_code: "40", fiscal_date: "2018-03-16", fiscal_month: 2, release_date: "2018-03-11", entry_number: "ann ent number", arrival_date: "2018-03-09", import_date: "2018-03-07", unlading_port_code: "1234", entry_port_code: "4321")}
    let!(:ci_ann) { create(:commercial_invoice, entry: e_ann, invoice_number: "inv num ann") }
    let!(:cil_ann) { create(:commercial_invoice_line, commercial_invoice: ci_ann, contract_amount: 6, related_parties: true, customs_line_number: 1, part_number: "part ann", po_number: "po ann", entered_value_7501: 6, product_line: "brand ann", country_origin_code: "AM", country_export_code: "country export ann", unit_price: 3, quantity: 4, unit_of_measure: "uom ann", value: 1, non_dutiable_amount: 4, miscellaneous_discount: 2, other_amount: -1) }
    let!(:cit_ann) { create(:commercial_invoice_tariff, commercial_invoice_line: cil_ann, hts_code: "hts code", duty_rate: 1.3, tariff_description: "tar descr ann", duty_amount: 3, entered_value_7501: 5, spi_primary: "spi ann", special_tariff: true) }
    let!(:vend_ann_810) { create(:vendor, name: "ann vend 810") }
    let!(:fact_ann_810) { create(:factory, name: "ann fact 810") }
    let!(:vend_ann_ord) { create(:vendor, name: "ann vend ord") }
    let!(:fact_ann_ord) { create(:factory, name: "ann fact ord") }
    let!(:i) { create(:invoice, importer: ann, invoice_number: "inv num ann", vendor: vend_ann_810, factory: fact_ann_810) }
    let!(:il) { create(:invoice_line, invoice: i, po_number: "po ann", part_number: "part ann", part_description: "part descr", air_sea_discount: 4, early_pay_discount: 2, trade_discount: 6, middleman_charge: 8)}
    let!(:ord_ann) do
      order = create(:order, order_number: "ATAYLOR-po ann", vendor: vend_ann_ord, factory: fact_ann_ord)

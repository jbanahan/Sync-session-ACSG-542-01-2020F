describe OpenChain::CustomHandler::Ascena::AscenaDutySavingsReport do
  ...
  describe "run_report" do
    ...
    let! (:entry) {
      ...
      line = inv.commercial_invoice_lines.create! po_number: "PO", part_number: "PART", product_line: "JST", non_dutiable_amount: 0, value: BigDecimal("10"), contract_amount: 0
      ...
      line = inv.commercial_invoice_lines.create! po_number: "PO", part_number: "PART2", product_line: "JST", non_dutiable_amount: 0, value: BigDecimal("10"), contract_amount: 0
      tariff = line.commercial_invoice_tariffs.create! hts_code: "1234567890", tariff_description: "DESC", entered_value: BigDecimal("10"), spi_primary: "", duty_rate: BigDecimal("0.1"), duty_amount: BigDecimal("1")
    ...
    it "skips anything that does not have duty savings" do
      ...
      expect(sheet.row(0)).to eq ["Broker Reference", "Transport Mode Code", "Fiscal Month", "Release Date", "Invoice Number", "PO Number", "Part Number", "Brand", "Non-Dutiable Amount", "First Sale Cost", "Invoice Line Value", "HTS Code", "Tariff Description", "Entered Value", "SPI", "Duty Rate", "Duty Amount", "Order Type", "Calculated Entered Value", "Calculated Duty", "Duty Savings"]
      expect(sheet.row(1)).to eq ["REF", "10", "2017-03", excel_date(Date.new(2017, 3, 1)), "INV", "PO", "PART", "JST", 0, 0, 10, "1234567890", "DESC", 10, nil, 0.1, 1, "AGS", 10, 1, 0]
      expect(sheet.row(2)).to eq ["REF", "10", "2017-03", excel_date(Date.new(2017, 3, 1)), "INV", "PO", "PART2", "JST", 0, 0, 10, "1234567890", "DESC", 10, nil, 0.1, 1, "AGS", 10, 1, 0]
    end
    ...
    it "records non-ags first sale data" do
      ...
      expect(sheet = (wb.worksheet "Data")).not_to be_nil
      expect(sheet.rows.count).to eq 3
      expect(sheet.row(1)).to eq ["REF", "10", "2017-03", excel_date(Date.new(2017, 3, 1)), "INV", "PO", "PART", "JST", 0, 0, 10, "1234567890", "DESC", 10, nil, 0.1, 1, "NONAGS", 10, 1, 0]
      expect(sheet.row(2)).to eq ["REF", "10", "2017-03", excel_date(Date.new(2017, 3, 1)), "INV", "PO", "PART2", "JST", 0, 0, 10, "1234567890", "DESC", 10, nil, 0.1, 1, "NONAGS", 10, 1, 0]
    end
    ...
    it "records savings that apply to more than one category" do
      OfficialTariff.create! country: Factory(:country, iso_code: "US"), hts_code: "1234567890", common_rate: "20%", common_rate_decimal: 0.2
      entry.update_attributes! transport_mode_code: "40"
      ...
      expect(sheet.row(1)).to eq ["Air Sea Differential", 1, 20, 2, 60, 6, 4, 66.67]
      expect(sheet.row(2)).to eq ["CAFTA", 1, 20, 2, 20, 4, 2, 50]
      expect(sheet.row(3)).to eq ["First Sale", 1, 20, 2, 40, 4, 2, 50]
    end

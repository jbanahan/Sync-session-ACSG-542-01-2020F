describe OpenChain::CustomHandler::Burlington::BurlingtonShipmentCiLoadGenerator do
  ...
  let (:burlington) { create(:importer, system_code: "BURLI")}
  ...
  let (:order) {
    order = create(:order, customer_order_number: "PO")
    order_line = create(:order_line, order: order, price_per_unit: BigDecimal("12.99"), product: product)

    order.reload
  }

  let (:shipment) {
    shipment = create(:shipment, importer: burlington, importer_reference: "ImpRef", master_bill_of_lading: "MASTERBILL", last_exported_from_source: "2017-02-01 00:00")
    shipment_line = create(:shipment_line, shipment: shipment, product: product, carton_qty: 10, gross_kgs: BigDecimal("100.50"), quantity: 99, linked_order_line_id: order.order_lines.first.id)
    shipment_line_2 = create(:shipment_line, shipment: shipment, product: product, carton_qty: 20, gross_kgs: BigDecimal("100.50"), quantity: 198, linked_order_line_id: order.order_lines.first.id)
  ...
  describe "generate_entry_data" do
    ...
    it "handles multiple shipments, making each one a new invoice" do
      shipment2 = create(:shipment, importer_reference: "Ref2", master_bill_of_lading: "MASTERBILL")
      shipment_line_2 = create(:shipment_line, shipment: shipment2, product: product, carton_qty: 1, gross_kgs: BigDecimal("10"), quantity: 10, linked_order_line_id: order.order_lines.first.id)
  ...
  describe "generate_and_send" do
    ...
    it "finds shipments already sent with same master bill and resends them" do
      shipment2 = create(:shipment, importer: shipment.importer, importer_reference: "ARef2", master_bill_of_lading: shipment.master_bill_of_lading)
      shipment_line_2 = create(:shipment_line, shipment: shipment2, product: product, carton_qty: 1, gross_kgs: BigDecimal("10"), quantity: 10, linked_order_line_id: order.order_lines.first.id)
  ...
  describe "find_generate_and_send" do
    ...
    it "groups multiple shipments together by master bill" do
      shipment2 = create(:shipment, importer: burlington, importer_reference: "ImpRef2", master_bill_of_lading: "MASTERBILL", last_exported_from_source: "2017-02-01")
    ...
    it "doesn't allow an error in one send to prevent a later send" do
      shipment2 = create(:shipment, importer: burlington, importer_reference: "ImpRef2", master_bill_of_lading: "MASTERBILL2", last_exported_from_source: "2017-01-01")

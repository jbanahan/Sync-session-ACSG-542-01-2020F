...
describe OpenChain::CustomHandler::Burlington::Burlington856Parser do
  ...
  describe "process_shipment" do
    ...
    context "standard_order" do
      ...
      it "errors on missing order" do
        ...
        expect { subject.process_shipment user, segments, "bucket", "file.edi" }.to raise_error "Burlington 856 references missing Order # '365947702'."
      end
      ...
      it "errors on missing importer" do
        ...
        expect { subject.process_shipment user, segments, "bucket", "file.edi" }.to raise_error "Unable to find Burlington importer account with system code of: 'BURLI'."
      end
  ...
  describe "parse" do
    ...
    it "parses edi file" do
      expect_any_instance_of(subject).to receive(:process_shipment) do |inst, user, segments, bucket, file|
        expect(user.username).to eq "integration"
        expect(bucket).to eq "bucket"
        expect(file).to eq "file.edi"
        expect(segments.length).to be > 0
      end

      subject.parse edi_data, bucket: "bucket", key: "file.edi"
    end

    it "emails errors" do
      err = StandardError.new "Error"

      expect_any_instance_of(subject).to receive(:process_shipment).and_raise err

      expect(subject).to receive(:send_error_email).with(instance_of(REX12::Transaction), err, "Burlington 856", "file.edi")
      expect(err).to receive(:log_me).with ["File: file.edi"]

...
describe OpenChain::CustomHandler::Hm::ValidationRuleHmInvoiceLineFieldFormat do
  let!(:ci) { Factory(:commercial_invoice, invoice_number: "12345") }
  
  context "with regex" do
    let(:rule) { described_class.new(rule_attributes_json:{model_field_uid: "cil_value", regex: "12.34"}.to_json) }
    let!(:ci_line_1) { Factory(:commercial_invoice_line, commercial_invoice: ci, value: 12.34, subheader_number: 1, customs_line_number: 2, part_number:'ABC123') }
    let!(:ci_line_2) { Factory(:commercial_invoice_line, commercial_invoice: ci, value: 12.34, subheader_number: 3, customs_line_number: 4, part_number:'321CBA') }
  ...
  context "with NOT regex" do
    ...
    let!(:ci_line_1) { Factory(:commercial_invoice_line, commercial_invoice: ci, value: 11.00, subheader_number: 1, customs_line_number: 2, part_number:'ABC123') }
    let!(:ci_line_2) { Factory(:commercial_invoice_line, commercial_invoice: ci, value: 12.48, subheader_number: 3, customs_line_number: 4, part_number:'321CBA') }
    ...
    it "fails if any line matches regex" do
      ...
      Factory(:commercial_invoice_line, value: 12.34, commercial_invoice: ci, subheader_number: 5, customs_line_number: 6, part_number:'CBA321')

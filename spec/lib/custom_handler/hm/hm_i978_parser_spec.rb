describe OpenChain::CustomHandler::Hm::HmI978Parser do
  ...
  let (:product_importer) { create(:importer, system_code: "HENNE") }
  let (:cn) { create(:country, iso_code: "CN") }
  let (:ca) { create(:country, iso_code: "CA") }
  let (:us) { create(:country, iso_code: "US") }
  let (:product) {
    # Delivery Item # 1
    p = create(:product, unique_identifier: "HENNE-0615141", importer_id: product_importer.id)
    p.update_hts_for_country country, "6115950000"
    p
  }
  let (:product_2) {
    # Delivery Item # 2 / 3
    p = create(:product, unique_identifier: "HENNE-0742769", importer_id: product_importer.id)
  ...
  describe "process_shipment_xml" do
    ...
    context "with canada import" do
      let! (:importer) { with_fenix_id(create(:importer), "887634400RM0001")}
      let! (:country) { ca }
      ...
      it "falls back to tracking HybrisOrderNumber if CustomerOrder is missing" do
        xml_data.gsub!("<CustomerOrder>217711434XXX</CustomerOrder>", "").gsub!("<CustomerItemNumber>000020</CustomerItemNumber>", "")
    ...
    context "with us import" do
      ...
      let! (:entry) {
        e = create(:entry, source_system: "Alliance", importer: importer, release_date: Time.zone.now, export_country_codes: "CN")
        inv = create(:commercial_invoice, entry: e, invoice_number: "123456")
        inv_line = create(:commercial_invoice_line, commercial_invoice: inv, country_origin_code: "CN", part_number: "0615141", mid: "MID1")
        inv_line_2 = create(:commercial_invoice_line, commercial_invoice: inv, country_origin_code: "CN", part_number: "0742769", mid: "MID2")
      ...
      context "with single invoice" do
        ...
        it "converts Myanmaar country code to Burma for US entries" do
          burma = create(:country, iso_code: "BU")
  ...
  describe "generate_missing_parts_spreadsheet" do
    ...
    let! (:entry) {
      e = create(:entry, source_system: "Alliance", importer: product_importer, release_date: Time.zone.now, export_country_codes: "CN", broker_reference: "REF")
      inv = create(:commercial_invoice, entry: e, invoice_number: "PO")
      e.reload
    }
    ...
    context "with ca data" do
      let (:country) { ca }
      let! (:importer) { create(:importer, fenix_customer_number: "887634400RM0001")}

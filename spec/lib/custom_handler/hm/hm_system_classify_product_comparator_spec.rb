describe OpenChain::CustomHandler::Hm::HmSystemClassifyProductComparator do
  let(:u) { create(:user, username: "integration") }
  let!(:company) { create(:company, system_code: "HENNE")}
  let!(:country_ca) { create(:country, iso_code: "CA")}
  let!(:country_us) { create(:country, iso_code: "US")}

  describe "accept?" do
    let!(:prod) { create(:product, importer: company) }
    let(:snap) { create(:entity_snapshot, recordable: prod) }
    ...
    it "returns false if the snapshot doesn't belong to a product" do
      snap.update_attributes(recordable: create(:entry))
      expect(described_class.accept?(snap)).to be false
    end
  end
  ...
  describe "update_product!" do
    ...
    it "sets CA tariff number, flag, customs description; saves entity snapshot" do
      ...
      tariff = create(:tariff_record, classification: create(:classification, country: country_ca, product: create(:product, importer: company)))
    ...
    it "does nothing if CA tariff number already exists" do
      tariff = create(:tariff_record, classification: create(:classification, country: country_ca), hts_1: "4444444444" )
      classi = tariff.classification
      prod = classi.product
      prod.assign_attributes(importer: company)
      prod.find_and_set_custom_value(flag_cdef, nil)
  ...
  describe "update_classi!" do
    let(:classi) { create(:classification, country: country_ca, product: create(:product, importer: company)) }

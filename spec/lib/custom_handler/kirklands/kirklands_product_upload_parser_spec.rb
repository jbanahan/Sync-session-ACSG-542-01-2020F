describe OpenChain::CustomHandler::Kirklands::KirklandsProductUploadParser do
  ...
  let! (:master_company) {create(:importer, name: "Kirklands", master: true, system_code: "KLANDS")}
  let (:user) { create(:user) }
  let (:countries) { [create(:country, iso_code: "US", import_location: true), create(:country, iso_code: "CN", import_location: true)] }
  ...
  describe "process" do
    ...
    let (:custom_defintions) {
      subject.class.prep_custom_definitions [ :prod_part_number, :prod_material, :prod_country_of_origin, :prod_additional_doc,

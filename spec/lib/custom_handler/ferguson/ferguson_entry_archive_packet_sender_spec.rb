describe OpenChain::CustomHandler::Ferguson::FergusonEntryArchivePacketSender do
  ...
  describe "process_entries" do
    it "sends archive packets" do
      ...
      Timecop.freeze(current) do
        ...
        ent_1.sync_records.create!(trading_partner: described_class::SYNC_TRADING_PARTNER, sent_at: current - 1.day, confirmed_at: current - 1.day)
        ...
        expect(OpenChain::S3).to receive(:download_to_tempfile)
          .with("the_bucket", "the_path", {original_filename: "1180119_567890_EntryPacket_20200324020508.pdf"})
        ...
        expect(OpenChain::S3).to receive(:download_to_tempfile)
          .with("the_bucket", "the_path", {original_filename: "1180119_567892_EntryPacket_20200324020508.pdf"})
  ...
  describe "ftp_credentials" do
    it "gets test creds" do
      ...
      expect(cred[:folder]).to eq "to_ecs/ferguson_docs_test"
    end

    it "gets production creds" do
      allow(stub_master_setup).to receive(:production?).and_return true
      cred = subject.ftp_credentials
      expect(cred[:folder]).to eq "to_ecs/ferguson_docs"
    end
  end

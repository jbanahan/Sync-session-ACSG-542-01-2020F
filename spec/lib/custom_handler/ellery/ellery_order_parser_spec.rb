describe OpenChain::CustomHandler::Ellery::ElleryOrderParser do
  ...
  let! (:us) { create(:country, iso_code: "US", iso_3_code: "USA") }
  let! (:china) { create(:country, iso_code: "CN", iso_3_code: "CHN" )}
  ...
  describe "parse_file" do
    ...
    it "updates an order" do
      existing = create(:order, importer: ellery, order_number: "ELLHOL-14945")
      existing_line = create(:order_line, order: existing)
    ...
    it "does not delete unreferenced order lines that have been booked" do
      existing = create(:order, importer: ellery, order_number: "ELLHOL-14945")
      existing_line = create(:order_line, order: existing)
      booking_line = create(:booking_line, product: existing_line.product, order_line: existing_line, order: existing)
    ...
    it "does not delete unreferenced order lines that have been shipped" do
      existing = create(:order, importer: ellery, order_number: "ELLHOL-14945")
      existing_line = create(:order_line, order: existing)
      shipment_line = create(:shipment_line, product: existing_line.product)
    ...
    it "doesn't update existing product data if hts stays the same" do
      existing = create(:product, importer: ellery, unique_identifier: "ELLHOL-15828052084MAR")
    ...
    it "doesn't update vendor data" do
      vendor = create(:vendor, system_code: "ELLHOL-BE1220", name: "Existing Vendor")
      subject.parse_file po_csv, log, bucket: "bucket", key: "path/to/file.csv"
    ...
    it "doesn't update ship to data" do
      ship_to = create(:address, company: ellery, name: "FAYETTEVILLE DC")

describe OpenChain::CustomHandler::VfiTrackProductApiSyncClient do
  ...
  describe "sync" do
    ...
    let! (:product) { 
      ...
      tariff = Factory(:tariff_record, hts_1: "1234567890", classification: classification)
      product.reload
    }
    ...
    subject { 
      Class.new(OpenChain::CustomHandler::VfiTrackProductApiSyncClient) {
        ...
        def query
          "SELECT products.id, products.unique_identifier, products.name, coo.string_value, fda.string_value, country.iso_code, cd.string_value, t.line_number, t.hts_1 " +
    ...
    it "syncs product requiring update to a remote VFI Track instance not having that product data" do
      ...
      expect(api_client).to receive(:find_by_uid).with("SYSCODE-" + product.unique_identifier, ["prod_uid", "*cf_43", "prod_name", "*cf_41", "*cf_78", "*cf_77", "class_cntry_iso", "*cf_99", "hts_line_number", "hts_hts_1", "prod_imp_syscode"]).and_return({'product'=>nil})
    ...
    it "syncs data the already exists in vfitrack without a classification" do
      ...
      expect(api_client).to receive(:find_by_uid).with("SYSCODE-" + product.unique_identifier, ["prod_uid", "*cf_43", "prod_name", "*cf_41", "*cf_78", "*cf_77", "class_cntry_iso", "*cf_99", "hts_line_number", "hts_hts_1", "prod_imp_syscode"]).and_return({'product'=>existing_product})

  ...
  describe "can_view?" do
    ...
    context "Kewill CI Upload custom feature enabled" do
      ...
      it "allows master users to view" do
        expect(subject.can_view?(create(:master_user))).to be_truthy
      end

      it "disallows regular user" do
        expect(subject.can_view?(create(:user))).to be_falsey
      end
    end
    ...
    it "disallows access when Kewill CI Upload feature is not enabled" do
       ms = MasterSetup.new
       allow(MasterSetup).to receive(:get).and_return ms
       expect(subject.can_view?(create(:master_user))).to be_falsey
    end
  end

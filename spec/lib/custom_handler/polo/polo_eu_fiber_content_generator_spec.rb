describe OpenChain::CustomHandler::Polo::PoloEuFiberContentGenerator do
  ...
  describe "generate" do
    it "should email report" do
      u = create(:master_user, username:'EU Fiber Content', email:'a@sample.com')
      mail = double('mail')
    ...
    it "should create user" do
      master_company = create(:company, master:true)
      mail = double('mail')
      expect(mail).to receive(:deliver_now)
      d = described_class.new
      f = double('file')
  ...
  describe "sync" do
    ...
    it "should get new record" do
      p = create(:product, unique_identifier:'UID', name:'NM', updated_at:1.day.ago)
      ...
      c = create(:classification, country:@italy, product:p)
      t = create(:tariff_record, hts_1:'1234567890', classification:c)
    ...
    it "should get changed record" do
      p = create(:product, unique_identifier:'UID', name:'NM', updated_at:1.second.ago)
      p.update_custom_value!(@cdefs[:merch_division], 'MD')
      p.update_custom_value!(@cdefs[:fiber_content], 'FC')
      p.update_custom_value!(@cdefs[:csm_numbers], 'CSM')
      p.update_custom_value!(@cdefs[:season], 'SEA')
    ...
    it "should not get record without Italy HTS 1" do
      ...
      p.update_custom_value!(@cdefs[:csm_numbers], 'CSM')
      c = create(:classification, country:@italy, product:p)
      t = create(:tariff_record, hts_1:'', classification:c)
      expect(do_sync).to eq []
    end
  end

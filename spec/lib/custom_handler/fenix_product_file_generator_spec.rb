describe OpenChain::CustomHandler::FenixProductFileGenerator do
  ...
  describe "find_products" do
    ...
    let! (:prod_1) { create(:tariff_record, hts_1: '1234567890', classification: create(:classification, country_id: canada.id)).product }
    let! (:prod_2) { create(:tariff_record, hts_1: '1234567891', classification: create(:classification, country_id: canada.id)).product }
    ...
    it "excludes products that don't have canada classifications but need sync" do
      create(:tariff_record, hts_1: '1234567891', classification: create(:classification)).product
      expect(subject.find_products.to_a).to eq([prod_1, prod_2])
    end
  ...
  describe "make_file" do
    ...
    it "only uses the first hts line" do
      h = generator("XYZ")
      create(:tariff_record, hts_1: "12345", classification: clas)

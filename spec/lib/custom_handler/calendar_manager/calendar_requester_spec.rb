describe OpenChain::CustomHandler::CalendarManager::CalendarRequester do
  ...
  let (:us_hol) {Factory.create(:calendar, calendar_type: 'USHol', year: now.year)}
  let (:ca_hol) {Factory.create(:calendar, calendar_type: 'CAHol', year: 1.year.ago.year)}
  let (:now) { DateTime.now }

  describe "run_schedulable" do
    it "calls new_dates_needed with each calendar which does not have future events and are in the options" do
      Factory.create(:calendar_event, calendar: us_hol, event_date: now)
      Factory.create(:calendar_event, calendar: ca_hol, event_date: now)
  ...
  describe "new_dates_needed?" do
    ...
    it "returns false if a given calendar does have dates beyond one month" do
      Factory.create(:calendar_event, calendar: us_hol, event_date: now + 2.months)
      expect(subject.new_dates_needed?('USHol')).to eq false
    end

    it "returns true if there is no calendar for this year" do
      Factory.create(:calendar_event, calendar: ca_hol, event_date: 1.year.ago + 1.month)

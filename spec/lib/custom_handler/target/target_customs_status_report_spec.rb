describe OpenChain::CustomHandler::Target::TargetCustomsStatusReport do
  ...
  describe "run_customs_status_report" do
    ...
    it "generates and emails spreadsheet" do
      ...
      # The exceptions for this entry are all resolved.  It should be
      # included on the report still, however, because it doesn't have one USG date set.  Its exceptions should be
      # shown as if they had not been resolved.
      ...
      # This entry has no unresolved exceptions, and has a one USG date.  It should not be included in the report.
      Factory(:entry, customer_number: "TARGEN", broker_reference: "entry-4", source_system: "Alliance",
                      one_usg_date: Date.new(2020, 1, 1))

      # This entry has an unresolved exception.  Even though it has one USG date, it should still show up on the report.
      ...
      # This entry has no exceptions.  It should be included because it has no One USG Date value.
      Factory(:entry, customer_number: "TARGEN", broker_reference: "entry-7", source_system: "Alliance")
      ...
      expect(sheet[2]).to eq ["TGMI", "316", "entry-1", "A,B", "C,D", Date.new(2018, 12, 23), Date.new(2018, 12, 25),
                              Date.new(2018, 12, 27), "BOW", "X", 2, "XYZA", "BCDE", "SS Minnow", "E,F", "G,H"]
      expect(sheet[3]).to eq ["TGMI", "316", "entry-1", "A,B", "C,D", Date.new(2018, 12, 23), Date.new(2018, 12, 25),
      ...
      expect(sheet[5]).to eq ["TGMI", "316", "entry-2", "B", "D", Date.new(2019, 12, 23), Date.new(2019, 12, 25),
                              Date.new(2019, 12, 27), "TGT", "This is a Target exception.", 0, "AZYX", "EDCB", "Venture X-2", "F", "H"]
      expect(sheet[6]).to eq ["TGMI", "316", "entry-3", "C", "E", Date.new(2019, 12, 24), Date.new(2019, 12, 26),
                              Date.new(2019, 12, 28), "BOW", "This exception was resolved.", 0, "BZYX", "FDCB", "Venture X-3", "G", "I"]
      expect(sheet[7]).to eq ["TGMI", "316", "entry-3", "C", "E", Date.new(2019, 12, 24), Date.new(2019, 12, 26),
                               Date.new(2019, 12, 28), "WOW", "Safely resolved", 0, "BZYX", "FDCB", "Venture X-3", "G", "I"]
      expect(sheet[8]).to eq ["TGMI", "316", "entry-5", nil, nil, nil, nil, nil, "ARF", nil, 0, nil, nil, nil, nil, nil]
      expect(sheet[9]).to eq ["TGMI", "316", "entry-7", nil, nil, nil, nil, nil, nil, nil, 0, nil, nil, nil, nil, nil]
    end

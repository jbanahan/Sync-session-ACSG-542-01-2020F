  ...
  describe "process_from_attachment" do
    ...
    context "errors" do
      ...
      it "must be attached to a DrawbackClaim" do
        att = FactoryBot(:attachment, attachable:FactoryBot(:order), attached_file_name:'a.xlsx')
      ...
      it "must be a user who can edit the claim" do
        att = FactoryBot(:attachment, attachable:FactoryBot(:drawback_claim), attached_file_name:'a.xlsx')
    ...
    it "should call parse_excel" do
      allow_any_instance_of(DrawbackClaim).to receive(:can_edit?).and_return true
      dc = FactoryBot(:drawback_claim, entry_number:'12345678901')
      att = FactoryBot(:attachment, attachable:dc, attached_file_name:'a.xlsx')
      x = double(:xl_client)
    ...
    it "should call parse_csv_from_attachment" do
      allow_any_instance_of(DrawbackClaim).to receive(:can_edit?).and_return true
      dc = FactoryBot(:drawback_claim, entry_number:'12345678901')
      att = FactoryBot(:attachment, attachable:dc, attached_file_name:'a.csv')
      p = double(:claim_audit_parser)
      expect(described_class).to receive(:new).and_return(p)
    ...
    it "should fail if not csv or xlsx" do
      allow_any_instance_of(DrawbackClaim).to receive(:can_edit?).and_return true
      dc = FactoryBot(:drawback_claim, entry_number:'12345678901')
      att = FactoryBot(:attachment, attachable:dc, attached_file_name:'a.txt')
      expect {described_class.process_from_attachment(att.id, @u.id)}.to change(@u.messages, :count).from(0).to(1)
...
DTA
      @dc = FactoryBot(:drawback_claim, entry_number:'1234567890')
    end

  ...
  describe "process_from_attachment" do
    ...
    context "errors" do
      ...
      it "must be attached to a DrawbackClaim" do
        att = create(:attachment, attachable:create(:order), attached_file_name:'hello.xlsx')
      ...
      it "must be a user who can edit the claim" do
        att = create(:attachment, attachable:create(:drawback_claim), attached_file_name:'hello.xlsx')
    ...
    it "should call parse_excel" do
      ...
      att = create(:attachment, attachable:claim, attached_file_name:'something.xlsx')
      x = double(:xl_client)
    ...
    it "should call parse CSV" do
      allow_any_instance_of(DrawbackClaim).to receive(:can_edit?).and_return true
      claim = create(:drawback_claim)
      att = create(:attachment, attachable:claim, attached_file_name:'something.csv')
      p = double(:export_history_parser)
      expect(described_class).to receive(:new).and_return(p)
    ...
    it "should fail if not xlsx or csv" do
      allow_any_instance_of(DrawbackClaim).to receive(:can_edit?).and_return true
      claim = create(:drawback_claim)
      att = create(:attachment, attachable:claim, attached_file_name:'something.txt')
      expect {described_class.process_from_attachment(att.id, @u.id)}.to change(@u.messages, :count).from(0).to(1)
...
DTA
      @dc = create(:drawback_claim, entry_number:'316-7052301-3')
    end
    ...
    it "should raise error if claim entry number doesn't match passed in claim" do
      bad_dc = create(:drawback_claim, entry_number:'31670523XXX')

describe OpenChain::CustomHandler::Lenox::LenoxProductParser do
  ...
  before :each do
    @imp = create(:importer, system_code:'LENOX')
  ...
  context "hashing" do
    it "should process line the first time" do
      expect {
        described_class.new.process @row, @u, log
      }.to change(Product, :count).by 1
    end
    it "should not process line the second time" do
      described_class.new.process @row, @u, InboundFile.new
      p = Product.first
      t = 1.year.ago
  ...
  context "product" do
    ...
    it "should create product" do
      ...
      expect(log.company).to eq @imp
      expect(log.get_identifiers(InboundFileIdentifier::TYPE_ARTICLE_NUMBER)[0].value).to eq "00200GB"
    ...
    describe "parse_file" do
      it "parses a product file" do
        ...
        described_class.parse_file @row, log

describe OpenChain::CustomHandler::Lenox::LenoxPoParser do
  ...
  let!(:lenox) { create(:company, system_code: 'LENOX') }
  ...
  it "moves existing product to correct importer" do
    p = create(:product, unique_identifier: 'LENOX-6083927')
    described_class.new.process testdata, log
  ...
  it "updates existing PO, updating but not deleting lines" do
    ord = create(:order, order_number: 'LENOX-RB057225', importer_id: lenox.id)
    create(:order_line, order: ord, line_number: 1) # update this one
    o_line2 = create(:order_line, order: ord, line_number: 100) # leave this one alone
    described_class.new.process testdata, log
    expect(Order.count).to eq 1
    o = Order.first
    expect(o.order_number).to eq 'LENOX-RB057225'
  ...
  it "deletes lines with D as first character" do
    ord = create(:order, order_number: 'LENOX-RB057225', importer_id: lenox.id)
    create(:order_line, order: ord, line_number: 1) # delete this one
    create(:order_line, order: ord, line_number: 100) # leave this one alone
    testdata[0] = 'D'
    described_class.new.process testdata, log
    ord.reload
  ...
  it "doesn't delete shipped lines" do
    ...
    prod = create(:product)
    o_line = create(:order_line, order: ord, line_number: 1, product: prod)
    s_line = create(:shipment_line, product: prod)
  ...
  it "updates earliest ship date on product if earlier than existing date" do
    p = create(:product, unique_identifier: 'LENOX-6083927', importer: lenox)

describe OpenChain::CustomHandler::LandsEnd::LeProductParser do
  ...
  describe "process_file" do
    ...
    it "parses products" do
      ...
      expect(p1.custom_value(cdefs[:prod_importer_style])).to eq "2196"
      ...
      expect(p2.unique_identifier).to eq "LANDS1-3388954"
      expect(p2.custom_value(cdefs[:prod_importer_style])).to eq "51356"
    ...
    it "sets the classification's set type if column I contains XVV" do
      row_1 = ["2196", "PERSONALIZED ORNAMENT", "4996236", "1706", "WHITE SWAN FINE PEWTER", "", "US", "", "XVV", "", "9505.10.2500"]
      row_2 = ["51357", "WM SS PLEATED SOFT BLOUSE", "3388954", "2594", "VENTURA ENTERPRISE CO INC", "", "LK", "", "ONE COLOR", "", "6206.40.3031"]

      expect(subject).to receive(:foreach).with(cf, skip_headers: true).and_yield(row_1).and_yield(row_2)
      subject.process_file cf, user
      p1, p2 = Product.all
      ...
      expect(cl.custom_value(cdefs[:class_set_type])).to eq "XVV"

      cl2 = p2.classifications.first
      expect(cl2.country).to eq us

      expect(cl2.custom_value(cdefs[:class_set_type])).to_not eq "ONE COLOR"
    end
    ...
    it "updates products" do
      ...
      row_2 = ["2197", "PERSONALIZED ORNAMENT", "4996236", "1706", "WHITE SWAN FINE PEWTER", "", "US", "", "", "", "9505.10.2500"]

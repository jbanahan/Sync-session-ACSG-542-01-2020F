...
23105002005,3901,10/10/2009,N,10/11/2009,6110202079,2769247 - PULLOVER,1,EA,5.01,16.50%"
  ...
  it "should not update line already put in a download file" do
    @p.parse @data
    d = create(:duty_calc_import_file)
  ...
  it "should not update line for different company" do
    p = create(:product, unique_identifier:'LANDSEND-2740747')
    d = DrawbackImportLine.create(importer_id:create(:company).id, product_id:p.id, part_number:'2740747', entry_number:'23105002004', quantity:10)
    @p.parse @data

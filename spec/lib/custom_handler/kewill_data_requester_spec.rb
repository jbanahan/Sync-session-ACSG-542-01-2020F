describe OpenChain::CustomHandler::KewillDataRequester do
  ...
  describe "request_updated_since_last_run" do
    it "requests updated entry data via sql proxy client" do
      ...
      start_date = nil
      end_date = nil
      expect(sql_proxy_client).to receive(:request_updated_entry_numbers) do |start_d, end_d|
        start_date = start_d
        end_date = end_d
        true
      end
    ...
    it "uses existing key value to determine when last request was done" do
      ...
      sql_proxy_client = double("KewillSqlProxyClient")
      expect(sql_proxy_client).to receive(:request_updated_entry_numbers).with(ActiveSupport::TimeZone["Eastern Time (US & Canada)"].parse(original_request), instance_of(ActiveSupport::TimeWithZone), nil)
      described_class.request_updated_since_last_run({'sql_proxy_client' => sql_proxy_client})
    ...
    it "passes through customer_numbers opt if non-blank" do
      sql_proxy_client = double("KewillSqlProxyClient")
      expect(sql_proxy_client).to receive(:request_updated_entry_numbers).with(instance_of(ActiveSupport::TimeWithZone), instance_of(ActiveSupport::TimeWithZone), "TESTING")
    ...
    it "offsets request time by X seconds if requested" do
      ...
      allow(ActiveSupport::TimeZone["Eastern Time (US & Canada)"]).to receive(:now).and_return now

      sql_proxy_client = double("KewillSqlProxyClient")
      expect(sql_proxy_client).to receive(:request_updated_entry_numbers).with(original_request - 2.minutes, now - 2.minutes, nil)
  ...
  describe "request_since_hours_ago" do
    it "requests data using given hours ago value" do
      ...
      sql_proxy_client = double("KewillSqlProxyClient")
      expect(sql_proxy_client).to receive(:request_updated_entry_numbers).with(now - 1.hour, now, nil)
    ...
    it "requests data using given hours ago value, passing customer_number" do
      ...
      expect(Time.zone).to receive(:now).and_return now
      sql_proxy_client = double("KewillSqlProxyClient")
      expect(sql_proxy_client).to receive(:request_updated_entry_numbers).with(now - 1.hour, now, "TEST, TESTING")
  ...
  describe "run_schedulable" do
    it "defaults to updated since last run call" do
      expect(described_class).to receive(:request_updated_since_last_run)

      described_class.run_schedulable
    end

    it "uses request update after hours ago if hours ago value is present" do
      expect(described_class).to receive(:request_update_after_hours_ago).with(10, {'hours_ago' => 10})
  ...
  describe "request_entry_batch_data" do
    it "requests entry data using old style data" do
      ...
      expect(described_class).to receive(:request_entry_data).with("1", "123", nil)
    ...
    it "requests entry data using new data format" do
      allow(described_class).to receive(:delay).and_return described_class
      expect(described_class).to receive(:request_entry_data).with("1", "123", 2)
    ...
    it "requests entry data for each value in the given hash" do
      allow(described_class).to receive(:delay).and_return described_class
      expect(described_class).to receive(:request_entry_data).with("1", "123", nil)
      expect(described_class).to receive(:request_entry_data).with("2", "234", 1)
...
"
      ...
      expect(described_class).not_to receive(:request_entry_data)

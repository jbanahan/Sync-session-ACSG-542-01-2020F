describe OpenChain::CustomHandler::UnderArmour::UnderArmourDrawbackProcessor do
  before :each do
    ...
    @product = create(:product, unique_identifier:'1234567')
  end
  describe "process_entries" do
    before :each do
      @color = '888'
      @importer = create(:company, :importer=>true)
      @c_line = create(:commercial_invoice_line, :quantity=>10, :part_number=>"#{@product.unique_identifier}-#{@color}", :po_number=>'12345')
  ...
  describe "link_commercial_invoice_line" do
    ...
    it 'should match one entry to two shipment lines by po / style' do
      ...
      @s_line2 = create(:shipment_line, :quantity=>20, :product=>@product)
    ...
    it 'should not match to a shipment that is already on another piece set matched to a ci_line' do
      @c_line.update_attributes(:quantity=>30)
      @c_line_used = create(:commercial_invoice_line, :quantity=>10, :part_number=>"#{@product.unique_identifier}-#{@color}", :po_number=>'12345')
      @c_line_used.commercial_invoice.entry.update_attributes(:arrival_date=>0.days.ago)
      @s_line_used = create(:shipment_line, :quantity=>20, :product=>@product)
    ...
    it "should allocate shipment remainders to another invoice line if left over" do
      @c_line.update_attributes(:quantity=>8)
      @c_line_2 = create(:commercial_invoice_line, :quantity=>8, :part_number=>"#{@product.unique_identifier}-#{@color}", :po_number=>'12345')
    ...
    it "should consider previous matches to determine how much invoice quantity is available to match" do
      @c_line.update_attributes(:quantity=>8)
      @s_line_used = create(:shipment_line, :quantity=>6, :product=>@product)
  ...
  describe "make_drawback_import_lines" do
    before :each do
      @color = '777'
      @importer = create(:company, :importer=>true)
      @c_line = create(:commercial_invoice_line, :quantity=>10, :part_number=>"#{@product.unique_identifier}-#{@color}", :po_number=>'12345', :country_origin_code=>'CN')
    ...
    it "should make multiple links for multiple shipment lines" do
      ...
      s_line2 = create(:shipment_line, :quantity=>2, :product=>@product)
      s2 = s_line2.shipment

describe OpenChain::CustomHandler::UnderArmour::UnderArmourPoXmlParser do
  ...
  let! (:ua_parts) { create(:importer, system_code: "UAPARTS")}
  ...
  describe "process_order" do
    ...
    it "handles updating orders" do
      order = create(:order, importer: ua, order_number: "UNDAR-4200001923")
      line = create(:order_line, order: order, line_number: 10, sku: nil)
    ...
    context "with line missing from XML" do
      let!(:order) { create(:order, importer: ua, order_number: "UNDAR-4200001923") }
      let!(:line) { create(:order_line, order: order, line_number: 30) }
      ...
      it "ignores order line if shipped" do
        ...
        PieceSet.create quantity: 1, order_line: line, shipment_line: create(:shipment_line, product: prod)
        subject.process_order xml, user, "bucket", "file.xml", log

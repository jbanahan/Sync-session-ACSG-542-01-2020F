describe OpenChain::CustomHandler::UnderArmour::UnderArmour315Generator do
  ...
  describe "receive" do
    let (:entry) { create(:entry, cadex_sent_date: Time.zone.now, release_date: Time.zone.now + 1.day, first_do_issued_date: Time.zone.now + 2.days) }
    let (:invoice) { create(:commercial_invoice, entry: entry) }
    let! (:inv_line_1) { create(:commercial_invoice_line, customer_reference: "ABC", commercial_invoice: invoice) }
    let! (:inv_line_2) { create(:commercial_invoice_line, customer_reference: "ABC", commercial_invoice: invoice) }
    let! (:inv_line_3) { create(:commercial_invoice_line, customer_reference: "DEF", commercial_invoice: invoice) }
  ...
  describe "generate_and_send" do
    ...
    it "pulls IBD number from shipment data" do
      create(:shipment, reference: "UNDAR-#{entry_data[:shipment_identifier]}", booking_number: "IBDNUMBER")

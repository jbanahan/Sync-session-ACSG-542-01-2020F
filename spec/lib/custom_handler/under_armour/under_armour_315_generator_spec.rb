...
describe OpenChain::CustomHandler::UnderArmour::UnderArmour315Generator do
  ...
  describe "accepts?" do
    ...
    it "should accept a UA entry with invoice lines" do
      ...
      @g = expect(described_class.new.accepts?(nil, entry)).to be_truthy
    end

    it "should not accept a UA entry without invoice lines" do
      entry = Entry.new importer_tax_id: "874548506RM0001"
      @g = expect(described_class.new.accepts?(nil, entry)).to be_falsey
    end
  ...
  describe "receive" do
    before :each do 
      ...
      allow(@g).to receive(:delay).and_return @g
    end
    ...
    it "should trigger one xml file per shipment id / event code combination" do
      expect(@g).to receive(:generate_and_send).with(event_code: '2315', shipment_identifier: 'ABC', date: @entry.cadex_sent_date)
      expect(@g).to receive(:generate_and_send).with(event_code: '2326', shipment_identifier: 'ABC', date: @entry.release_date)
      expect(@g).to receive(:generate_and_send).with(event_code: '2902', shipment_identifier: 'ABC', date: @entry.first_do_issued_date)

      expect(@g).to receive(:generate_and_send).with(event_code: '2315', shipment_identifier: 'DEF', date: @entry.cadex_sent_date)
      expect(@g).to receive(:generate_and_send).with(event_code: '2326', shipment_identifier: 'DEF', date: @entry.release_date)
      expect(@g).to receive(:generate_and_send).with(event_code: '2902', shipment_identifier: 'DEF', date: @entry.first_do_issued_date)
    ...
    it "should not trigger xml for date values that have already been sent" do
      ...
      @g.receive nil, @entry
      expect(@g).not_to receive(:generate_and_send)
    end
    ...
    it "should trigger xml for updated date values" do
      ...
      expect(@g).to receive(:generate_and_send).with(event_code: '2315', shipment_identifier: 'ABC', date: @entry.cadex_sent_date + 1.day)
  ...
  describe "generate_and_send" do
    before :each do
      @g = described_class.new
      allow(@g).to receive(:ftp_file) do |file, opts|
    ...
    it "should generate xml and ftp it" do
      ...
      expect(REXML::XPath.first(@xml_data, "/tXML/Message/MANH_TPM_Shipment/@Id").value).to eq @entry_data[:shipment_identifier]
      expect(REXML::XPath.first(@xml_data, "/tXML/Message/MANH_TPM_Shipment/Shipment/@Id").value).to eq @entry_data[:shipment_identifier]
      expect(REXML::XPath.first(@xml_data, "/tXML/Message/MANH_TPM_Shipment/@DocSource").value).to eq "Vande"
      expect(REXML::XPath.first(@xml_data, "/tXML/Message/MANH_TPM_Shipment/Shipment/Event/EventLocation/@InternalId").value).to eq "VFICA"
      expect(REXML::XPath.first(@xml_data, "/tXML/Message/MANH_TPM_Shipment/Shipment/Event/@Id").value).to eq sha
      expect(REXML::XPath.first(@xml_data, "/tXML/Message/MANH_TPM_Shipment/Shipment/Event/@Code").value).to eq @entry_data[:event_code]
      expect(REXML::XPath.first(@xml_data, "/tXML/Message/MANH_TPM_Shipment/Shipment/Event/@DateTime").value).to eq @entry_data[:date].in_time_zone("GMT").iso8601[0..-7]
    end
  end
  ...
  describe "generate_file" do
    it "should generate data to a file and return the file" do
      ...
      expect(REXML::XPath.first(doc, "/tXML/Message/MANH_TPM_Shipment/@Id").value).to eq entry_data[:shipment_identifier]
    end
  end

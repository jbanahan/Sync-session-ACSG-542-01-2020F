describe OpenChain::CustomHandler::UnderArmour::UaArticleMasterParser do
  ...
  let!(:ca) { create(:country, name: "Canada", iso_code: "CA") }
  let!(:imp) { create(:importer, name: "Under Armour", system_code: "UAPARTS")}
  ...
  describe "parse" do
    ...
    it "makes no updates if data unchanged" do
      ...
      p = create(:product, importer: imp, unique_identifier: "UAPARTS-art num", name: "art descr")
      cl = create(:classification, product: p, country: ca)
      cl.update_custom_value!(cdefs[:class_customs_description], "customs descr")
      create(:tariff_record, classification: cl, hts_1: "1111111111")
      var = create(:variant, product: p, variant_identifier: "sku 1")
  ...
  context "instance methods" do
    ...
    describe "create_or_update_product!" do
      ...
      context "when article number isn't nil" do
        ...
        it "updates product if it exists and has been changed" do
          prod = create(:product, unique_identifier: "UAPARTS-art num", name: nil, importer: imp)
        ...
        it "doesn't update product if unchanged" do
          prod = create(:product, unique_identifier: "UAPARTS-art num", name: "art descr", importer: imp)
          subject.create_or_update_product! art_elem, change_flag
          prod.reload
          expect(prod.name).to eq "art descr"
          expect(Product.count).to eq 1
    ...
    describe "create_or_update_variants!" do
      ...
      context "prepacks" do
        let(:prepack_prod) { create(:product, unique_identifier: 'UAPARTS-1271424-437', importer: prod.importer )}
        let!(:var_1) do
          var = create(:variant, variant_identifier: '1271424-43700-SM', product: prepack_prod)
        ...
        let!(:var_2) do
          var = create(:variant, variant_identifier: '1271424-43700-LG', product: prepack_prod)
    ...
    describe "create_or_update_classi!" do
      let!(:classi) do
        c = create(:classification, product: prod, country: ca)
    ...
    describe "create_or_update_tariff!" do
      let!(:classi) { create(:classification, product: prod, country: ca)}
      ...
      it "removes tariff if given more than one HTS" do
        create(:tariff_record, classification: classi, hts_1: "1111111111")
        classi.reload
    ...
    describe "pluck_unique_hts_values" do
      ...
      it "removes duplicates" do
        v3 = create(:variant, product: prod)
  ...
  describe "error emailing" do
    let! (:user) {
      u = create(:user, email: "me@there.com")
      u.groups << group
      u
    }

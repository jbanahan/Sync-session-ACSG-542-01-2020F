...
describe OpenChain::CustomHandler::LumberLiquidators::LumberSapArticleXmlParser do
  describe "parse_dom" do
    ...
    it "should update article" do
      dom = REXML::Document.new(@test_data)
      op = FactoryBot(:product, unique_identifier:'000000000010000328')
      ord = FactoryBot(:order, order_number:'TEST_ORDER')
      ol = FactoryBot(:order_line, product: op, order: ord)
      c = op.classifications.create! country: FactoryBot(:country, iso_code: "US")
      c.tariff_records.create! line_number: 1, hts_1: "1234567890"

      op.create_snapshot(FactoryBot(:user))
    ...
    it "should not change the article number if it already exists" do
      dom = REXML::Document.new(@test_data)
      op = FactoryBot(:product, unique_identifier:'000000000010000328')
      ol = FactoryBot(:order_line, product: op)

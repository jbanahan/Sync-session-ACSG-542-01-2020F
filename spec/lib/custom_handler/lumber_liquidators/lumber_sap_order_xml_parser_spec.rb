describe OpenChain::CustomHandler::LumberLiquidators::LumberSapOrderXmlParser do
  ...
  describe "parse_dom" do
    ...
    before :each do
      @usa = create(:country, iso_code:'US')
      ...
      @importer = Company.where(master: true).first.presence || create(:master_company)
      @vendor = create(:company, vendor:true, system_code:'0000100131')
      ...
      @product1= create(:product, name: 'Widgets', unique_identifier:'000000000010001547')
    ...
    context 'assigned agent' do
      let :gelowell do
        gelowell = create(:company, system_code:'GELOWELL')
        gelowell.linked_companies << @vendor
        gelowell
      end
      let :ro do
        ro = create(:company, system_code:'RO')
    ...
    it "should not update order if previous extract time is newer than this doc" do
      existing_order = create(:order, order_number:'4700000325')
  ...
  describe "setup_folders" do
    ...
    it "skips creating folders that already exist" do
      create(:folder, base_object: ord, created_by: user, name: 'Quality')
      subject.setup_folders ord

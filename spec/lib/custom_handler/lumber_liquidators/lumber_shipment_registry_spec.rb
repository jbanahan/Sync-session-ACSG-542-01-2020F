describe OpenChain::CustomHandler::LumberLiquidators::LumberShipmentRegistry do
  ...
  describe "cancel_shipment_hook" do
    it "deletes booking lines on cancel" do
      u = double(:user)
      s = create(:shipment)
      bl = create(:booking_line, shipment:s)
  ...
  describe "can_cancel?" do
    ...
    context "can_cancel_as_agent?" do
      ...
      let!(:cdef) { create(:custom_definition, cdef_uid: "ord_assigned_agent", data_type: :string, module_type: "Order") }
      let!(:agent_2) { create(:company, agent: true, system_code: "Konvenientz") }
      let!(:o1) { o = create(:order, order_lines: [create(:order_line)]); o.update_custom_value! cdef, "ACME"; o }
      let!(:o2) { o = create(:order, order_lines: [create(:order_line)]); o.update_custom_value! cdef, "ACME"; o }
      let!(:o3) { o = create(:order, booking_lines: [create(:booking_line, order_line: create(:order_line))]); o.update_custom_value! cdef, "ACME"; o }
      let!(:o4) { o = create(:order, booking_lines: [create(:booking_line, order_line: create(:order_line))]); o.update_custom_value! cdef, "ACME"; o }

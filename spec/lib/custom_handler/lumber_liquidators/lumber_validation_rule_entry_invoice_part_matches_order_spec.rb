describe OpenChain::CustomHandler::LumberLiquidators::LumberValidationRuleEntryInvoicePartMatchesOrder do
  before :each do
    ...
    allow(subject).to receive(:api_client).and_return @api_client_double
  end

  it "passes if every invoice line has a PO with a product that matches the line's part number" do
    expect(@api_client_double).to receive(:find_by_order_number).with(@cil.po_number, [:ord_rule_state, :ordln_puid]).and_return @order_hsh

describe OpenChain::CustomHandler::LumberLiquidators::LumberProductVendorCarbStatementUploader do
  ...
  describe "process" do
    ...
    let! (:xref_code) { DataCrossReference.create! key: "CODE", value: "TEXT", cross_reference_type: "ll_carb_statement"}
    let! (:xref_code_2) { DataCrossReference.create! key: "CODE2", value: "TEXT2", cross_reference_type: "ll_carb_statement" }
    ...
    it "parses file data" do
      ...
      expect(email.subject).to eq "Vendor CARB Statement Upload Complete"
      ...
      expect(s.context).to eq "CARB Statement Upload"
  ...
  describe "can_view?" do
    ...
    let (:user) { create(:user) }
    let (:group) { Group.use_system_group "CARBASSIGN" }
    ...
    context "LL system" do
      ...
      it "allows users in CARBASSIGN group" do
        user.groups << group

describe OpenChain::CustomHandler::LumberLiquidators::LumberOrderCloser do
  ...
  describe '#open_closed_orders' do
    it 'should open closed orders' do
      o = create(:order, order_number:'12345', closed_at:Time.now)
      keep_closed = create(:order, closed_at:Time.now)
      expect(described_class.open_closed_orders ['12345'], create(:user)).to eq 1
  ...
  describe '#close_orders' do
    before :each do
      ...
      @keep_open = create(:order, order_number:'12345', order_date: @effective_date)
      @close = create(:order, order_date: @effective_date - 1)
    end

    it 'should close open orders not on list' do
      expect(described_class.close_orders ['12345'], @effective_date, create(:user)).to eq 1
    ...
    it 'skips unlisted orders on/after effective_date' do
      ...
      described_class.close_orders ['12345'], @effective_date, create(:user)

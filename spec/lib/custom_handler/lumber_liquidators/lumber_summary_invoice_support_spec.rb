...
describe OpenChain::CustomHandler::LumberLiquidators::LumberSummaryInvoiceSupport do
  ...
  let (:broker_invoice) { Factory(:broker_invoice, invoice_total: BigDecimal("100"), invoice_date: Date.new(2016, 4, 1)) }
  let (:broker_invoice_2) { Factory(:broker_invoice, invoice_total: BigDecimal("10"), invoice_date: Date.new(2016, 3, 1)) }
  ...
  describe "generate_summary" do
    ...
    it "creates a summary page with data from each invoice" do
      ...
      expect(sheet.row(5)).to eq [broker_invoice.invoice_number, broker_invoice.invoice_date, broker_invoice.invoice_total.to_f]
      expect(sheet.row(6)).to eq [broker_invoice_2.invoice_number, broker_invoice_2.invoice_date, broker_invoice_2.invoice_total.to_f]
    ...
    it "skips summary date line if no date is given" do
      ...
      expect(sheet.row(4)).to eq [broker_invoice.invoice_number, broker_invoice.invoice_date, broker_invoice.invoice_total.to_f]

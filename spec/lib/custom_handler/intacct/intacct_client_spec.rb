describe OpenChain::CustomHandler::Intacct::IntacctClient do
  ...
  subject { described_class.new xml_gen }
  ...
  describe "send_dimension" do
    ...
    it "logs error responses" do
      ...
      expect(subject).to receive(:post_xml).with("company", false, false, get_xml, control)
                                           .and_raise OpenChain::CustomHandler::Intacct::IntacctClient::IntacctClientError, "Intacct Error"

      expect(subject.send_dimension("type", "id", "value", "company")).to be_nil
      expect(ErrorLogEntry.last.additional_messages_json).to match(%r{Failed to find and/or create dimension type id for location company})
    end
    ...
    it "swallows errors returned by Intacct API related to duplicate create calls" do
      ...
      control = "controlid"
      expect(xml_gen).to receive(:generate_dimension_get).with("type", "id").and_return [control, get_xml]
      expect(subject).to receive(:post_xml).with(nil, false, false, get_xml, control)
                                           .and_raise(OpenChain::CustomHandler::Intacct::IntacctClient::IntacctClientError,
                                                      "A successful transaction has already been recorded with the control id #{control}")
    ...
    it "swallows errors returned by Intacct API related to duplicate Class create calls" do
      ...
      expect(subject).to receive(:post_xml).with(nil, false, false, get_xml, control)
                                           .and_raise(OpenChain::CustomHandler::Intacct::IntacctClient::IntacctClientError,
                                                      "Another Class with the given value(s) 12345 already exists.")
    ...
    it "swallows errors returned by Intacct API related to duplicate Project create calls" do
      ...
      expect(subject).to receive(:post_xml).with(nil, false, false, get_xml, control)
                                           .and_raise(OpenChain::CustomHandler::Intacct::IntacctClient::IntacctClientError,
                                                      "Another Project with the given value(s) 12345 already exists.")

      expect(subject.send_dimension("type", "id", "value")).to eq "id"
    end

    it "swallows errors returned by Intacct API related to concurrent request errors" do
      get_xml = "<xml>test</xml>"
      control = "controlid"
      expect(xml_gen).to receive(:generate_dimension_get).with("type", "id").and_return [control, get_xml]
      expect(subject).to receive(:post_xml).with(nil, false, false, get_xml, control)
                                           .and_raise(OpenChain::CustomHandler::Intacct::IntacctClient::IntacctClientError,
                                                      "Concurrent request in process")
  ...
  describe "send_receivable" do
    ...
    it "sends missing dimension" do
      ...
      cid = "controlid"
      xml = "<recv>receivable</recv>"
      expect(xml_gen).to receive(:generate_receivable_xml).with(r).twice.and_return [cid, xml]
      expect(subject).to receive(:post_xml).with("c", true, true, xml, cid)
                                           .and_raise(OpenChain::CustomHandler::Intacct::IntacctClient::IntacctInvalidDimensionError.new("Error", "Broker File", "12345"))
      expect(subject).to receive(:post_xml).with("c", true, true, xml, cid).and_return create_result_key_response(cid, "R-Key")
  ...
  describe "send_payable" do
    ...
    it "sends payable information and sets key and upload date back into payable" do
      ...
      expect(subject).to receive(:post_xml).with("c", true, true, xml + check_xml, [cid, check_cid])
                                           .and_return(create_multi_result_key_response({cid => "P-Key", check_cid => "C-Key"}))
  ...
  describe "send_check" do
    ...
    it "sends checks and adjustments" do
      ...
      expect(subject).to receive(:post_xml).with("c", true, true, (xml + adj_xml), [cid, adj_id])
                                           .and_return(create_multi_result_key_response({cid => "GL-Account-Key", adj_id => "Adj-Key"}))
  ...
  describe "post_xml" do
    ...
    context "in production" do
      ...
      it "raises retry error for errors with correction values of 'Check the transaction for errors or inconsistencies, then try again.'" do
        error = "<errormessage>
        <error><errorno>BL01001973</errorno><description></description><description2>Desc 1</description2>
        <correction>Check the transaction for errors or inconsistencies, then try again.</correction></error>
      ...
      it "raises dimension error for errors with descriptions indicating missing dimensions" do
        error = "<errormessage>
        <error><errorno>BL01001973</errorno><description></description><description2>Invalid Brokerage File '2399341' specified.</description2>
  ...
  describe "read_object" do
    ...
    it "generates and sends a read request" do
      ...
      expect(subject).to receive(:post_xml).with("location", false, false, "<read>object</read>", ["control_id"], request_options: {api_version: "3.0"}, read_only: true)
                                           .and_return(response)
    ...
    it "generates a request for multiple results and returns multiple results if an array of keys is passed" do
      ...
      response = generate_intacct_read_response([{control_id: "control_id", object_type: "object", value: "value"},
                                                 {control_id: "control_id2", object_type: "object", value: "value2"}])
      expect(subject).to receive(:post_xml).with("location", false, false, "<read>object</read><read>object2</read>",
                                                 ["control_id", "control_id2"], request_options: {api_version: "3.0"}, read_only: true).and_return response
  ...
  describe "list_objects" do
    ...
    it "executes a readMore if the response indicates there are more results to request" do
      ...
      expect(subject).to receive(:post_xml).with("location", false, false, "<readMore>resultId</readMore>", "control_id2",
                                                 request_options: {api_version: "3.0"}, read_only: true)

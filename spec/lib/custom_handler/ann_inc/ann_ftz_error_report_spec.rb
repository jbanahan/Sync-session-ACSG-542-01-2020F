describe OpenChain::CustomHandler::AnnInc::AnnFtzErrorReport do
  ...
  let(:user) { create(:user, username: "Nigel Tufnel") }
  let(:approved_date) { Date.new(2019, 3, 15) }
  let(:prod) do
    p = create(:product, unique_identifier: "style", last_updated_by: user)
  ...
  let(:tariff_1) do
    tr = create(:tariff_record, classification: classi, hts_1: "123456789", line_number: 1)
  ...
  let(:tariff_2) do
    tr = create(:tariff_record, classification: classi, hts_1: "987654321", line_number: 2)
  ...
  let(:bvt) { create(:business_validation_template, module_type: "Product", system_code: "FTZ") }
  let(:bvre) { create(:business_validation_result, business_validation_template: bvt, validatable: prod) }
  let(:bvru) { create(:business_validation_rule, business_validation_template: bvt) }
  let(:bvrr) { create(:business_validation_rule_result_without_callback, business_validation_rule: bvru, business_validation_result: bvre, state: "Fail", message: "FAIL!!") }
  ...
  describe "run_schedulable" do
    ...
    it "creates and emails report" do
      ...
      dist_list = create(:mailing_list, system_code: "FTZ list", email_addresses: "tufnel@stonehenge.biz")

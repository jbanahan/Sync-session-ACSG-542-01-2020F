describe OpenChain::CustomHandler::FenixInvoiceParser do
  ...
  let! (:ent) { create(:entry, source_system: 'Fenix', broker_reference: '280952') }
  let (:ent2) { create(:entry, source_system: 'Fenix', broker_reference: '281350') }
...
INV
  ...
  it "assigns fiscal month to broker invoice" do
    imp = create(:company, fiscal_reference: "ent_release_date")
    ent.update!(importer: imp, release_date: "20130105")
    fm = create(:fiscal_month, company: imp, year: 2013, month_number: 1, start_date: Date.new(2013, 1, 1), end_date: Date.new(2015, 1, 31))
    subject.parse_file content, log

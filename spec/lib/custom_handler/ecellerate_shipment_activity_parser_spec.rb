describe OpenChain::CustomHandler::EcellerateShipmentActivityParser do
  ...
  describe "can_view?" do
    it "must have ecellerate custom feature" do
      ...
      expect(described_class.can_view?(create(:master_user))).to be_falsey
    end
    ...
    it "should pass for user who can edit shipments, is master, and has custom feature enabled" do
      ...
      allow_any_instance_of(User).to receive(:edit_shipments?).and_return true
      expect(described_class.can_view?(create(:master_user))).to be_truthy
    end
  end
  describe "process" do
  ...
  describe "parse" do
    ...
    before :each do
      @imp = create(:company, importer:true, ecellerate_customer_number:'CUSTNUM')
    end
    it "should update existing shipment" do
      s = create(:shipment, importer:@imp, house_bill_of_lading:'HBOL123')
      r = default_row
    ...
    it "should skip shipments where ecellerate_customer_number not found for importer" do
      ...
      d = 1.week.ago
      s = create(:shipment, importer:@imp, house_bill_of_lading:r[1], updated_at:d)

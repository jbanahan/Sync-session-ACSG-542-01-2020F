describe OpenChain::CustomHandler::Siemens::SiemensEntryArchiveSender do
  ...
  let(:ent1) do
    ent = Factory(:entry, broker_reference: "brok1", importer: co1, entry_number: "ent_num1", file_logged_date: Date.new(2020, 1, 6))
  ...
  let(:ent2) do
    ent = Factory(:entry, broker_reference: "brok2", importer: co2, entry_number: "ent_num2", file_logged_date: Date.new(2020, 1, 6))
    ent.sync_records.create! trading_partner: described_class::XML_SYNC_TRADING_PARTNER, sent_at: yesterday, confirmed_at: yesterday + 1.minute
    ent
  end

  let(:ent3) do
    ent = Factory(:entry, broker_reference: "brok3", importer: co3, entry_number: "ent_num3", file_logged_date: Date.new(2020, 1, 6))
    # included to test importer -- non-Siemens entries would never have one of these
    ent.sync_records.create! trading_partner: described_class::XML_SYNC_TRADING_PARTNER, sent_at: yesterday, confirmed_at: yesterday + 1.minute
    ent
  end

  let(:att1) { Factory(:attachment, attachable: ent1, attachment_type: "Archive Packet", created_at: yesterday - 1.minute) }
  let(:att2) { Factory(:attachment, attachable: ent2, attachment_type: "Archive Packet", created_at: yesterday - 1.minute) }
  let(:att3) { Factory(:attachment, attachable: ent3, attachment_type: "Archive Packet", created_at: yesterday - 1.minute) }
  ...
  before do
    ...
    [ent1, ent2, ent3].each { |ent| Factory(:broker_invoice, entry: ent, invoice_date: "2020-03-15") }

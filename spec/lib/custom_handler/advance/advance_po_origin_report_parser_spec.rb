describe OpenChain::CustomHandler::Advance::AdvancePoOriginReportParser do
  ...
  describe "process" do
    ...
    let (:product1) {
      create(:product, importer: cq, unique_identifier: "CQ-PART1")
    }

    let (:product2) {
      prod = create(:product, importer: cq, unique_identifier: "CQ-PART2")
    }

    let (:product3) {
      prod = create(:product, importer: cq, unique_identifier: "CQ-PART3")
    }
    ...
    context "with all products present" do
      ...
      context "with different part number hyphenation" do
        ...
        it "falls back to part number matching if multiple parts are found by sku number" do
          ...
          prod = create(:product, importer: cq, unique_identifier: "CQ-PART-1X")

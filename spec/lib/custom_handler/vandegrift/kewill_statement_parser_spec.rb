describe OpenChain::CustomHandler::Vandegrift::KewillStatementParser do
  ...
  let (:test_importer) { with_customs_management_id(create(:importer), "TEST") }
  let (:user) { create(:user) }
  let (:port) { create(:port, schedule_d_code: "4103")}
  ...
  describe "process_daily_statement" do
    ...
    let! (:entry) { create(:entry, source_system: Entry::KEWILL_SOURCE_SYSTEM, broker_reference: "2316567") }
    let! (:broker_invoice) {
      inv_line = create(:broker_invoice_line, broker_invoice: create(:broker_invoice, entry: entry), charge_code: "0001", charge_amount: 100)
      inv_line = create(:broker_invoice_line, broker_invoice: create(:broker_invoice, entry: entry), charge_code: "0002", charge_amount: 100)

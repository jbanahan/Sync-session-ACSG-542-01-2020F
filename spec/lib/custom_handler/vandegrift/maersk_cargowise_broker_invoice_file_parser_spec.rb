describe OpenChain::CustomHandler::Vandegrift::MaerskCargowiseBrokerInvoiceFileParser do
  ...
  describe "parse" do
    ...
    it "creates a broker invoice" do
      ...
      expect(entry.broker_invoice_total).to eq BigDecimal.new("270.59")
      expect(entry.total_duty_direct).to eq BigDecimal.new("27.77")
      ...
      expect(inv.invoice_total).to eq BigDecimal.new("270.59")
      ...
      expect(inv_line_1.charge_amount).to eq BigDecimal.new("0.00")
      ...
      expect(inv_line_2.charge_amount).to eq BigDecimal.new("83.00")
      ...
      expect(inv_line_3.charge_amount).to eq BigDecimal.new("17.59")
      ...
      expect(inv_line_4.charge_amount).to eq BigDecimal.new("170.00")
      ...
      expect(log).to_not have_info_message "Cargowise-sourced entry matching Broker Reference 'BQMJ01119290922' was not found, so a new entry was created."
    end
    ...
    it "updates an existing broker invoice" do
      ...
      expect(entry.broker_invoice_total).to eq BigDecimal.new("326.03")
    ...
    it "creates new entry when entry not found" do
      ...
      expect(Entry.where(broker_reference:"BQMJ01119290922", source_system:Entry::CARGOWISE_SOURCE_SYSTEM).first).to_not be_nil
    ...
    it "creates new entry when broker-ref-matching entry is of wrong type" do
      entry = Factory(:entry, broker_reference:"BQMJ01119290922", source_system:Entry::KEWILL_SOURCE_SYSTEM)
    ...
    it "rejects when broker reference is missing" do
      ...
      expect(log).to_not have_info_message "Broker invoice successfully processed."
    ...
    it "handles missing optional values" do
      ...
      expect(inv.invoice_total).to eq BigDecimal.new(0)
      expect(inv.bill_to_country_id).to be_nil

      inv_line_1 = inv.broker_invoice_lines[0]
      expect(inv_line_1.charge_amount).to eq BigDecimal.new(0)

  ...
  describe "accepts?" do
    ...
    context "with 'ISF 315' custom feature" do
      before :each do 
        ...
        ms.stub(:custom_feature?).with("ISF 315").and_return true
      end
      ...
      it "does not isf with missing importer account code" do
        ...
        expect(subject.accepts? :save, isf).to be_false
      end

      it "accepts isf with all standard info" do
        mnc.save!
        expect(subject.accepts? :save, isf).to be_true
      end
  ...
  describe "receive" do
    ...
    it "sends ISF data" do
      ...
      subject.should_receive(:generate_and_send_xml_document) do |cust_no, data, testing|
        ...
        cap = data
      end.and_yield(fake_data)

      Lock.should_receive(:acquire).with("315-ref").and_yield
    ...
    it "sends if all search criterions pass" do
      ...
      subject.should_receive(:generate_and_send_315s)
    ...
    it "does not send if any search criterion fails" do
      ...
      # If this method is called, then receive has done its job
      subject.should_not_receive(:generate_and_send_315s)

describe OpenChain::FiscalCalendarSchedulingSupport do 
  ...
  describe "run_if_fiscal_day" do
    ...
    it "yields if the given date is on the Xth date of a fiscal quarter" do
      ...
      expect {|b| subject.run_if_fiscal_day(importer, 7, current_time: Date.new(2017, 9, 13), quarterly: true, &b)}.to yield_with_args(fiscal_quarter_first_month, Date.new(2017, 9, 13))
    end
    ...
    it "does not yield if the given date is not on the Xth date of a fiscal quarter" do
      ...
      expect { |b| subject.run_if_fiscal_day(importer, 6, current_time: Date.new(2017, 9, 13), quarterly: true, &b) }.not_to yield_control
    end
    ...
    it "yields if the given date is on the Xth date of a fiscal quarter involving month-spanning" do
      ...
      expect {|b| subject.run_if_fiscal_day(importer, 70, current_time: Date.new(2017, 9, 13), quarterly: true, &b)}.to yield_with_args(fiscal_quarter_first_month, Date.new(2017, 9, 13))
    end
    ...
    it "yields if the given date is on the Xth date of a fiscal quarter involving month-and-year-spanning" do
      ...
      expect {|b| subject.run_if_fiscal_day(importer, 37, current_time: Date.new(2018, 1, 21), quarterly: true, &b)}.to yield_with_args(fiscal_quarter_first_month, Date.new(2018, 1, 21))
    end
    ...
    it "handles case where first month of quarter is missing" do
      ...
      expect { |b| subject.run_if_fiscal_day(importer, 37, current_time: Date.new(2018, 1, 21), quarterly: true, &b) }.not_to yield_control
    end
  end
  ...
  describe "run_if_configured" do
    ...
    it "uses configuration with default values" do
      expect(subject).to receive(:run_if_fiscal_day).with("ASCE", 5, relative_to_timezone: "America/New_York", relative_to_start: true, quarterly:false).and_yield fiscal_month, fiscal_date
    ...
    it "passes alternate timezone, relative to start and quarterly values" do
      ...
      expect(subject).to receive(:run_if_fiscal_day).with("ASCE", 5, relative_to_timezone: "UTC", relative_to_start: false, quarterly:true)

  ...
  describe OpenChain::ActivitySummary::USEntrySummaryGenerator do
    ...
    describe :generate_unpaid_duty_section do
      it "should delegate to #single_company_unpaid_duty and #linked_companies_unpaid_duty, producing joined/flattened list" do
        ...
        expect_any_instance_of(described_class::USEntrySummaryGenerator).to receive(:single_company_unpaid_duty).with(c, Date.today).and_return single_total
        expect_any_instance_of(described_class::USEntrySummaryGenerator).to receive(:linked_companies_unpaid_duty).with(c, Date.today).and_return [linked_total_1, linked_total_2, linked_total_3]
    ...
    describe :linked_companies_unpaid_duty do
      it "should populate an array with the totals of an importer's linked companies" do
        ...
        expect_any_instance_of(described_class::USEntrySummaryGenerator).to receive(:single_company_unpaid_duty) {|instance, c, date| [company_name: c.name]}.exactly(3).times
    ...
    before(:each) do
      ...
      describe :create_digest do
        it "should delegate to build and get_entries" do
          ...
          c = double('company')
          e = double('entries')
          expect(described_class::DutyDetail).to receive(:get_entries).with(u, c).and_return e
          expect(described_class::DutyDetail).to receive(:build_digest).with(e).and_return 'abc'

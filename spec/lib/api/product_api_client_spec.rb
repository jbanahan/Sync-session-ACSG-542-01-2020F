...
describe OpenChain::Api::ProductApiClient do
  ...
  describe "find_by_uid" do
    it "uses get with the correct path for find_by_uid" do
      expect(@c).to receive(:mf_uid_list_to_param).with([:uid]).and_return({'param'=>'value'})
      expect(@c).to receive(:get).with("/products/by_uid", {'param'=>'value', :uid => "style"}).and_return "json"
  ...
  describe "show" do
    it "uses get to find by a specific id" do
      expect(@c).to receive(:mf_uid_list_to_param).with([:uid]).and_return({'param'=>'value'})
      expect(@c).to receive(:get).with("/products/1", {'param'=>'value'}).and_return "json"
  ...
  describe "create" do
    it "uses post to create" do
      expect(@c).to receive(:post).with("/products", hash).and_return "json"
  ...
  describe "update" do
    it "uses put to update" do
      ...
      expect(@c).to receive(:put).with("/products/1", create_hash).and_return "json"
    ...
    it "uses handles non-symbols for finding id to update" do
      ...
      expect(@c).to receive(:put).with("/products/2", create_hash).and_return "json"
      expect(@c.update(create_hash)).to eq "json"
    end

describe OpenChain::Api::ApiEntityXmlizer do
  ...
  context 'with data' do
    ...
    it 'should create xml with base tag names' do
      xml = subject.entity_to_xml(create(:admin_user), order, fields)
    ...
    it 'should user xml_tag_overrides from ModelField' do
      cd = create(:custom_definition, module_type:'OrderLine', data_type:'string')
      ...
      expect(subject.entity_to_xml(create(:admin_user), order, fields)).to eq IO.read('spec/fixtures/files/api_entity_xmlizer_sample_custom_tag.xml')
    end
  end

describe CsvMaker do
  context "make_from_search_query" do
    before :each do
      ...
      @entry = FactoryBot(:entry, :first_it_date=>Date.new(2014, 7, 30), :file_logged_date=>@logged_date, :broker_reference => "x")
      @entry.reload # get right rails date objects
      @u = FactoryBot(:master_user, :entry_view=>true, :time_zone=>"Hawaii")
    ...
    it "should strip newline characters" do
      val = "abc\ndef"
      FactoryBot(:product, :unique_identifier=>val)
      ss = FactoryBot(:search_setup, :module_type=>"Product", :user=>FactoryBot(:master_user))

describe ResultCache do
  ...
  describe "next" do
    it "should load cache if empty" do
      ss = FactoryBot(:search_setup, :module_type=>"Product")
      ss.sort_criterions.create!(:model_field_uid=>:prod_uid)
      p = []
      2.times {|i| p << FactoryBot(:product, :unique_identifier=>"rc#{i}").id }
    ...
    it "should find in next page" do
      ...
      ss.sort_criterions.create!(:model_field_uid=>:prod_uid)
      p = []
      6.times {|i| p << FactoryBot(:product, :unique_identifier=>"rc#{i}").id }
    ...
    it "should not return same object id from next page" do
      ...
      p1 = FactoryBot(:product, unique_identifier: "1")
      p2 = FactoryBot(:product, unique_identifier: "2")
      3.times do |i|
        FactoryBot(:classification, :product=>p1)
      end
      FactoryBot(:classification, :product=>p2)

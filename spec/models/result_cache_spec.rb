describe ResultCache do
  ...
  describe "next" do
    it "should load cache if empty" do
      ss = create(:search_setup, :module_type=>"Product")
      ss.sort_criterions.create!(:model_field_uid=>:prod_uid)
      p = []
      2.times {|i| p << create(:product, :unique_identifier=>"rc#{i}").id }
    ...
    it "should find in next page" do
      ...
      ss.sort_criterions.create!(:model_field_uid=>:prod_uid)
      p = []
      6.times {|i| p << create(:product, :unique_identifier=>"rc#{i}").id }
    ...
    it "should not return same object id from next page" do
      ...
      p1 = create(:product, unique_identifier: "1")
      p2 = create(:product, unique_identifier: "2")
      3.times do |i|
        create(:classification, :product=>p1)
      end
      create(:classification, :product=>p2)

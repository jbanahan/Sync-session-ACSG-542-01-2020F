  ...
  context 'security' do
    describe '#can_view?' do
      it 'should defer to product.can_view?' do
        u = double('user')
        p = Product.new
        p.should_receive(:can_view?).with(u).and_return 'view'
    ...
    describe '#can_edit?' do
      it 'should defer to product.can_classify?' do
        u = double('user')
        p = Product.new
        p.should_receive(:can_classify?).with(u).and_return 'cs'
    ...
    describe '#search_secure' do
      it 'should wrap to product.search_where' do
        u = double('user')
        Product.should_receive(:search_where).with(u).and_return '99=99'

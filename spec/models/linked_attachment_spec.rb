describe LinkedAttachment do
  context 'validations' do
    it 'should require an attachable' do
      a = LinkedAttachment.create(:linkable_attachment=>create(:linkable_attachment))
      expect(a.errors.full_messages.size).to eq(1)
    end
    it 'should require a linkable attachment' do
      a = LinkedAttachment.create(:attachable=>create(:product))
      expect(a.errors.full_messages.size).to eq(1)
    end
    it 'should save with an attachable and linkable attachment' do
      a = LinkedAttachment.create(:attachable=>create(:product), :linkable_attachment=>create(:linkable_attachment))
  ...
  describe 'static creators' do
    before(:each) do
      non_matching_linkable = create(:linkable_attachment)
      non_matching_attachable = create(:product)
    end


    describe 'create_from_attachable_by_class_and_id' do
      it "should load object and pass" do
        prod_id = create(:product).id
    ...
    describe 'create_from_attachable' do
      before(:each) do
        @product = create(:product, :unit_of_measure=>'uom')
      end
      it 'should do nothing with no matching linkable attachments' do
        r = LinkedAttachment.create_from_attachable(create(:product))
        expect(r.size).to eq(0)
        expect(LinkedAttachment.all.size).to eq(0)
      end
      it 'should create 1 with 1 matching linkable attachment' do
        linkable = create(:linkable_attachment, :model_field_uid=>'prod_uid', :value=>@product.unique_identifier)
      ...
      it 'should create 2 with 2 matching linkable attachments on different model fields' do
        linkables = []
        {'prod_uid'=>@product.unique_identifier, 'prod_uom'=>@product.unit_of_measure}.each {|k, v| linkables << create(:linkable_attachment, :model_field_uid=>k, :value=>v)}
        r = LinkedAttachment.create_from_attachable(@product)
      ...
      it 'should do nothing if already linked to all potential matches' do
        2.times {|i| create(:linkable_attachment, :model_field_uid=>'prod_uid', :value=>@product.unique_identifier)}
    ...
    describe 'create_from_linkable_attachment' do
      before(:each) do
        @product = create(:product, :unit_of_measure=>'uomx')
        @linkable = create(:linkable_attachment, :model_field_uid=>'prod_uom', :value=>@product.unit_of_measure)
      end
      ...
      it 'should create 2 with 2 matching attachables' do
        p2 = create(:product, :unit_of_measure=>@product.unit_of_measure)
        found = LinkedAttachment.create_from_linkable_attachment @linkable

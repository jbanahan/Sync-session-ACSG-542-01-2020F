  ...
  describe "valid_isf?" do
    describe 'when the ISF is valid' do
      before :each do
        ...
        create(:shipment_line, shipment:@shipment, manufacturer_address:create(:full_address))
        create(:shipment_line, shipment:@shipment, manufacturer_address:create(:full_address))
    ...
    describe "when the ISF is not valid" do
      it "returns false" do
        ...
        expect(create(:shipment).valid_isf?).to be_falsey
      end
    end
  end
  ...
  describe "validate_isf" do
    describe 'when the ISF is not valid' do
      describe "when the importer doesn't have an kewill customs number" do
        before :each do
          importer = create(:importer, irs_number:'ashjdajdashdad')
          @shipment = create(:shipment, importer:importer)
      ...
      describe "when the consignee doesn't have an IRS number" do
        before :each do
          ...
          @shipment = create(:shipment, consignee:consignee)
          @shipment.validate_isf
        end
      ...
      describe 'when the shipment is missing both bill of lading numbers' do
        before :each do
          @shipment = create :shipment, master_bill_of_lading:nil, house_bill_of_lading:nil
          @shipment.validate_isf
        end
      ...
      describe 'when one of the shipment lines is missing a manufacturer address' do
        before :each do
          ...
          create :shipment_line, shipment:@shipment, manufacturer_address:create(:full_address)
          create :shipment_line, shipment:@shipment, manufacturer_address:create(:full_address)
          @no_address_line = create :shipment_line, shipment:@shipment, manufacturer_address_id:nil
          @shipment.validate_isf
        end
      ...
      describe "when manufacturer address is missing required fields" do
        before :each do
          @shipment = create :shipment
          @line = create :shipment_line, shipment:@shipment, manufacturer_address:create(:address)
      ...
      describe "when parties are missing address fields" do
        before :each do
          ...
          @shipment = create(:shipment,
                              importer:importer,
                              consignee: consignee,
                              seller_address: create(:blank_address),
                              buyer_address: create(:blank_address),
                              ship_to_address: create(:blank_address),
                              container_stuffing_address: create(:blank_address),
                              consolidator_address: create(:blank_address),
                              house_bill_of_lading:'this is a bill')
        end
    ...
    describe "when Country of Origin or HTS number is missing from a shipment line" do
      ...
      it "is invalid when country of origin is missing from shipment line" do
        line_1 = create(:shipment_line, shipment:@shipment)

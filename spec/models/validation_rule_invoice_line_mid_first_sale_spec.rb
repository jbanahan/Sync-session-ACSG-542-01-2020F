describe ValidationRuleInvoiceLineMidFirstSale do
  ...
  it 'passes if all invoice lines that have a mid are marked first_sale (handling nil as first_sale)' do
    ...
    invoice_line_1 = create(:commercial_invoice_line, commercial_invoice: invoice, mid: '123456789', first_sale: nil)
  ...
  it 'passes if all invoice lines that have a mid are marked first_sale' do
    invoice = create(:commercial_invoice, entry: entry)
    invoice_line_1 = create(:commercial_invoice_line, commercial_invoice: invoice, mid: '123456789', first_sale: true)
    xref = DataCrossReference.create!(company_id: company.id, cross_reference_type: DataCrossReference::ENTRY_MID_VALIDATIONS, key: 123456789)
    invoice_line_1 = create(:commercial_invoice_line, commercial_invoice: invoice, first_sale: true)
  ...
  it 'fails if any invoice lines have a mid not marked first_sale' do
    invoice = create(:commercial_invoice, entry: entry)
    invoice_line_1 = create(:commercial_invoice_line, commercial_invoice: invoice, po_number: "12345", mid: '123456789', first_sale: false)
    xref = DataCrossReference.create!(company_id: company.id, cross_reference_type: DataCrossReference::ENTRY_MID_VALIDATIONS, key: 123456789)

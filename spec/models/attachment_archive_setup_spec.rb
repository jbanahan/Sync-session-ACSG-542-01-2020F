...
describe AttachmentArchiveSetup do
  ...
  describe "create_entry_archive!" do
    it "should include relevant attachments" do
      ...
      expect(archive.company).to eq(@c)
      expect(archive.attachments.size).to eq(2)
    ...
    it "should not include attachments invoiced before start_date" do
      ...
      expect(@setup.create_entry_archive!("my name", 1000).attachments.count).to eq(0)
    end
    ...
    it "should include attachments that have one invoice newer than 30 days ago" do
      ...
      expect(@setup.create_entry_archive!("my name", 1000).attachments.count).to eq(2)
    end
    ...
    it "should not include attachments on another archive" do
      ...
      expect(@setup.create_entry_archive!("my name", 1000).attachments.to_a).to eq([@att2])
    end
  ...
  describe "entry_attachments_available?" do
    it "should return true if unarchived attachments exist for company" do
      expect(@setup).to be_entry_attachments_available
    end
    ...
    it "uses override list if given" do
      ...
      @setup.update_attributes! end_date: Date.new(2013, 1, 1)
      @setup.broker_reference_override = [@entry.broker_reference]
      expect(@setup.entry_attachments_available?).to be_truthy
    end
  end
end

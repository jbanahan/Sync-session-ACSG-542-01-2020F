describe AttachmentArchiveSetup do
  ...
  let (:entry) { create(:entry, importer: importer, arrival_date: 1.month.ago, broker_reference: "REF") }
  ...
  let (:invoice) { create(:broker_invoice, entry: entry, invoice_date: (Time.current.midnight - 30.days) - 1.second) }
  ...
  describe "create_entry_archive!" do
    ...
    it "includes attachments that have one invoice newer than 30 days ago" do
      create(:broker_invoice, entry: entry, invoice_date: Time.current)
    ...
    it "includes attachments only once" do
      ...
      create(:broker_invoice, entry: entry, invoice_date: 40.days.ago)
      expect(archive_setup.create_entry_archive!("my name", 1000).attachments.length).to eq(2)
    end
  ...
  describe "create_entry_archives_for_reference_numbers!" do
    ...
    it "creates multiple archives when there are too many files for one archive" do
      entry_2 = create(:entry, broker_reference: "file2", importer: importer, arrival_date: Time.zone.now.to_date)

describe Plant do
  describe '#search_where' do
    it "should find plant only where user can find company" do
      plant = create(:plant)
      plant_not_to_find = create(:plant)
      user = create(:user, company:plant.company)
  ...
  describe "can_view?" do
    it "should allow if user can view company as vendor" do
      ...
      u = double(:user)
      expect(create(:plant).can_view?(u)).to be_truthy
    end
    ...
    it "should not allow if user cannot view company or company as vendor" do
      ...
      u = double(:user)
      expect(create(:plant).can_view?(u)).to be_falsey
    end
  end
  describe "can_attach?" do
    it "should allow if user can attach to company" do
      ...
      u = double(:user)
      expect(create(:plant).can_attach?(u)).to be_truthy
    end
    it "should not allow if user cannot attach to company" do
      ...
      u = double(:user)
      expect(create(:plant).can_attach?(u)).to be_falsey
    end
  end
  ...
  describe "unassigned_product_groups" do
    it "shoud return unassigned product groups" do
      plant = create(:plant)
      pg1 = create(:product_group, name: 'PGA')
      pg2 = create(:product_group, name: 'PGB')

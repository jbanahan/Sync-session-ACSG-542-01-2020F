describe CoreModule do
  ...
  it 'should return class by calling klass' do
    expect(CoreModule::PRODUCT.klass).to be Product
  end
  describe 'top?' do
    it "should return true for top of chain" do
      expect(CoreModule::PRODUCT.default_module_chain.top?(CoreModule::PRODUCT)).to be_truthy
    end
    it "should return false for item with parent in chain" do
      expect(CoreModule::PRODUCT.default_module_chain.top?(CoreModule::CLASSIFICATION)).to be_falsey
    end
  ...
  describe 'key_columns' do
    it 'should return for entry' do
      ...
      expect(uids).to eq([:ent_brok_ref])
      expect(uids.first).to be_a_model_field_uid
    end
    it 'should return for order' do
      uids = CoreModule::ORDER.key_model_field_uids
      expect(uids).to eq([:ord_ord_num])
  ...
  describe "walk_object_heirarchy" do
    it "yields each object in core module heirarchy" do
      p = create(:product)
      t1 = create(:tariff_record, classification: create(:classification, product: p))
      t2 = create(:tariff_record, classification: create(:classification, product: p))
  ...
  describe "find_by" do
    ...
    it "does not override traditional find_by behavior for CoreModules" do
      ord = create(:order, order_number: "555ord666")

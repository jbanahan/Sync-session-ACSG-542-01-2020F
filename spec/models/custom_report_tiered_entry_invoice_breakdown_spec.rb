describe CustomReportTieredEntryInvoiceBreakdown do
  ...
  context "static methods" do
    ...
    it "should allow all column fields as criterion fields" do
      expect(@kls.criterion_fields_available(@importer_user)).to eq(@kls.column_fields_available(@importer_user))
    end
    ...
    it "should allow users who can view broker invoices to view" do
      ...
      expect(@kls.can_view?(@master_user)).to be_truthy
    end
    it "should not allow users who cannot view broker invoices to view" do
      ...
      expect(@kls.can_view?(@master_user)).to be_falsey
    end
  end
  ...
  it "produces report" do
    master_user = create(:master_user)
    allow(master_user).to receive(:view_broker_invoices?).and_return(true)
    invoice_line = create(:broker_invoice_line, :charge_description=>"CD1", :charge_amount=>100.12)
    invoice_line.broker_invoice.entry.update_attributes(:entry_number=>"31612345678", :broker_reference=>"1234567")
    create(:broker_invoice_line, :broker_invoice=>invoice_line.broker_invoice, :charge_description=>"CD2", :charge_amount=>55)
    broker_invoice_2 = create(:broker_invoice, entry: invoice_line.broker_invoice.entry)

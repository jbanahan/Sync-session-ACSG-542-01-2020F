describe User do
  describe "new_announcements" do
    ...
    it "returns associated announcements within date range for type 'all' in descending order" do
      anc1 = create(:announcement, category: 'all', start_at: now - 1.day, end_at: now + 1.day)
      anc2 = create(:announcement, category: 'all', start_at: now - 1.day, end_at: now + 1.day)
      anc3 = create(:announcement, category: 'all', start_at: now - 1.day, end_at: now + 1.day)
    ...
    it "returns associated announcements within date range for type 'user'" do
      user2 = create(:user, time_zone: "Eastern Time (US & Canada)")
      anc1 = create(:announcement, category: 'all', start_at: now - 1.day, end_at: now + 1.day)
      anc2 = create(:announcement, category: 'users', start_at: now - 1.day, end_at: now + 1.day)
    ...
    it "omits confirmed announcements" do
      anc = create(:announcement, category: 'all', start_at: now - 1.day, end_at: now + 1.day)
      create(:user_announcement_marker, announcement: anc, user: user, confirmed_at: now)

      expect(user.new_announcements).to be_empty
    end

    it "omits announcements before UTC start date" do
      create(:announcement, category: 'all', start_at: now + 1.hour, end_at: now + 1.day)

      expect(user.new_announcements).to be_empty
    end

    it "omits announcements after UTC end date" do
      create(:announcement, category: 'all', start_at: now - 1.day, end_at: now - 1.hour)
  ...
  describe "api_hash" do
    let (:user) do
      create(:user, first_name: 'Joe', last_name: 'User', username: 'uname', email: 'j@sample.com',
  ...
  describe "groups" do
    let(:group_a) { create(:group, system_code: 'groupA') }
    let(:group_b) { create(:group, system_code: 'groupB') }
  ...
  describe "available_importers" do
    let!(:company_one) { create(:company, importer: true) }
    let!(:company_two) { create(:company, importer: true) }
    let!(:company_three) { create(:company, importer: true) }
    let!(:company_four) { create(:company) }
    ...
    it "shows all linked importers" do
      u = create(:user, company: company_four)
    ...
    it "shows me if i'm an importer" do
      u = create(:user, company: company_two)
  ...
  describe "api_admin" do
    ...
    it "returns api_admin if it exits" do
      u = create(:master_user, username: 'ApiAdmin')
  ...
  describe "integration" do
    ...
    it "returns integration if it exits" do
      u = create(:master_user, username: 'integration')
      expect(described_class.integration).to eq u
    end
  end

  describe "magic_columns" do
    let(:user) { create(:user, updated_at: 1.year.ago) }
  ...
  context "permissions" do
    ...
    context "official tariffs" do
      it "allows master company user" do
        expect(create(:master_user)).to be_view_official_tariffs
      end

      it "does not allow non master company user" do
        expect(create(:user)).not_to be_view_official_tariffs
      end

      it "allows if user can view products" do
        expect(create(:user, product_view: true).view_official_tariffs?).to be_truthy
      end
    end
    ...
    context "attachment_archives" do
      it "allows for master user who can view entries" do
        u = create(:user, company: create(:company, master: true))
    ...
    context "security filing" do
      context "company has permission" do
        ...
        it "allows if permission set and company has permission" do
          u = create(:user, security_filing_view: true, security_filing_edit: true, security_filing_attach: true, security_filing_comment: true)
        ...
        it "does not allow if user permission not set and company has permission" do
          u = create(:user, security_filing_view: false, security_filing_edit: false, security_filing_attach: false, security_filing_comment: false)
    ...
    context "broker invoice" do
      context "with company permission" do
        ...
        it "allows view if permission is set" do
          expect(create(:user, broker_invoice_view: true).view_broker_invoices?).to be_truthy
        end

        it "allows edit if permission is set" do
          expect(create(:user, broker_invoice_edit: true).edit_broker_invoices?).to be_truthy
        end

        it "does not allow view without permission" do
          expect(create(:user, broker_invoice_view: false).view_broker_invoices?).to be_falsey
        end

        it "does not allow edit without permission" do
          expect(create(:user, broker_invoice_edit: false).edit_broker_invoices?).to be_falsey
        end
      end
      ...
      context "without company permission" do
        ...
        it "does not allow view even if permission is set" do
          expect(create(:user, broker_invoice_view: true).view_broker_invoices?).to be_falsey
        end

        it "does not allow edit even if permission is set" do
          expect(create(:user, broker_invoice_edit: true).edit_broker_invoices?).to be_falsey
        end
      end
    end
    ...
    context "vfi invoice" do
      context "with company permission" do
        ...
        it "allows view if permission is set" do
          expect(create(:user, vfi_invoice_view: true).view_vfi_invoices?).to be_truthy
        end

        it "allows edit if permission is set" do
          expect(create(:user, vfi_invoice_edit: true).edit_vfi_invoices?).to be_truthy
        end

        it "does not allow view without permission" do
          expect(create(:user, vfi_invoice_view: false).view_vfi_invoices?).to be_falsey
        end

        it "does not allow edit without permission" do
          expect(create(:user, vfi_invoice_edit: false).edit_vfi_invoices?).to be_falsey
        end
      end
      ...
      context "without company permission" do
        ...
        it "does not allow view even if permission is set" do
          expect(create(:user, vfi_invoice_view: true).view_vfi_invoices?).to be_falsey
        end

        it "does not allow edit even if permission is set" do
          expect(create(:user, vfi_invoice_edit: true).edit_vfi_invoices?).to be_falsey
        end
      end
    end
    ...
    context "entry" do
      let(:company) { create(:company, broker: true) }

      it "allows user to edit entry if permission is set and company is not broker" do
        expect(create(:user, entry_edit: true, company: company)).to be_edit_entries
      end
    ...
    context "variant" do
      let(:user) { create(:master_user, product_edit: true) }
    ...
    context "powers of attorney" do
      let!(:group) { create(:group, system_code: "maintain_poa")}
      let!(:user) { create(:user) }
  ...
  describe "access_allowed?" do
    ...
    it "validates user company is not locked" do
      user = create(:user, company: create(:company, locked: true))
      expect(described_class.access_allowed?(user)).to be_falsey
    end
  ...
  describe "on_successful_login" do
    it "sets last_login_at, current_login_at, failed_login_count and creates a history record" do
      user = create(:user, current_login_at: Date.new(2014, 1, 1), failed_login_count: 10)
    ...
    it "doesn't update host with port if it's not blank" do
      user = create(:user, host_with_port: "www.test.com")
  ...
  describe "from_omniauth" do
    let!(:user) { create(:user, email: "condoleeza@rice.com") }
  ...
  describe "email validation" do
    it "updates email field if all members of semicolon/comma-separated list match regex pattern" do
      u = create(:user, email: "default@vandegriftinc.com")
  ...
  describe "view_statements?" do
    let (:user) { create(:user, statement_view: true) }
  ...
  describe "user_auth_token" do
    let (:user) do
      create(:user, username: "username", api_auth_token: "authtoken")
    end

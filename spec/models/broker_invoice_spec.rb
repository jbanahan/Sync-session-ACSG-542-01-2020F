describe BrokerInvoice do
  describe "hst_amount" do
    it "calculates HST based on existing charge codes" do
      with_hst_code_1 = create(:charge_code, apply_hst: true)
      with_hst_code_2 = create(:charge_code, apply_hst: true)
      without_hst = create(:charge_code, apply_hst: false)
  ...
  context 'security' do
    ...
    let!(:importer_user) { create(:user, company_id: importer.id, broker_invoice_view: true) }
    let!(:entry) {  create(:entry, importer_id: importer.id) }
    let!(:inv) { create(:broker_invoice, entry_id: entry.id) }

    context 'search secure' do
      before do
        create(:broker_invoice, entry_id: create(:entry, importer_id: create(:company, importer: true)).id)
      end
      ...
      it 'allows all for master' do
        u = create(:user, broker_invoice_view: true)
      ...
      it 'allows for linked company' do
        child = create(:company, importer: true)
        i3 = create(:broker_invoice, entry: create(:entry, importer_id: child.id))
    ...
    it 'is not visible for another importer' do
      u = create(:user, company_id: create(:company, importer: true).id, broker_invoice_view: true)
      expect(inv.can_view?(u)).to be_falsey
    end

    it 'is visible for parent importer' do
      parent = create(:company, importer: true)
      parent.linked_companies << importer
      u = create(:user, company_id: parent.id, broker_invoice_view: true)

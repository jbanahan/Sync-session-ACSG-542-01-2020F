...
describe BusinessValidationRuleResult do
  ...
  describe "can_edit" do
    ...
    let(:business_validation_rule) { create(:business_validation_rule, group: group) }
    let(:business_validation_rule_result) { create(:business_validation_rule_result, business_validation_rule: business_validation_rule) }

    it "allows users who belong to rule's override group" do
      u = create(:user, groups: [group])
  ...
  describe "run_validation" do
    ...
    it "does not do anything if validation rule is inactive" do
      ...
      rule = create(:business_validation_rule_result)
      rule.business_validation_rule = vr
      rule.state = 'X'
  ...
  describe "after_destroy" do
    let(:business_validation_rule_result) { create(:business_validation_rule_result) }
    ...
    it "does nothing if destroyed with at least one sibling" do
      bvrr_2 = create(:business_validation_rule_result, business_validation_result: business_validation_result)
      bvrr_2.destroy
  ...
  describe "cancel_override" do
    it "sets overridden_at, overridden_by, and note to nil" do
      u = User.new
      r = create(:business_validation_rule_result, overridden_at: Time.zone.now, overridden_by: u, note: "Some message.")
      r.cancel_override

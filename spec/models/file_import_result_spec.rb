describe FileImportResult do
  ...
  describe "collected_messages" do
    before :each do
      @fir = create(:file_import_result)
      @cr1 = create(:change_record, failed: true, record_sequence_number: 1)
      @crm1 = create(:change_record_message, message: "INFO: Hello", change_record: @cr1)
      @crm2 = create(:change_record_message, message: "ERROR: Hello", change_record: @cr1)
    end
  ...
  describe "create_excel_report" do
    before :each do
      @imported_file = create(:imported_file, attached_file_name: "file name")
      @fir = create(:file_import_result, imported_file: @imported_file)
      @user.messages.delete_all

      @cr1 = create(:change_record, failed: true, record_sequence_number: 1, unique_identifier: "foo")
      @cr2 = create(:change_record, failed: false, record_sequence_number: 2, unique_identifier: "bar")
  ...
  describe "download_results" do
    before :each do
      ...
      @cr1 = create(:change_record, failed: true, record_sequence_number: 1)
      @cr2 = create(:change_record, failed: false, record_sequence_number: 2)
      @fir.change_records << @cr1
      @fir.change_records << @cr2
    end
  ...
  it 'should set changed_object_count on save' do
    file_import_result = create(:file_import_result)
    3.times do |i|
      p = create(:product)

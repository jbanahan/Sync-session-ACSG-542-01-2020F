describe MailingList do
  describe 'mailing_lists_for_user' do
    ...
    let! (:mailing_list) { create(:mailing_list, name: 'blah', user: user, email_addresses: 'test@domain.com', company: company) }
    ...
    it "returns hidden mailing lists to sys admins" do
      mailing_list.update! hidden: true
      expect(described_class.mailing_lists_for_user(create(:sys_admin_user, company: company))).to include mailing_list
    end
  end
  ...
  describe "validate_email_addresses" do
    it 'sets non_vfi_addresses to true if a non_vfi_address is present' do
      vfi_user = create(:user)
      mailing_list = create(:mailing_list, name: 'blah', email_addresses: "#{vfi_user.email}, unknown@domain.com")
      mailing_list.reload
    ...
    it 'handles non-vfitrack emails at the beginning of the list' do
      vfi_user = create(:user)
      mailing_list = create(:mailing_list, name: 'blah', email_addresses: vfi_user.email.to_s)
      mailing_list.reload
    ...
    it 'handles duplicate vfitrack email addresses' do
      vfi_user = create(:user)
      mailing_list = create(:mailing_list, name: 'blah', email_addresses: "#{vfi_user.email}, #{vfi_user.email}")
      mailing_list.reload
      expect(mailing_list.non_vfi_addresses).to be_falsey
    end

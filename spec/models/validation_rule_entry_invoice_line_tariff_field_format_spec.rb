describe ValidationRuleEntryInvoiceLineTariffFieldFormat do
  ...
  describe "run_validation" do
    ...
    it "validates tariff level field formats" do
      tariff = FactoryBot(:commercial_invoice_tariff, hts_code: "1234", commercial_invoice_line: FactoryBot(:commercial_invoice_line, line_number: 1, commercial_invoice: FactoryBot(:commercial_invoice, invoice_number: "INV")))
      expect(@rule.run_validation tariff.commercial_invoice_line.entry).to be_nil
    end

    it "reports invalid field formats" do
      tariff = FactoryBot(:commercial_invoice_tariff, hts_code: "ABC", commercial_invoice_line: FactoryBot(:commercial_invoice_line, line_number: 1, commercial_invoice: FactoryBot(:commercial_invoice, invoice_number: "INV")))
    ...
    context "fail_if_matches" do
      ...
      it "validates tariff-level field formats" do
        tariff = FactoryBot(:commercial_invoice_tariff, hts_code: "abcd", commercial_invoice_line: FactoryBot(:commercial_invoice_line, line_number: 1, commercial_invoice: FactoryBot(:commercial_invoice, invoice_number: "INV")))
        expect(rule.run_validation tariff.commercial_invoice_line.entry).to be_nil
      end
    ...
    it "reports only reports one invalid field formats" do
      ...
      tariff = FactoryBot(:commercial_invoice_tariff, hts_code: "ABC", commercial_invoice_line: FactoryBot(:commercial_invoice_line, line_number: 1, commercial_invoice: FactoryBot(:commercial_invoice, invoice_number: "INV")))
      tariff2 = FactoryBot(:commercial_invoice_tariff, hts_code: "ABC", commercial_invoice_line: tariff.commercial_invoice_line)
      tariff3 = FactoryBot(:commercial_invoice_tariff, hts_code: "ABC", commercial_invoice_line: FactoryBot(:commercial_invoice_line, line_number: 2, commercial_invoice: tariff.commercial_invoice_line.commercial_invoice))
      expect(@rule).to receive(:stop_validation).and_call_original
      expect(@rule.run_validation tariff.commercial_invoice_line.entry).to eq "Invoice # INV / Line # 1 #{ModelField.find_by_uid(:cit_hts_code).label} value 'ABC' does not match '\\d{4}' format."
    end

    it "filters at the invoice line level (not tariff level)" do
      tariff = FactoryBot(:commercial_invoice_tariff, hts_code: "ABC")

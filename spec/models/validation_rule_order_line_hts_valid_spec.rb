describe ValidationRuleOrderLineHtsValid do
  let!(:template) { create(:business_validation_template, name: "template name")}
  let!(:rule) do
  ...
  let!(:ordln_1) { create(:order_line, order: order, hts: "123456789", line_number: 1 ) }
  let!(:ordln_2) { create(:order_line, order: order, hts: "246810121", line_number: 2 ) }
  let!(:us) { create(:country, iso_code: "US") }
  let!(:ca) { create(:country, iso_code: "CA") }
  let!(:ot_1) { create(:official_tariff, country: us, hts_code: "123456789") }
  let!(:ot_2) { create(:official_tariff, country: us, hts_code: "246810121") }
  let!(:ot_3) { create(:official_tariff, country: ca, hts_code: "987654321") }

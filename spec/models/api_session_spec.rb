describe ApiSession do
  ...
  let(:api_session) { create(:api_session, last_server_response: nil, class_name: "OpenChain::CustomHandler::SomeGenerator") }
  ...
  context "attachments" do
    let(:req_file) { create(:attachment, attachable: api_session, attachment_type: "request", attached_file_name: "req") }
    let(:resp_file1) { create(:attachment, attachable: api_session, attachment_type: "response", attached_file_name: "resp1") }
    let(:resp_file2) { create(:attachment, attachable: api_session, attachment_type: "response", attached_file_name: "resp2") }
  ...
  describe "purge" do
    ...
    it "removes anything older than given date" do
      session = nil
      Timecop.freeze(Time.zone.now - 1.second) { session = create(:api_session) }
    ...
    it "does not remove items newer than given date" do
      session = nil
      now = Time.zone.now
      Timecop.freeze(now + 1.second) { session = create(:api_session) }

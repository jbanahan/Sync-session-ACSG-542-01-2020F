describe TppHtsOverride do
  ...
  describe '#active' do
    ...
    it 'should only find active override' do
      ...
      old_one = create(:tpp_hts_override, start_date: Date.new(1900, 1, 1), end_date: Date.new(1900, 1, 2), trade_preference_program: @tpp)
      expect(old_one).to_not be_active

      # not active yet, don't find it
      new_one = create(:tpp_hts_override, start_date: Date.new(2999, 1, 1), end_date: Date.new(2999, 1, 2), trade_preference_program: @tpp)
      expect(new_one).to_not be_active

      # active, find it
      find_me = create(:tpp_hts_override, start_date: Date.new(1900, 1, 1), end_date: Date.new(2999, 1, 1), trade_preference_program: @tpp)
    ...
    it 'should take effective date parameters' do
      ...
      dont_find = create(:tpp_hts_override, start_date: Date.new(1900, 1, 1), end_date: Date.new(2100, 1, 1), trade_preference_program: @tpp)

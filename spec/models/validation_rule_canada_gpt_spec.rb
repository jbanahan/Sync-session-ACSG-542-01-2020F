describe ValidationRuleCanadaGpt do
  ...
  describe "run_validation" do
    before :each do
      @t = create(:commercial_invoice_tariff, hts_code: "1234567890", spi_primary: "0", commercial_invoice_line: create(:commercial_invoice_line, country_origin_code: "ZZ"))
      @l = @t.commercial_invoice_line
      @entry = @l.entry
      @canada = create(:country, iso_code: "CA")
    ...
    it "stops after recording a single error on the tariff" do
      @l.update_attributes! country_origin_code: "AF"
      t2 =  create(:commercial_invoice_tariff, hts_code: "1234567890", spi_primary: "0", commercial_invoice_line: @l)
    ...
    it "stops after recording an error on the first line" do
      ...
      @l.update_attributes! country_origin_code: "AF"
      t = create(:commercial_invoice_tariff, hts_code: "1234567890", spi_primary: "0", commercial_invoice_line: create(:commercial_invoice_line, country_origin_code: "AI", commercial_invoice: @l.commercial_invoice))
      expect(described_class.new.run_validation @entry).to eq "Tariff Treatment should be set to 9 when HTS qualifies for special GPT rates.  Country: #{@l.country_origin_code} HTS: #{@t.hts_code}"
    end

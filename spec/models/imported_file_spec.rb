describe ImportedFile do
  describe "result_keys_where" do
    it "builds proper where clause" do
      f = described_class.new id: 25
      query = nil
      ...
      # remove spaces, newlines introduced by heredoc formatting
      expect(query.gsub(/(\n\s+)|(\n)/, " ").strip).to eq "select distinct recordable_id from change_records inner join"\
                                                          " (select id from file_import_results where imported_file_id = #{f.id}"\
                                                          " order by finished_at DESC limit 1) as fir ON change_records.file_import_result_id = fir.id"
  ...
  describe 'make_updated_file' do
    context 'product' do
      ...
      let(:imported_file) { Factory(:imported_file, module_type: "Product", user: Factory(:user), attached_file_name: 'abc.xls') }
      let(:expected_alternate_location) { %r{#{master_setup.uuid}/updated_imported_files/#{imported_file.user_id}/[0-9]{10}\.xls} }

      before do
        expect(attached).to receive(:path).and_return("some/location.xls") # rubocop:disable RSpec/ExpectInHook
        expect(OpenChain::XLClient).to receive(:new).with("some/location.xls").and_return(xlc) # rubocop:disable RSpec/ExpectInHook
        expect(imported_file).to receive(:attached).and_return(attached) # rubocop:disable RSpec/ExpectInHook
        success_hash = {"result" => "success"}
        expect(xlc).to receive(:save).with(expected_alternate_location).and_return(success_hash) # rubocop:disable RSpec/ExpectInHook
      end

      it 'saves the result file' do
      ...
      it 'updates header level products' do
        ...
        expect(xlc).to receive(:get_row).with(0, 0).and_return([{"position" => {"column" => 0}, "cell" => {"value" => "oldname1", "datatype" => "string"}},
                                                                {"position" => {"column" => 1}, "cell" => {"value" => " #{p1.unique_identifier} ", "datatype" => "string"}}])
        expect(xlc).to receive(:get_row).with(0, 1).and_return([{"position" => {"column" => 0}, "cell" => {"value" => "oldname2", "datatype" => "string"}},
                                                                {"position" => {"column" => 1}, "cell" => {"value" => p2.unique_identifier, "datatype" => "string"}}])
        expect(xlc).to receive(:get_row).with(0, 2).and_return([{"position" => {"column" => 0}, "cell" => {"value" => "oldname3", "datatype" => "string"}},
                                                                {"position" => {"column" => 1}, "cell" => {"value" => p3.unique_identifier, "datatype" => "string"}}])
      ...
      it 'does not clear fields when product missing' do
        missing_value = "missing val"
        ["prod_name", "prod_uid"].each_with_index {|v, i| imported_file.search_columns.create!(model_field_uid: v, rank: i)}
        expect(xlc).to receive(:last_row_number).and_return(0)
        expect(xlc).to receive(:get_row).with(0, 0).and_return([{"position" => {"column" => 0}, "cell" => {"value" => "oldname1", "datatype" => "string"}},
                                                                {"position" => {"column" => 1}, "cell" => {"value" => missing_value, "datatype" => "string"}}])
  ...
  describe "sanitize callback" do
    it "sanitizes the attached filename" do
      i = described_class.new module_type: "Product", starting_row: 1, starting_column: 1, update_mode: "any"
  ...
  describe "process" do
    ...
    let(:file) { Factory(:imported_file, search_setup: Factory(:search_setup)) }

    it "processes an imported file" do

      file.search_setup.search_columns.create! model_field_uid: :unique_identifier
      ...
      expect(file.search_columns.size).to eq 1
      expect(file.search_columns.first.model_field_uid).to eq "unique_identifier"
    end

    it "processes an imported file on delayed job queue" do

      expect_any_instance_of(ImportedFile::FileImportProcessJob).to receive(:enqueue_job)
      r = file.process user, defer: true
      expect(r).to be_truthy
    end

    it "returns false if errors were found while processing" do
      expect_any_instance_of(ImportedFile::FileImportProcessJob).to receive(:perform) do
        file.errors[:base] << "Error"
        nil
      end
  ...
  describe "FileImportProcessJob#perform" do
    ...
    let(:user) { Factory(:user) }
    let(:file) { Factory(:imported_file, search_setup: Factory(:search_setup), attached_file_name: "test.txt") }
    ...
    it "calls process on FileImportProcessor" do
      ...
      ImportedFile::FileImportProcessJob.new(file, user).perform
    end

    it "requeues if another job with the same file is running" do
      # Need to create two here to get real conditions, current job + another running same file
      Delayed::Job.enqueue ImportedFile::FileImportProcessJob.new(file, user)
      Delayed::Job.enqueue ImportedFile::FileImportProcessJob.new(file, user)
    ...
    it "emails if errors were encountered" do
      ...
      mail = double("mail") # rubocop:disable RSpec/VerifiedDoubles
      expect(mail).to receive :deliver_now
      expect(OpenMailer).to receive(:send_imported_file_process_fail).with(file, file.search_setup.user).and_return mail
      ImportedFile::FileImportProcessJob.new(file, user).perform

      expect(ErrorLogEntry.last.additional_messages).to eq ["Imported File ID: #{file.id}"]
    end
  end

  describe "process_integration_imported_file" do
    let(:temp) do
      tmp = Tempfile.new ['temp', '.csv']
      Attachment.add_original_filename_method tmp
      tmp.original_filename = 'file.csv'
      tmp
    end
    ...
    it "downloads file from s3 and imports it" do
      ...
      imp_file = described_class.first
  ...
  describe ImportedFile::FileImportProcessorListener do
    describe "process_row" do
      ...
      let(:messages) do
        mf_value = "mf value"
        def mf_value.unique_identifier?;
          true
        end

describe DataCrossReference do
  ...
  context "find_ascena_brand" do
    before do
      DataCrossReference.create!(key: "justice", value: "JST", cross_reference_type: DataCrossReference::ASCE_BRAND)
    end

    it "should find an ascena brand from the department" do
      expect(DataCrossReference.find_ascena_brand('justice')).to eq('JST')
    end
  ...
  describe "has_key?" do
    it "determines if an xref key is present in the db table" do
      ...
      expect(DataCrossReference.key? 'x', DataCrossReference::UA_PLANT_TO_ISO).to be_truthy
      expect(DataCrossReference.key? 'askjfda', DataCrossReference::UA_PLANT_TO_ISO).to be_falsey
      expect(DataCrossReference.key? nil, DataCrossReference::UA_PLANT_TO_ISO).to be_falsey
    end
  end
  ...
  context "HM Pars" do
    ...
    describe "find_and_mark_next_unused_hm_pars_number" do
      it "finds the next cross reference and marks it as being used" do
        ...
        expect(DataCrossReference.find_and_mark_next_unused_hm_pars_number).to eq unused_data_cross_reference
      ...
      it "uses alphabetic sorting to determine next pars number to use" do
        unused_data_cross_reference
        new_unused = DataCrossReference.add_hm_pars_number("A Pars-Unused")
        expect(DataCrossReference.find_and_mark_next_unused_hm_pars_number).to eq new_unused
      end
    end

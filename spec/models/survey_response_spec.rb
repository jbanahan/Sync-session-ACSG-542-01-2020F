describe SurveyResponse do
  describe "last_logged_by_user" do
    it "finds most recently saved message created_at associated with given user" do
      ...
      srl.create!(message: 'newer different user', user_id: create(:user).id)
      sr.reload
  ...
  describe "rated?" do
    it "returns true if there is a master rating" do
      expect(create(:survey_response, rating: 'abc')).to be_rated
    end

    it "returns false if there is no master rating or answers with ratings" do
      expect(create(:survey_response, rating: nil)).not_to be_rated
    end

    it "returns true if any answers have ratings" do
      expect(create(:answer, rating: 'abc').survey_response).to be_rated
    end
  end

  it "requires survey" do
    expect(described_class.new(user: create(:user)).save).to be_falsey
  end

  it "requires user" do
    expect(described_class.new(survey: create(:survey)).save).to be_falsey
  end
  ...
  describe "can_view?" do
    ...
    it "passes if user can view_survey? and survey is created by user's company" do
      other_user = create(:user, company: survey.company, survey_view: true)
  ...
  describe "search_secure" do
    it "finds assigned to me, even if I cannot view_survey" do
      u = create(:user, survey_view: false)
    ...
    it "finds where survey is created by my company and I can view" do
      u = create(:user, survey_view: true)
    ...
    it "does not find where survey is created by my company and I canNOT view surveys" do
      u = create(:user, survey_view: false)
      create(:survey_response, survey: create(:survey, company: u.company))
  ...
  describe "was_archived" do
    let(:survey) { create(:question).survey }
    let(:user) { create(:user) }
  ...
  describe "assigned_to_user?" do
    ...
    it "does not show as assigned if user is not response user" do
      response = survey.generate_response! user
      expect(response.assigned_to_user?(create(:user))).to be_falsey
    end
  ...
  describe "clear_expired_checkouts" do
    it "clears checkout information and logs expiration" do
      ...
      Timecop.freeze(now) do
        user = create(:user)
        sr = create(:survey_response, checkout_token: "token", checkout_by_user: user, checkout_expiration: now - 1.day)
        sr2 = create(:survey_response, checkout_token: "token", checkout_by_user: user, checkout_expiration: now - 1.day + 2.seconds)
  ...
  describe '#not_expired' do
    it "finds unexpired survey responses" do
      u = create(:user)
      s = create(:survey, expiration_days: 5)
      ...
      no_expiration = create(:survey).generate_response! u

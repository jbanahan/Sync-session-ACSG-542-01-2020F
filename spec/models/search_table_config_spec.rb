describe SearchTableConfig do
  describe '#for_user' do
    it 'should find generic' do
      u = create(:user)
      generic = create(:search_table_config, page_uid:'abc')
      for_me = create(:search_table_config, page_uid:'abc', user:u)
      for_company = create(:search_table_config, page_uid:'abc', company:u.company)

      # don't find these
      create(:search_table_config, page_uid:'other')
      create(:search_table_config, page_uid:'abc', user:create(:user))
      create(:search_table_config, page_uid:'abc', company:create(:company))

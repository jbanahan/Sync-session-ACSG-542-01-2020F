describe Shipment do
  ...
  describe "search_secure" do
    let!(:master_only) { Factory(:shipment) }
  ...
  describe "uncancel_shipment!" do
    it "removes cancellation" do
      u = Factory(:user)
      s = Factory(:shipment, canceled_by: u, canceled_date: Time.zone.now, cancel_requested_at: Time.zone.now, cancel_requested_by: u)
  ...
  describe "revise booking" do
    it "removes received, requested, approved and confirmed date and 'by' fields" do
      ...
      s = Factory(:shipment, booking_approved_by: u, booking_requested_by: u, booking_confirmed_by: u,
  ...
  describe "available_orders" do
    ...
    context "with_data" do
      let(:imp) { Factory(:company, importer: true) }
      let(:vendor_1) { Factory(:company, vendor: true) }
      let(:vendor_2) { Factory(:company, vendor: true) }
      let!(:order_1) { Factory(:order, importer: imp, vendor: vendor_1, approval_status: 'Accepted') }
      let!(:order_2) { Factory(:order, importer: imp, vendor: vendor_2, approval_status: 'Accepted') }
      let(:s) { described_class.new(importer_id: imp.id) }

      before do
        Factory(:order, importer: Factory(:company, importer: true), vendor: vendor_1, approval_status: 'Accepted')
      end
  ...
  describe "available_products" do
    let(:imp) { Factory(:importer) }
    let(:shipment) { Factory(:shipment, importer: imp) }

    before do
      Factory(:product, importer: imp)
      Factory(:product)
    end

describe ValidationRuleEntryInvoiceValueMatchesDaPercent do
  ...
  before :each do
    ...
    @invoice = FactoryBot(:commercial_invoice, invoice_number: "123B", invoice_value_foreign: 100)
    @invoice_line = FactoryBot(:commercial_invoice_line, commercial_invoice: @invoice, adjustments_amount: 4.5)
    FactoryBot(:commercial_invoice_line, commercial_invoice: @invoice, adjustments_amount: 4.5)
  end
  ...
  it "fails if the sum of the invoice line's DA amount isn't the target percentage of the invoice's value" do
    FactoryBot(:commercial_invoice_line, commercial_invoice: @invoice, adjustments_amount: 0.4)

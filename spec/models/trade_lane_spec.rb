  ...
  context 'security' do
    describe '#can_view?' do
      it "should allow user who can view trade_lane" do
        u = User.new
        u.stub(:view_trade_lanes?).and_return true
        expect(TradeLane.new.can_view?(u)).to be_true
      end
      it "should not allow user who cannot view trade lanes" do
        u = User.new
        u.stub(:view_trade_lanes?).and_return false
        expect(TradeLane.new.can_view?(u)).to be_false
      end
    end
    describe '#can_edit?' do
      it "should allow user who can edit trade_lane" do
        u = User.new
        u.stub(:edit_trade_lanes?).and_return true
        expect(TradeLane.new.can_edit?(u)).to be_true
      end
      it "should not allow user who cannot edit trade lanes" do
        u = User.new
        u.stub(:edit_trade_lanes?).and_return false
        expect(TradeLane.new.can_edit?(u)).to be_false
      end
    end

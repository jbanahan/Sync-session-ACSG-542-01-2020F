RSpec.describe Calendar, :type => :model do
  ...
  describe "find_all_events_in_calendar_month" do
    ...
    it "should return all holidays in a given month and country calendar" do
      c1 = Factory(:calendar, calendar_type: us.key, year: 2019)
      hol1 = Factory(:calendar_event, event_date: Date.parse('2019-05-16'), calendar_id: c1.id)
      hol2 = Factory(:calendar_event, event_date: Date.parse('2019-05-17'), calendar_id: c1.id)
      hol3 = Factory(:calendar_event, event_date: Date.parse('2019-05-18'), calendar_id: c1.id)
    ...
    it "should find calendar events relavent to a given company and not nil" do
      comp = Factory(:company, name: "ACME")
      c1 = Factory(:calendar, calendar_type: k84.key, company_id: comp.id, year: 2019)
      event = Factory(:calendar_event, event_date: Date.parse('2019-05-18'), calendar_id: c1.id)

describe SummaryStatement do
  ...
  describe "total" do
    it "returns the sum of the invoice totals" do
      @ss.broker_invoices = [FactoryBot(:broker_invoice, invoice_total: 10), FactoryBot(:broker_invoice, invoice_total: 5)]
      @ss.save!
  ...
  describe "remove!" do
    before(:each) { @ss.broker_invoices << FactoryBot(:broker_invoice) }
    ...
    it "raises an error if the invoice doesn't belong to the statement assigned" do
      unassigned_bi_id = FactoryBot(:broker_invoice, invoice_number: '123456789').id
  ...
  describe "add!" do
    ...
    it "raises an error if the invoice company is neither the summary statement customer nor a linked company" do
      bi = FactoryBot(:broker_invoice, invoice_number: '123456789', entry: FactoryBot(:entry, importer: FactoryBot(:company)))
    ...
    it "raises an error if the invoice is already assigned" do
      bi = FactoryBot(:broker_invoice, invoice_number: '123456789')
      @ss.broker_invoices << bi
      ss_2 = FactoryBot(:summary_statement)

describe OpenChain::Report::KewillCurrencyRatesReport do
  ...
  describe "process_results" do
    ...
    it "handles all post-query processing on data returned from sql_proxy server" do
      ...
      expect(@tf.original_filename).to eq "Currency Rates On #{settings['start_date']}.xls"
      wb = Spreadsheet.open @tf.path
      sheet = wb.worksheets.find {|s| s.name == "Currency Rates"}
      expect(sheet).not_to be_nil
      expect(sheet.row(0)).to eq ["Country", "Name", "Currency", "Exchange Date", "Exchange Rate", "Currency Type", "Remarks"]
      expect(sheet.row(1)).to eq ["CN", "China", "CNY", excel_date(Date.new(2016, 4, 1)), BigDecimal("0.123456"), "Daily", "Notes"]
    end
  end
end
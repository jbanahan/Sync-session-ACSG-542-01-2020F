describe OpenChain::Report::EntryContainerCostBreakdown do
  ...
  context "with entry data" do
    before :each do
      @tariff = create(:commercial_invoice_tariff, duty_amount: 10, commercial_invoice_line: create(:commercial_invoice_line, value: 20, hmf: 30, mpf: 40))
      @line = @tariff.commercial_invoice_line
      @tariff2 = create(:commercial_invoice_tariff, duty_amount: 15, commercial_invoice_line: @line)
      @entry = @line.entry
      @entry.update_attributes! customer_number: "CQ", release_date: "2015-06-01", master_bills_of_lading: "MBOL", entry_number: "EN12345"

      @tariff3 = create(:commercial_invoice_tariff, duty_amount: 10,
                          commercial_invoice_line: create(:commercial_invoice_line, value: 10, hmf: 20, mpf: 30,
                                                            commercial_invoice: create(:commercial_invoice, entry: @entry)))
      @line2 = @tariff3.commercial_invoice_line

      @container1 = create(:container, entry: @entry, commercial_invoice_lines: [@line])
      @container2 = create(:container, entry: @entry, commercial_invoice_lines: [@line2])

      @broker_invoice_line = create(:broker_invoice_line, broker_invoice: create(:broker_invoice, entry: @entry, invoice_total: 50), charge_amount: 50)
      @broker_invoice = @broker_invoice_line.broker_invoice

      # Create a freight charge
      @broker_invoice_line_freight = create(:broker_invoice_line, broker_invoice: create(:broker_invoice, entry: @entry), charge_amount: 50, charge_code: '0600')
      @broker_invoice_freight = @broker_invoice_line_freight.broker_invoice

      @user = create(:master_user, time_zone: "UTC", entry_view: true)
    ...
    describe "run" do
      ...
      it "excludes entries user does not have access to" do
        ...
        wb = subject.run create(:user, time_zone: "UTC"), {'start_date' => '2015-06-01', 'end_date' => '2015-06-02', 'customer_number' => "CQ"}
        wb = Spreadsheet.open(wb.path)
        sheet = wb.worksheet "06-01-15 - 06-01-15"
        expect(sheet.row(1)).to eq []
      end

describe OpenChain::Report::EddieBauerStatementSummary do
  before :each do
    @user = create(:user)
    @imp = with_customs_management_id(create(:company), "EDDIE")
    @monthly_date = 0.seconds.ago
    @daily_date = 1.day.ago
    @ent = create(:entry, :importer=>@imp,
    ...
    @line = create(:commercial_invoice_line,
      :commercial_invoice=>create(:commercial_invoice, :entry=>@ent, :invoice_number=>@invoice_number),
  ...
  context "default mode (not_paid)" do
    it "should return entries not paid on PMS but on daily statement" do
      dont_find_because_paid = create(:entry, :importer=>@imp,
        :daily_statement_number=>'123456', :monthly_statement_paid_date=>Time.now)
      dont_find_because_not_on_daily = create(:entry, :importer=>@imp)
      dont_find_because_not_eddie = create(:entry, :daily_statement_number=>'12345')
  ...
  context "previous_month mode" do
    it "should return entries from month / year specified regardless of paid status" do
      find_even_though_paid = create(:entry, :importer=>@imp, :monthly_statement_paid_date=>Time.now, :release_date=>((Time.zone.now - 3.months) + 1.minute).in_time_zone("America/New_York"))
      @ent.update_attributes!(:release_date=>(Time.zone.now - 3.months).in_time_zone("America/New_York"))
      dont_find_even_though_unpaid_because_different_month = create(:entry, :importer=>@imp, :release_date=>1.hour.from_now.in_time_zone("America/New_York"))
  ...
  it "should create summary tab" do
    ...
    l2 = create(:commercial_invoice_line, :commercial_invoice=>create(:commercial_invoice, :entry=>e2, :invoice_number=>'123'),
    ...
    l3 = create(:commercial_invoice_line, :commercial_invoice=>create(:commercial_invoice, :entry=>e2, :invoice_number=>'123'),

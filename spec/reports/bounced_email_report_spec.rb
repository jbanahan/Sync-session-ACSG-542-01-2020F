  ...
  describe "get_bounced_emails_for_dates" do
    describe 'beginning date' do
      it 'retrieves emails after the beginning date' do
        Timecop.freeze(Time.zone.now) do
          beginning, ending = subject.get_yesterday_in_timezone("America/New_York")
          zero_hour_email = create(:sent_email, email_date: beginning, delivery_error: 'it dun goofed')
          after_beginning_email = create(:sent_email, email_date: beginning + 1.hour, delivery_error: 'really goofed')
    ...
    describe 'ending date' do
      it 'retrieves emails before the ending date' do
        beginning, ending = subject.get_yesterday_in_timezone("America/New_York")
        zero_hour_email = create(:sent_email, email_date: ending, delivery_error: 'it dun goofed')
        after_beginning_email = create(:sent_email, email_date: ending - 1.hour, delivery_error: 'really goofed')
    ...
    it 'does not include emails before the beginning or after the ending date' do
      beginning, ending = subject.get_yesterday_in_timezone("America/New_York")
      before_beginning = create(:sent_email, email_date: beginning - 1.hour, delivery_error: 'it dun goofed')
      after_ending_email = create(:sent_email, email_date: ending + 1.hour, delivery_error: 'really goofed')
    ...
    it 'does not include emails without errors' do
      beginning, ending = subject.get_yesterday_in_timezone("America/New_York")
      bad_email = create(:sent_email, email_date: ending, delivery_error: 'it dun goofed')
      good_email = create(:sent_email, email_date: ending - 1.hour)

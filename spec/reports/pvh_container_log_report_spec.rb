describe OpenChain::Report::PvhContainerLogReport do
  ...
  describe "run_report" do
    before :each do
      ...
      @port = create(:port)
      @container1 = create(:container, container_number: "CONT1", quantity: 5, entry: create(:entry, broker_reference: "123", customer_number: "PVH", arrival_date: Time.zone.now, worksheet_date: Time.zone.parse("2014-10-01 00:00"), entry_port_code: @port.schedule_d_code, store_names: "A\n B", customer_references: "A123 10/1/14\n1234\nB123"))
      @entry = @container1.entry
      @container2 = create(:container, container_number: "CONT1", quantity: 10, entry: @entry)
    end
  ...
  describe "permission?" do
    context "as www-vfitrack-net system code" do
      ...
      context "with pvh company" do
        ...
        let! (:pvh) { create(:company, importer: true, system_code: "PVH") }

describe MonthlyStatementsController do
  ...
  describe "show" do
    ...
    it "redirects to error if user can't view this statement" do
      expect_any_instance_of(User).to receive(:view_statements?).and_return true
      expect(MonthlyStatement).to receive(:find).with(statement.id.to_s).and_return statement
  ...
  describe "reload_statement" do
    ...
    it "reloads a statement from kewill customs" do
      expect_any_instance_of(MonthlyStatement).to receive(:can_view?).with(user).and_return true
      ...
      expect(OpenChain::CustomHandler::Vandegrift::KewillStatementRequester).to receive(:request_monthly_statements).with ["STATEMENT"]
    ...
    it "doesn't allow users who can't view to reload" do
      expect_any_instance_of(MonthlyStatement).to receive(:can_view?).and_return false

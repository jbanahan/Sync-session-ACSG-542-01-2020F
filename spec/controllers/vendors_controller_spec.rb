describe VendorsController do
  ...
  describe "matching_vendors" do
    ...
    it "should return matches based on first 3 characters" do
      c1 = create(:company, vendor:true, name:'abcxxx')
      create(:company, vendor:false, name:'abc') # don't match non-vendors
      create(:company, vendor:true, name:'xxx') # don't match non-3 letter name matches
      allow_any_instance_of(User).to receive(:view_vendors?).and_return true
  ...
  describe "orders" do
    ...
    it "should search_secure orders" do
      allow_any_instance_of(Company).to receive(:can_view_as_vendor?).and_return true
      @u.company.update_attributes(vendor:true)
      o = create(:order, vendor_id:@u.company.id)
      create(:order) # don't find this one
      get :orders, id: @u.company.id.to_s
      expect(response).to be_success
  ...
  describe "survey_responses" do
    ...
    it "should search_secure surveys" do
      ...
      sr = create(:survey_response, survey:create(:survey, company:@u.company), base_object:c)
      sr2 = create(:survey_response, user:@u, base_object:c)
      create(:survey_response, base_object:c) # don't find this one
      get :survey_responses, id: @u.company_id.to_s
      expect(response).to be_success
  ...
  describe "products" do
    ...
    it "should search_secure products" do
      ...
      p = create(:product)
      p.vendors << @u.company
      create(:product) # don't find this one
      get :products, id: @u.company_id.to_s
      expect(response).to be_success

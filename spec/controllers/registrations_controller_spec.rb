  ...
  describe "send_email" do
    ...
    context "with valid recaptcha", :disable_delayed_jobs do
      ...
      it "emails Vandegrift with the registration form data and the server's system_code" do
        ...
        expect(OpenMailer).to receive(:send_registration_request).with("john_doe@acme.com", "John", "Doe", "Acme", "Jane Smith",
                                                                       "123456789", master_setup.system_code).and_return message_delivery
      ...
      it "validates presence of email, first name, last name, company, contact" do
        [:email, :fname, :lname, :company].each {|k| params[k] = "" }
        params[:contact] = nil

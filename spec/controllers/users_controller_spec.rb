describe UsersController do
  ...
  describe "create" do
    it "creates user with apostrophe in email address" do
  ...
  describe "create_from_template" do
    ...
    it "creates user based on template" do
      ...
      expect(OpenChain::UserSupport::CreateUserFromTemplate)
        .to receive(:build_user).with(t, c, 'Joe', 'Smith', 'jsmith@sample.com', 'jsmith@sample.com',
                                      'Eastern Time (US & Canada)').and_call_original
  ...
  describe "new" do
    context "with admin authorization" do
      ...
      it "makes search setups belonging to a copied user available to the view" do
        ...
        expect(assigns(:available_search_setups)).to eq([ss_assigned_1, ss_assigned_2]
                                                            .sort {|ss1, ss2| ss1.name <=> ss2.name })
      ...
      it "makes custom reports belonging to a copied user available to the view" do
        ...
        expect(assigns(:available_custom_reports)).to eq([cr_assigned_1, cr_assigned_2]
                                                             .sort {|cr1, cr2| cr1.name <=> cr2.name })
      end
      ...
      it "makes group memberships belonging to a copied user available to the view" do
        ...
        expect(assigns(:available_groups)).to eq([gr_available_1, gr_available_2, gr_available_3]
                                                     .sort { |gr1, gr2| gr1.name <=> gr2.name })
      end

      it "makes copied-user permissions available to view" do
        copied_user.update(drawback_view: true)
        get :new, company_id: copied_user.company.id, copy: copied_user.id
  ...
  describe 'show_bulk_upload' do
    it "shows bulk upload for admin" do
      user.admin = true
      user.save!
    ...
    it "does not show bulk upload for non-admin" do
      get :show_bulk_upload, 'company_id' => user.company_id.to_s
      expect(response).to be_redirect
      expect(flash[:errors].size).to eq(1)
    end
  ...
  describe 'bulk_upload' do
    it "creates users from csv" do
      ...
      expect(u.encrypted_password).not_to be_nil
    ...
    it "shows errors for some users while creating others" do
      allow_any_instance_of(User).to receive(:admin?).and_return(true)
  ...
  describe 'preview_bulk_upload' do
    ...
    it "returns 400 with error if not valid csv" do
      data = "uname,\""
      post :preview_bulk_upload, 'company_id' => user.company_id.to_s, 'bulk_user_csv' => data
      expect(response.status).to eq(400)
    end

    it "returns 400 with error if not right number of elements" do
      data = "uname"
      post :preview_bulk_upload, 'company_id' => user.company_id.to_s, 'bulk_user_csv' => data
      expect(response.status).to eq(400)
    end
  ...
  describe "bulk_invite" do
    ...
    it "onlies allow admins access" do
      user.admin = false
      user.save
    ...
    it "verifies at least one user selected" do
      user.admin = true
      user.save
  ...
  describe "set_homepage" do
    ...
    it "sets unsets the users homepage" do
      user.update! homepage: "/index.html"
  ...
  describe "move_to_new_company" do
    ...
    it "moves users to the correct company" do
      user.admin = true
      user.save
  ...
  describe "unlock_user" do
    ...
    it "errors when the user is not found" do
      user.admin = true
      user.save
    ...
    it "errors when the current user is not an admin" do
      user.admin = false
      user.save
    ...
    it "unlocks the user when the current user is an admin" do
      user.admin = true
      user.save
  ...
  describe "enable_run_as" do
    ...
    it "enables run as functionality for a non-disabled user when current user is an admin" do
      user.admin = true
      user.save
    ...
    it "errors when the run as user is not found" do
      user.admin = true
      user.save
    ...
    it "errors when run as user has been disabled" do
      user.admin = true
      user.save

      Factory(:user, username: "AugustusGloop", disabled: true, password_locked: false, password_expired: false,
    ...
    it "errors when run as user's password has been locked" do
      user.admin = true
      user.save

      Factory(:user, username: "AugustusGloop", disabled: false, password_locked: true, password_expired: false, password_reset: false)
    ...
    it "errors when run as user's password has expired" do
      user.admin = true
      user.save

      Factory(:user, username: "AugustusGloop", disabled: false, password_locked: false, password_expired: true,
                     password_reset: false)
    ...
    it "errors when run as user's password is in reset status" do
      user.admin = true
      user.save

      Factory(:user, username: "AugustusGloop", disabled: false, password_locked: false, password_expired: false,
                     password_reset: true)

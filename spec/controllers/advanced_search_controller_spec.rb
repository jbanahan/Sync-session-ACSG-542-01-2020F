describe AdvancedSearchController do
  ...
  describe "destroy" do
    it "should destroy and return id of previous search" do
      old_search = create(:search_setup, :module_type=>'Product', :user=>@user)
      new_search = create(:search_setup, :module_type=>'Product', :user=>@user)
  ...
  describe "update" do
    before :each do
      @ss = create(:search_setup, :name=>"X", :user=>@user, :include_links=>true, :include_rule_links=>true, :no_time=>false,
        :module_type=>"Product")
    end
  ...
  describe "last_search_id" do
    it "should get last search based on search_run" do
      ss_first = create(:search_setup, :user=>@user)
      ss_second = create(:search_setup, :user=>@user)
      ss_first.touch
      get :last_search_id
    ...
    it "should return last search updated if no search_runs" do
      ss_first = create(:search_setup, :user=>@user, updated_at:1.day.ago)
      ss_second = create(:search_setup, :user=>@user, updated_at:1.year.ago)
      get :last_search_id
      expect(JSON.parse(response.body)['id']).to eq(ss_first.id.to_s)
    end
  ...
  describe "setup" do
    ...
    it "should write response for json" do
      @ss = create(:search_setup, :user=>@user, :name=>'MYNAME',
    ...
    it "indicates if the search belongs to Integration User" do
      ...
      @ss = create(:search_setup, user: integration, module_type: "Product", name: "MYNAME")
      get :setup, :id=>@ss.id, :format=>'json'
      expect(response).to be_success
      h = JSON.parse response.body
    ...
    it "should return full search list for current module_type" do
      ss = create(:search_setup, :user=>@user, :name=>"B", :module_type=>"Order")
      ss2 = create(:search_setup, :user=>@user, :name=>"A", :module_type=>"Order")
      get :setup, :id=>ss.id, :format=>:json
      h = JSON.parse response.body
      list = h['search_list']
    ...
    it "should not return search list for different module_types" do
      ss = create(:search_setup, :user=>@user, :name=>"B", :module_type=>"Order")
      ss2 = create(:search_setup, :user=>@user, :name=>"A", :module_type=>"Order")
      dont_find = create(:search_setup, :user=>@user, :module_type=>"Product")
      get :setup, :id=>ss.id, :format=>:json
      h = JSON.parse response.body
      list = h['search_list']
      expect(list.size).to eq(2)
    ...
    it "should not show model fields that user cannot view" do
      ...
      ss = create(:search_setup, :user=>@user, :module_type=>'Entry')
      get :setup, :id=>ss.id, :format=>:json
      h = JSON.parse response.body
    ...
    it "should default date format when none is provided in the search setup" do
      @ss = create(:search_setup, user: @user, name: 'MYNAME', module_type: "Product")
      get :setup, id: @ss.id, format: 'json'
      expect(response).to be_success
      h = JSON.parse response.body
  ...
  context "audit" do
    let(:ss) { create(:search_setup, user: @user, name: "search name") }
    ...
    describe "show_audit" do
      it "renders if called with search setup that belongs to user" do
        ...
        ra3 = RandomAudit.create! user: create(:user), search_setup: ss, report_date: Date.new(2018, 3, 17)
      ...
      it "redirects otherwise" do
        ss.update_attributes user: create(:user)
        get :show_audit, id: ss.id

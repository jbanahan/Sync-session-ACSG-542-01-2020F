describe Api::V1::UsersController do
  ...
  describe "login" do
    ...
    it "validates a user's login credentials" do
      u = create(:user, username: "user", api_auth_token: "auth_token")
  ...
  describe "google_oauth2", :without_partial_double_verification do
    let!(:user) { create(:user, username: "user", api_auth_token: "auth_token", email: "me@gmail.com") }
    ...
    it "uses an omniauth request to validate a user's access token" do
      ...
      expect(response).to be_success
      expect(JSON.parse(response.body)).to eq({"id" => user.id, "username" => "user", "token" => "user:auth_token", "full_name" => user.full_name})
    end
  ...
  describe "enabled_users" do
    it "returns enabled users belonging to current_user's visible companies" do
      linked_co = create(:company, name: 'Konvenientz')
      co = create(:company, name: 'Acme', linked_companies: [linked_co])
      u = create(:user, company: co, first_name: 'Nigel', last_name: 'Tufnel', username: 'ntufnel', disabled: false)
      u2 = create(:user, company: linked_co, first_name: 'David', last_name: 'St. Hubbins', username: 'dsthubbins', disabled: nil)
      u3 = create(:user, company: linked_co, first_name: 'AAA', last_name: 'ZZZZ', username: 'AAA ZZZZ', disabled: nil)
      create(:user, company: linked_co, first_name: 'Derek', last_name: 'Smalls', username: 'dsmalls', disabled: true)
      allow_api_access u

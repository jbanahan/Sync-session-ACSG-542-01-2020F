describe Api::V1::CompaniesController do
  describe "index" do
    before :each do
      @c = create(:company, name:'c1', importer:true, system_code:'A')
      @u = create(:user, company:@c)
      allow_api_access @u
    end
    it "should only return self and linked companies" do
      c2 = create(:company, name:'c2', importer:true, system_code:'B')
      c3 = create(:company, name:'a3', vendor:true, system_code:'C')
      create(:company, name:'bad', system_code:'BAD') # don't find this
    ...
    it "only returns companies linked with company in linked_with param" do
      c2 = create(:company, name:'c2', vendor:true, system_code:'B')
      c3 = create(:company, name:'a3', vendor:true, system_code:'C')
      [c2, c3].each {|c| @c.linked_companies << c}

      master_company = create(:importer, name: "Master")
    ...
    it "should find all companies for master" do
      @c.master = true
      @c.save!
      create(:company, name:'c2', importer:true, system_code:'c2')
      get :index
      expect(response).to be_success
      j = JSON.parse(response.body)['companies']
    ...
    context "role" do
      it "should return self if matches role" do
        c2 = create(:company, name:'c2', importer:true, system_code:'c2')
        @c.linked_companies << c2
      ...
      it "should return multiple roles" do
        c2 = create(:company, name:'c2', importer:true, vendor:true, system_code:'c2')
        @c.linked_companies << c2

describe Api::V1::SurveyResponsesController do
  ...
  describe "index" do
    it "returns a list of surveys accessible to the user" do
      s = create(:survey_response, subtitle: "sub")
    ...
    it "returns list of surveys assign to a user's group" do
      ...
      user = create(:user, survey_view: true)
    ...
    it "does not return archived survey repsonses" do
      s = create(:survey_response, subtitle: "sub", archived: true)
      allow_api_access s.user
      s.user.update! survey_view: true
    ...
    it "includes checkout information" do
      user = create(:user, survey_view: true)
      s = create(:survey_response, subtitle: "sub", user: user, checkout_by_user: user, checkout_token: "token", checkout_expiration: Time.zone.now + 1.day)
    ...
    it "excludes checkout token if checked out by another user" do
      user = create(:user, survey_view: true)
      s = create(:survey_response, subtitle: "sub", user: user, checkout_by_user: create(:user), checkout_token: "token", checkout_expiration: Time.zone.now + 1.day)
  ...
  describe "show" do
    let(:user) { create(:user, survey_view: true) }
  ...
  describe "checkout" do
    ...
    it "fails if survey is already checked out" do
      survey_response.checkout_by_user = create(:user)
      survey_response.save!
  ...
  describe "cancel_checkout" do
    let(:user) { create(:user, survey_view: true) }
    let(:survey_response) { create(:survey_response, user: user, checkout_by_user: user, checkout_token: "token", checkout_expiration: Time.zone.now) }
  ...
  describe "checkin" do
    ...
    let!(:question) { create(:question, survey: survey, content: "Is this a test?", choices: "Yes\nNo") }
    let!(:question2) { create(:question, survey: survey, content: "Is this a second question?", choices: "Y\nN") }
  ...
  describe "submit" do
    ...
    context "without locked questions" do
      ...
      it "validates questions marked as requiring comments have them, ignoring comments by non-survey takers" do
        ...
        question.save!
        answer.update! choice: "Yes"
        answer.answer_comments.create! content: "Comment", user: create(:user)
        post :submit, {id: survey_response.id}
        expect(response.status).to eq 403
        expect(JSON.parse(response.body)).to eq({'errors' => ["Question #1 is a required question. You must provide a comment."]})
      end
      ...
      it "validates questions marked as requiring attachments have them, ignoring attachments by non-survey takers" do
        ...
        question.save!
        answer.update! choice: "Yes"
        answer.attachments.create! attached_file_name: "file.text", uploaded_by: create(:user)
        post :submit, {id: survey_response.id}
        expect(response.status).to eq 403
        expect(JSON.parse(response.body)).to eq({'errors' => ["Question #1 is a required question. You must provide an attachment."]})
      end

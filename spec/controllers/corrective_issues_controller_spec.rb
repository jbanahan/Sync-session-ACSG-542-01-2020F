describe CorrectiveIssuesController do
  ...
  describe "update" do
    it "should allow editor to update description and suggested action" do
      ...
      put :update, params: { :id => ci.id.to_s, 'corrective_issue' => {'id' => ci.id.to_s, 'description' => 'nd', 'suggested_action' => 'ns', 'action_taken' => 'na'} }, :format => 'json'
      ci.reload
    ...
    it "should update extra fields if current_user can edit" do
      ...
      put :update, params: { :id => ci.id.to_s, 'corrective_issue' => {'id' => ci.id.to_s, 'description' => 'nd', 'suggested_action' => 'nsa', 'action_taken' => 'nat'} }, :format => 'json'
  ...
  describe 'create' do
    it "should allow create if user can edit plan" do
      allow_any_instance_of(CorrectiveActionPlan).to receive(:can_edit?).and_return true
      post :create, params: {corrective_action_plan_id: @cap.id.to_s}
  ...
  describe 'update_resolution_status' do
    it 'should allow update if user can edit the parent plan' do
      ...
      post :update_resolution_status, params: { id: ci.id.to_s, is_resolved: true, format: :json }
      ci.reload
      expect(ci.resolved).to eq(true)

      # set it from true to false
      post :update_resolution_status, params: { id: ci.id.to_s, is_resolved: false, format: :json }
      ci.reload

describe PlantProductGroupAssignmentsController do
  ...
  describe "show" do
    it "should show if can view" do
      ppga = create(:plant_product_group_assignment)
  ...
  describe "update" do
    before :each do
      @cd = create(:custom_definition, module_type:'PlantProductGroupAssignment', data_type:'text')
      @ppga = create(:plant_product_group_assignment)
      @h = {"*cf_#{@cd.id}"=>'myval'}
    end
    it "should update if can edit" do
      allow_any_instance_of(PlantProductGroupAssignment).to receive(:can_edit?).and_return true
    ...
    it "should not update if cannot edit" do
      allow_any_instance_of(PlantProductGroupAssignment).to receive(:can_edit?).and_return false
      post :update, vendor_id: @ppga.plant.company_id, vendor_plant_id:@ppga.plant_id, id: @ppga.id, plant_product_group_assignment: @h
      expect(response).to be_redirect
      @ppga.reload

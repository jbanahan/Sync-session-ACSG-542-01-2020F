describe StateToggleButtonsController do
  ...
  describe "index" do
    let!(:stb) do
      user_cdef = create(:custom_definition, data_type: "integer", module_type: "Order", is_user: true, label: "Closed By")
      create(:state_toggle_button, module_type: "Order", user_custom_definition_id: user_cdef.id, date_attribute: "ord_closed_at")
    end
  ...
  describe "create" do
    it "creates STB for an admin" do
      ...
      expect(stb.disabled?).to eq true
  ...
  describe "edit" do
      let!(:stb) { create(:state_toggle_button, module_type: "Shipment", user_attribute: "shp_canceled_by", date_attribute: "shp_canceled_date") }

describe SurveysController do
  let(:user) { Factory(:user, survey_view: true, survey_edit: true) }
  ...
  describe 'index' do
    it "only shows surveys for logged in company" do
      to_find = Factory(:survey, company: user.company)
      Factory(:survey, company: user.company, archived: true)
      Factory(:survey)
      get :index
    ...
    it "shows archived surveys if prompted" do
      to_find = Factory(:survey, company: user.company)
      archived = Factory(:survey, company: user.company, archived: true)
  ...
  describe "show" do
    let(:survey) { Factory(:survey, company_id: user.company_id, name: "Name") }
  ...
  describe "edit" do
    let(:survey) { Factory(:survey, company_id: user.company_id) }
  ...
  describe "assign", :disable_delayed_jobs do
    ...
    it "assigns to the same user twice" do
      u2 = Factory(:user)
      Factory(:survey_response, survey: survey, user: u2) # making this one exist already
  ...
  describe "toggle subscription", :disable_delayed_jobs do
    ...
    it "destroys existing subscription" do
      Factory(:survey_subscription, survey_id: survey.id, user_id: user.id)
      expect do
  ...
  describe "restore" do
    it "restores surveys" do
      to_find = Factory(:survey, company: user.company, archived: true)

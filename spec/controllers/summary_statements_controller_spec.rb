describe SummaryStatementsController do
  ...
  describe "show" do
    ...
    it "renders" do
      allow(@u).to receive(:view_summary_statements?).and_return true
      us_entry = create(:entry, import_country: create(:country, iso_code: 'US'))
      ca_entry = create(:entry, import_country: create(:country, iso_code: 'CA'))
      bi_1 = create(:broker_invoice, entry: us_entry)
      bi_2 = create(:broker_invoice, entry: ca_entry)
      bi_3 = create(:broker_invoice, entry: ca_entry)
  ...
  describe "new" do
    ...
    it "limits the choice of companies to those with entries, sorts by name" do
      c1 = create(:company, name: "B")
      c2 = create(:company, name: "A")
      create(:company, name: "D")
      create(:entry, importer: c1)
      create(:entry, importer: c2)
  ...
  describe "create" do
    before(:each) do
      allow(@u).to receive(:edit_summary_statements?).and_return true
      @co_1 = create(:company, importer: true)
      @co_2 = create(:company)
  ...
  describe "update" do
    ...
    context "removing invoices" do
      before(:each) do
        @bi_1 = create(:broker_invoice, summary_statement: @ss).id.to_s
        @bi_2 = create(:broker_invoice, summary_statement: @ss).id.to_s
        create(:broker_invoice, summary_statement: @ss).id.to_s
      end
      ...
      it "makes no change and displays error if an invoice marked for removal doesn't belong to the statement" do
        bi_other = create(:broker_invoice, invoice_number: "123456789").id.to_s
    ...
    context "adding invoices" do
      ...
      it "adds invoices to statement" do
        create(:broker_invoice, invoice_number: '123456789', entry: create(:entry, importer: @company))
        create(:broker_invoice, invoice_number: '987654321', entry: create(:entry, importer: @company))
      ...
      it "makes no change and displays error if an invoice is ineligible to be added" do
        create(:broker_invoice, invoice_number: '123456789', entry: create(:entry, importer: create(:company))).id.to_s
        put :update, id: @ss, selected: {to_add: "123456789"}

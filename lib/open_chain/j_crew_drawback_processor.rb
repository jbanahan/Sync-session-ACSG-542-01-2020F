module OpenChain
  class JCrewDrawbackProcessor < DrawbackProcessor
    ...
    def find_shipment_lines commercial_invoice_line
      ...
      po_search.apply ShipmentLine.select("shipment_lines.*").joins(:shipment).joins(:product).where("products.unique_identifier = ?",commercial_invoice_line.part_number).

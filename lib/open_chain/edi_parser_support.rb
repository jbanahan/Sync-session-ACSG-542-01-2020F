  ...
  module ClassMethods
    ...
    def send_error_email transaction, error, parser_name, filename, to_address: "edisupport@vandegriftinc.com", include_backtrace: true
      Tempfile.create(["EDI-#{isa_code(transaction)}-", '.edi']) do |f|
        ...
        body = "<p>There was a problem processing the attached #{parser_name} EDI. A re-creation of only the specific EDI transaction file that errored is attached.</p><p>Error: "

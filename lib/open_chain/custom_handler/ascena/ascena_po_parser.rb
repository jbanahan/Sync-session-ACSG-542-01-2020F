...
module OpenChain; module CustomHandler; module Ascena; class AscenaPoParser
  ...
  def process_file(pipe_delimited_content, opts={})
    ...
    send_error_email(pipe_delimited_content) if errors[:missing_shipped_order_lines].presence
  end
  ...
  def send_error_email(file_content)
  ...
  def process_po rows, row_num, opts
    ActiveRecord::Base.transaction do
      ...
      validate_header(header, header_row_num)
  ...
  def process_detail_rows detail_rows, header, row_num, ord, opts
    details = detail_rows.map { |dr| map_detail dr }
    ...
    details.each_with_index do |detail, i|
      validate_detail(detail, header, row_num + i)
      product = get_or_create_product(detail, header, opts)
  ...
  def get_or_create_product detail, header, opts
    ...
    Lock.acquire("ASCENA-#{detail[:prod_part_number]}") do 
      product = Product.where("unique_identifier = ? AND importer_id = ?", "ASCENA-#{detail[:prod_part_number]}", importer.id).first_or_initialize
      already_existed = product.persisted?
      product = Product.create!(unique_identifier: "ASCENA-#{detail[:prod_part_number]}") unless already_existed
    end
    ...
    unless already_existed
      ...
      product.create_snapshot user, nil, opts[:key]
    end
    product
  end
  ...
  def validate_header header, row_num
  ...
  def validate_detail detail, header, row_num

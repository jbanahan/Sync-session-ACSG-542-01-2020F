...
# Although this class is "abstract" it's really only intended to hold the shared functionality of AscenaBillingInvoiceGenerator
# and AscenaFtzBillingInvoiceGenerator, not to serve as a basis for other projects. Consequently the methods have all been
# left "private," and rely on the unit tests of the two aforementioned classes.

module OpenChain; module CustomHandler; module Ascena; class AbstractAscenaBillingInvoiceFileGenerator
  include OpenChain::EntityCompare::ComparatorHelper
  include OpenChain::FtpFileSupport

  DUTY_SYNC ||= "ASCE_DUTY_BILLING".freeze
  BROKERAGE_SYNC ||= "ASCE_BROKERAGE_BILLING".freeze
  DUTY_CORRECTION_SYNC ||= "ASCE_DUTY_CORRECTION_BILLING".freeze
  LEGACY_SYNC ||= "ASCE_BILLING".freeze

  def calculate_duty_amounts _entry_snapshot
    raise "Requires an implementation"
  end
  ...
  protected
    ...
    def generate_invoice entry_snapshot, invoice_data, entry
    ...
    def send_file lines, sync_records, prefix, invoice_number: nil, ftz: false, duty_file: false
      # Skip brokerage invoice if there's at least one commercial-invoice PO
      return if !duty_file && !ci_po_numbers
      filename = [prefix, "#{duty_file ? "DUTY" : "BROKER"}_INVOICE", ftz ? "FTZ" : nil, "AP", invoice_number,
                  ActiveSupport::TimeZone["America/New_York"].now.strftime("%Y%m%d%H%M%S%L")].compact.join("_") + ".dat"
      ...
      Tempfile.open([File.basename(filename, ".*"), File.extname(filename)]) do |f|
        ...
        f.flush
        f.rewind
        ftp_sync_file f, sync_records, connect_vfitrack_net("to_ecs/_ascena_billing", filename)
      end
    end
    ...
    def generate_invoice_file_data entry_snapshot, broker_invoice_snapshot, po_brand_org_codes, sync_type, invoice_lines
      ...
      if sync_type == DUTY_SYNC
        ...
        file_data = generate_broker_invoice_file(broker_invoice_snapshot, po_brand_org_codes, invoice_lines)
      elsif sync_type == DUTY_CORRECTION_SYNC
        file_data = generate_duty_correction_file(broker_invoice_snapshot, po_brand_org_codes, invoice_lines)
      end
    ...
    def generate_broker_invoice_file(broker_invoice_snapshot, po_brand_org_codes, invoice_lines)
      file_data = []
    ...
    def generate_duty_correction_file(broker_invoice_snapshot, po_brand_org_codes, invoice_lines)
      file_data = []
    ...
    def duty_sync_type? sync_type
      [DUTY_SYNC, DUTY_CORRECTION_SYNC].include? sync_type
    end

    def file_prefix cust_num
      cust_num == "MAUR" ? "MAUR" : "ASC"
    end
    ...
    def invoice_line_duty_fields entry_snapshot, broker_invoice_snapshot, next_line_number, po_org_codes
      ...
      po_duty_amounts.each_pair do |po, amount|
        ...
        line[4] = amount
        line[5] = "Duty"
        # In the FTZ generator blank PO values are assigned a unique string since they're needed as hash keys. Don't record them, though.
        line[6] = po unless po =~ /^Missing/
        line[7] = po_org_codes[po]
    ...
    def split_broker_invoice_lines broker_invoice_snapshot
      ...
      json_child_entities(broker_invoice_snapshot, "BrokerInvoiceLine") do |invoice_line|
        ...
        case charge_code
        when duty_charge_codes
          duty_lines << invoice_line
        when skip_charge_codes # rubocop:disable Lint/EmptyWhen
          # skip these lines
        when post_summary_duty_charge_codes
    ...
    def post_summary_duty_charge_codes
    ...
    def generate_duty_credit_invoice broker_invoice_snapshot
      ...
      original_invoices = BrokerInvoice.joins(:entry, :broker_invoice_lines)
                                       .where(broker_invoice_lines: {charge_code: '0001', charge_amount: duty_amount})
                                       .where(entries: {broker_reference: broker_reference, customer_number: mf(broker_invoice_snapshot, "bi_ent_cust_num")})
    ...
    def unsent_invoices entry, broker_invoice_snapshots, trading_partners, override_sync_records: false
      invoices = {}

      ...
      encrypt_file(outfile) do |encrypted_file|
        ...
        begin
          ...
          if completed
            ...
            OpenMailer.send_simple_html(Group.use_system_group("canada-accounting", "VFI Canada Accounting"), "[VFI Track] Siemens Billing Report #{now.strftime("%Y-%m-%d")}", "Attached is the duty data that was sent to Siemens on #{now.strftime("%Y-%m-%d")}.", report).deliver!
          else

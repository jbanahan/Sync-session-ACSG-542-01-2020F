...
module OpenChain; module CustomHandler; module GtNexus; class AbstractGtnInvoiceXmlParser
  ...
  def initialize configuration
    ...
    # For implementations where we don't get PO data, we can skip looking up PO order lines.
    @link_to_orders = configuration[:link_to_orders].nil? ? true : configuration[:link_to_orders].nil?
  end
  ...
  def parse_invoice_line invoice, item_xml
    ...
    country_code = item_identifier_value(base_item, "CountryOfOrigin")
    line.country_origin = find_country(country_code) unless country_code.blank?

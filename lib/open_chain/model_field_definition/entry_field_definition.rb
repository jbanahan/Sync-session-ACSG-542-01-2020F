module OpenChain; module ModelFieldDefinition; module EntryFieldDefinition
  def add_entry_fields
    add_fields CoreModule::ENTRY, [
      ...
      [13, :ent_first_billed_date, :first_billed_date, "First Bill Issued Date", {
        data_type: :date,
        read_only: true,
        export_lambda: ->(ent) { ent.broker_invoices.collect(&:invoice_date).compact.min},
      ...
      [58, :ent_lading_port_name, :name, "Port of Lading Name",
        {
          data_type: :string,
      ...
      [59, :ent_unlading_port_name, :name, "Port of Unlading Name",
        {
          data_type: :string,
          import_lambda: lambda { |ent, data|
            port = Port.find_by name: data
      ...
      [60, :ent_entry_port_name, :name, "Port of Entry Name",
        {
          data_type: :string,
          import_lambda: lambda { |ent, data|
            port = Port.find_by name: data
          ...
          qualified_field_name:
              "(
                CASE entries.source_system
                WHEN 'Fenix' THEN
                (
                  SELECT name FROM ports WHERE ports.cbsa_port = entries.entry_port_code
                )
                ELSE (
                  SELECT name FROM ports WHERE ports.schedule_d_code = entries.entry_port_code
                )
                END
              )"
        }
      ...
      [79, :ent_broker_invoice_total, :broker_invoice_total, "Total Broker Invoice", {data_type: :decimal, currency: :usd, can_view_lambda: ->(u) { u.view_broker_invoices?}}],
      ...
      [85, :ent_charge_codes, :charge_codes, "Charge Codes Used", {data_type: :string, can_view_lambda: ->(u) { u.view_broker_invoices?}}],
      ...
      [92, :ent_ci_line_count, :ci_line_count, "Commercial Invoice Line Count",
        {
          import_lambda: ->(_obj, _data) { "Commercial Invoice Line Count ignored. (read only)"},
          export_lambda: ->(obj) { obj.commercial_invoice_lines.count},
          qualified_field_name:
              "(
                select
                  count(*)
                from
                  commercial_invoice_lines cil
      ...
      [97, :ent_census_warning, :census_warning, "Census Warning", {data_type: :boolean, can_view_lambda: ->(u) { u.company.broker?}}],
      [98, :ent_error_free_release, :error_free_release, "Error Free Release", {data_type: :boolean, can_view_lambda: ->(u) { u.company.broker?}}],
      ...
      [100, :ent_pdf_count, :pdf_count, "PDF Attachment Count",
        {
          import_lambda: ->(_obj, _data) { "PDF Attachment Count ignored. (read only)"},
          export_lambda: ->(obj) { obj.attachments.where("attached_content_type = 'application/pdf' OR lower(attached_file_name) LIKE '%pdf'").count},
          qualified_field_name:
              "(
                select
                  count(*)
                from
                  attachments
                where
                  attachable_type = 'Entry' and
                  attachable_id = entries.id and
                  (
                    attached_content_type='application/pdf' OR
                    lower(attached_file_name) LIKE '%pdf'
                  )
              )",
          data_type: :integer,
          can_view_lambda: ->(u) { u.company.broker?}
        }
      ],
      ...
      [123, :ent_statement_month, :statement_month, "PMS Month", {
        import_lambda: ->(_obj, _data) { "PMS Month ignored. (read only)"},
        export_lambda: ->(obj) { obj.monthly_statement_due_date ? obj.monthly_statement_due_date.month : nil},
        qualified_field_name: "month(monthly_statement_due_date)",
        data_type: :integer
      }],
      [124, :ent_first_7501_print, :first_7501_print, "7501 Print Date - First", {data_type: :datetime, can_view_lambda: ->(u) { u.company.broker?}}],
      [125, :ent_last_7501_print, :last_7501_print, "7501 Print Date - Last", {data_type: :datetime, can_view_lambda: ->(u) { u.company.broker?}}],
      [126, :ent_duty_billed, :duty_billed, "Total Duty Billed", {
        import_lambda: ->(_obj, _data) { "Total Duty Billed ignored. (read only)"},
        export_lambda: ->(obj) { obj.total_billed_duty_amount },
        qualified_field_name: Entry.total_duty_billed_subquery,
        data_type: :decimal,
        can_view_lambda: ->(u) { u.view_broker_invoices? && u.company.broker?}
      }],
      ...
      [140, :ent_employee_name, :employee_name, "Employee", {data_type: :string, can_view_lambda: ->(u) { u.company.broker?}}],
      ...
      [156, :ent_expected_update_time, :expected_update_time, "Expected Update Time", {data_type: :datetime, can_view_lambda: ->(u) { u.company.broker?}}],
      ...
      [159, :ent_currencies, :currencies, "Currencies", {
        import_lambda: ->(_obj, _data) { "Currencies ignored. (read only)"},
        export_lambda: ->(obj) { obj.commercial_invoices.map(&:currency).uniq.join("\n")},
      ...
      [162, :ent_user_notes, :user_notes, "User Notes", {
        data_type: :string,
        read_only: true,
        import_lambda: ->(_obj, _data) { "User Notes ignored. (read only)"},
        ...
        qualified_field_name: lambda {
            "(
              SELECT
                GROUP_CONCAT(
                  IF(ec.generated_at IS NOT NULL,
                    CONCAT(
                      ec.body,
                      ' (',
                      DATE_FORMAT(CONVERT_TZ(ec.generated_at, 'UTC', '#{Time.zone.tzinfo.name}'),'%Y-%m-%d %H:%i'),
                      ' - ',
                      ec.username,
                      ')'
                    ),
                    CONCAT(ec.body, ' (', ec.username, ')')
                  )
                SEPARATOR '\n')
              FROM
                entry_comments ec
              ...
              WHERE
                e.id = entries.id AND
                ec.username NOT IN (#{EntryComment::USER_TYPE_MAP.keys.map(&:inspect).join(', ')})
            )"
        }
      }],
      [163, :ent_first_sale_savings, :ent_first_sale_savings, "First Sale Savings",
        {
          data_type: :decimal, read_only: true,
          import_lambda: ->(_obj, _data) { "First Sale Savings ignored. (read only)"},
          export_lambda: ->(obj) { obj.first_sale_savings },
          qualified_field_name:
              "IFNULL(
                (
                  SELECT
                    SUM(
                      ROUND((cil.contract_amount - cil.value) *
                      IFNULL(
                        (
                          SELECT
                            cit.duty_amount / cit.entered_value
                          FROM
                            commercial_invoice_tariffs cit
                          WHERE
                            cit.commercial_invoice_line_id = cil.id
                          ORDER BY
                            cit.id limit 1
                        ),
                      0),
      ...
      [173, :ent_container_count, :container_count, "Container Count", {
        data_type: :integer,
        read_only: true,
        export_lambda: ->(ent) { ent.containers.length},
      ...
      [210, :ent_hold_date, :hold_date, "Hold Date", {
        data_type: :datetime,
        read_only: true,
        import_lambda: ->(_obj, _data) { "Hold Date ignored. (read only)" }
      }],
      [211, :ent_hold_release_date, :hold_release_date, "Hold Release Date", {
        data_type: :datetime,
        read_only: true,
        import_lambda: ->(_obj, _data) { "Hold Release Date ignored. (read only)"}
      }],
      [212, :ent_on_hold, :on_hold, "On Hold", {
        data_type: :boolean,
        read_only: true,
        import_lambda: ->(_obj, _data) { "On Hold ignored. (read only)"}
      }],
                ...
                IF(
                  (
                    ...
                    FROM
                      countries
                    WHERE
                      countries.id = entries.import_country_id
                  ) = 'CA',
                  LEFT(entry_number, 5),
                  LEFT(entry_number, 3)
                ),
      ...
      [215, :ent_total_miscellaneous_discount, :total_miscellaneous_discount, "Total Miscellaneous Discount", {
        data_type: :decimal,
        import_lambda: ->(_obj, _data) { "Commercial Invoice Line Count ignored. (read only)"},
        export_lambda: ->(obj) { obj.commercial_invoice_lines.sum(:miscellaneous_discount)},
        qualified_field_name:
            "(
              SELECT
                SUM(cil.miscellaneous_discount)
              FROM
                commercial_invoice_lines AS cil
                INNER JOIN commercial_invoices AS ci ON ci.id = cil.commercial_invoice_id
              WHERE
                ci.entry_id = entries.id
            )"
      }],
      [216, :ent_import_date, :import_date, "Import Date", {data_type: :date}],
      [217, :ent_split_shipment, :split_shipment, "Split Shipment",
        {
          data_type: :boolean,
          export_lambda: ->(obj) { obj.split_shipment? },
          qualified_field_name: "IFNULL(split_shipment, false)"
        }
      ],
      [218, :ent_split_release_option, :split_release_option, "Split Release Option",
        {
          data_type: :string,
          export_lambda: ->(obj) { obj.split_release_option_value },
      ...
      [220, :ent_total_billed_duty_amount, :total_billed_duty_amount, "Total Billed Duty Amount",
        {
          data_type: :decimal, currency: :usd, read_only: true,
          export_lambda: ->(obj) { obj.total_billed_duty_amount },
          qualified_field_name: Entry.total_duty_billed_subquery
        }
      ],
      [221, :ent_total_taxes, :total_taxes, "Total Taxes", {data_type: :decimal, currency: :usd}],
      [222, :ent_total_duty_taxes_fees_penalties, :total_duty_taxes_fees_penalties, "Total Duty, Taxes, Fees & Penalties",
        {
          data_type: :decimal, currency: :usd, read_only: true,
          export_lambda: ->(obj) { obj.total_duty_taxes_fees_amount },
          qualified_field_name:
              "(
                IFNULL(entries.total_duty,0) +
                IFNULL(entries.total_taxes,0) +
                IFNULL(entries.total_fees,0) +
                IFNULL(entries.total_cvd,0) +
                IFNULL(entries.total_add,0)
              )"
        }
      ],
          ...
          qualified_field_name:
              "(
                ...
                WHEN (
                  ...
                  WHERE
                    entries.id = ci.entry_id AND
                    cil.psc_date IS NOT NULL
                ) THEN 1 ELSE 0 END
              )"
        }
      ],
      ...
      [233, :ent_total_value_tax, :total_value_tax, "Total Value for Tax", {
        data_type: :decimal,
        read_only: true,
        import_lambda: ->(_obj, _data) { "Total Value for Tax ignored. (read only)"},
        export_lambda: ->(obj) { obj.value_for_tax },
      ...
      [244, :ent_master_bills_of_lading_count, :master_bills_of_lading_count, "Total Master Bills of Lading",
        {
          data_type: :integer,
          read_only: true,
          import_lambda: ->(_obj, _data) { "Total Master Bills of Lading ignored. (read only)" },
          export_lambda: ->(obj) { obj.master_bills_of_lading.present? ? obj.split_newline_values(obj.master_bills_of_lading).length : 0 },
          qualified_field_name: <<-SQL
      ...
      [245, :ent_house_bills_of_lading_count, :house_bills_of_lading_count, "Total House Bills of Lading",
        {
          data_type: :integer,
          read_only: true,
          import_lambda: ->(_obj, _data) { "Total House Bills of Lading ignored. (read only)" },
          export_lambda: ->(obj) { obj.house_bills_of_lading.present? ? obj.split_newline_values(obj.house_bills_of_lading).length : 0 },
          qualified_field_name: <<-SQL
      ...
      [247, :ent_broker_invoice_list, :broker_invoice_list, "Broker Invoice Number(s)",
        {
          data_type: :string,
          read_only: true,
          import_lambda: ->(_obj, _data) { "Broker Invoice numbers ignored. (read only)"},
          export_lambda: ->(obj) { obj.broker_invoices.map(&:invoice_number).uniq.join("\n") },
      ...
      [261, :ent_exception, :exception, "Exception Flag", {
        data_type: :boolean,
        read_only: true,
        import_lambda: ->(_obj, _data) { "Exception Flag ignored. (read only)" },
        export_lambda: ->(obj) { Entry.milestone_exception_fields.map { |f| obj.public_send f }.any? },
        qualified_field_name: <<-SQL
      ...
      [265, :ent_daily_statement_duty_amount_paid, :daily_statement_duty_amount_paid, "Statement Duty Amt Paid",
        {
          data_type: :decimal,
          read_only: true,
          export_lambda: ->(obj) { obj.daily_statement_entry.present? ? obj.daily_statement_entry.duty_amount : 0},
      ...
      [266, :ent_daily_statement_tax_amount_paid, :daily_statement_tax_amount_paid, "Statement Tax Amount Paid",
        {
          data_type: :decimal,
          read_only: true,
          export_lambda: ->(obj) { obj.daily_statement_entry.present? ? obj.daily_statement_entry.tax_amount : 0},
      ...
      [267, :ent_daily_statement_add_amount_paid, :daily_statement_add_amount_paid, "Statement ADD Amount Paid",
        {
          data_type: :decimal,
          read_only: true,
          export_lambda: ->(obj) { obj.daily_statement_entry.present? ? obj.daily_statement_entry.add_amount : 0},
      ...
      [268, :ent_daily_statement_cvd_amount_paid, :daily_statement_cvd_amount_paid, "Statement CVD Amount Paid",
        {
          data_type: :decimal,
          read_only: true,
          export_lambda: ->(obj) { obj.daily_statement_entry.present? ? obj.daily_statement_entry.cvd_amount : 0},
      ...
      [269, :ent_daily_statement_fee_amount_paid, :daily_statement_fee_amount_paid, "Statement Fee Amount Paid",
        {
          data_type: :decimal,
          read_only: true,
          export_lambda: ->(obj) { obj.daily_statement_entry.present? ? obj.daily_statement_entry.fee_amount : 0},
      ...
      [270, :ent_daily_statement_total_amount_paid, :daily_statement_total_amount_paid, "Statement Total Amount Paid",
        {
          data_type: :decimal,
          read_only: true,
          export_lambda: ->(obj) { obj.daily_statement_entry.present? ? obj.daily_statement_entry.total_amount : 0},
      ...
      [275, :ent_open_exception_codes, :open_exception_codes, "Open Exception Codes",
        {
          data_type: :string,
          read_only: true,
          import_lambda: ->(_obj, _data) { "Open Exception Codes ignored. (read only)"},
          export_lambda: lambda { |obj|
            obj.entry_exceptions.select { |eex| eex.resolved_date.nil? }.map(&:code).uniq.join("\n")
          },
          qualified_field_name:
              '(
                SELECT
                  GROUP_CONCAT(DISTINCT eex.code SEPARATOR "\n")
                FROM
                  entry_exceptions AS eex
                WHERE
                  eex.entry_id = entries.id AND eex.resolved_date IS NULL
              )'
        }
      ],
      [276, :ent_resolved_exception_codes, :resolved_exception_codes, "Resolved Exception Codes",
        {
          data_type: :string,
          read_only: true,
          import_lambda: ->(_obj, _data) { "Resolved Exception Codes ignored. (read only)"},
          export_lambda: lambda { |obj|
            obj.entry_exceptions.select { |eex| eex.resolved_date.present? }.map(&:code).uniq.join("\n")
          },
          qualified_field_name:
              '(
                SELECT
                ...
                FROM
                  entry_exceptions AS eex
                WHERE
                  eex.entry_id = entries.id AND
                  eex.resolved_date IS NOT NULL
              )'
        }
      ],
      ...
      [279, :ent_classification_inquiry_exception_opened_date, :classification_inquiry_exception_opened_date,
          "Classification Inquiry Exception Opened Date", {data_type: :datetime}],
      [280, :ent_classification_inquiry_exception_resolved_date, :classification_inquiry_exception_resolved_date,
          "Classification Inquiry Exception Resolved Date", {data_type: :datetime}],
      [281, :ent_customer_requested_hold_exception_opened_date, :customer_requested_hold_exception_opened_date,
          "Customer Requested Hold Exception Opened Date", {data_type: :datetime}],
      [282, :ent_customer_requested_hold_exception_resolved_date, :customer_requested_hold_exception_resolved_date,
          "Customer Requested Hold Exception Resolved Date", {data_type: :datetime}],
      ...
      [285, :ent_document_discrepancy_exception_opened_date, :document_discrepancy_exception_opened_date,
          "Document Discrepancy Exception Opened Date", {data_type: :datetime}],
      [286, :ent_document_discrepancy_exception_resolved_date, :document_discrepancy_exception_resolved_date,
          "Document Discrepancy Exception Resolved Date", {data_type: :datetime}],
      ...
      [299, :ent_pending_customs_review_exception_opened_date, :pending_customs_review_exception_opened_date,
          "Pending Customs Review Exception Opened Date", {data_type: :datetime}],
      [300, :ent_pending_customs_review_exception_resolved_date, :pending_customs_review_exception_resolved_date,
          "Pending Customs Review Exception Resolved Date", {data_type: :datetime}],
      ...
      [317, :ent_exception_notes, :exception_notes, "Exception Notes",
        {
          data_type: :string,
          read_only: true,
          import_lambda: ->(_obj, _data) { "Exception Notes ignored. (read only)"},
          export_lambda: lambda { |obj|
            obj.entry_exceptions.select { |eex| eex.comments.present? }.map(&:comments).uniq.join("\n")
          },
          qualified_field_name:
              '(
                SELECT
                  GROUP_CONCAT(DISTINCT eex.comments SEPARATOR "\n")
                FROM
                  entry_exceptions AS eex
                WHERE
                  eex.entry_id = entries.id AND
    ...
    add_fields CoreModule::ENTRY, make_attachment_arrays(700, 'ent', CoreModule::ENTRY, {ent_attachment_types: ->(u) { u.company.master?}})
  ...
  def make_pga_flag_field rank, agency_code
    [rank, "ent_pga_#{agency_code.downcase}".to_sym, "pga_#{agency_code.downcase}".to_sym, "PGA - #{agency_code}",
      {
        import_lambda: ->(_obj, _data) { "PGA - #{agency_code} ignored. (read only)"},
        export_lambda: ->(obj) { obj.includes_pga_summary_for_agency? agency_code },

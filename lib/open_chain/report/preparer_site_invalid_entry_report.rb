...
module OpenChain; module Report; class PreparerSiteInvalidEntryReport
  ...
  def run emails, start_date=nil, end_date=nil
    ...
    wb = XlsMaker.create_workbook 'Statement Preparer Site Invalid'
    sheet = wb.worksheets[0]
    rows = table_from_query sheet, qry
    if rows == 0
      XlsMaker.add_body_row sheet, 1, ["No invalid statement preparer sites"]
    end

    workbook_to_tempfile(wb, "Statement Preparer Site Invalid", file_name: "Statement Preparer Site Invalid - #{Time.zone.now.to_date}.xls") do |t|
...
WHERE
        ...
        AND n.body LIKE '%STMNT PREPARER SITE INVALID%'

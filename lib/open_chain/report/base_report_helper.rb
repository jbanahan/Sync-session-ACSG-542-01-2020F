module OpenChain; module Report; module BaseReportHelper
  ...
  def transport_mode_us_ca_translation_lambda
    ...
    ->(_result_set_row, raw_column_value) { transport_modes[raw_column_value.to_i] }
  end
  ...
  # Allows report results to be indexed by field name.
  # Inheriting class should override #initialize like so:
  #
  #  def initialize row
  #    super row, FIELD_MAP
  #  end
  #
  # where FIELD_MAP is a hash of name => index
  #
  class RowWrapper
    ...
    def self.wrap_all collection
      wrapped = []
      collection.each { |r| wrapped << self.new(r) }
      wrapped
    end

    def self.unwrap_all wrapped_collection
      wrapped_collection.map(&:to_a)
    end
  end
end; end; end

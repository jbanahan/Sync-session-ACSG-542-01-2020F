  ...
  module Report
    class MonthlyEntrySummation
      ...
      def run(run_by, settings)
        ...
        table_from_query sheet, entry_summation_query(start_date, end_date, settings['customer_number'])
...
SELECT
  ...
  (SELECT SUM(charge_amount) FROM broker_invoices JOIN broker_invoice_lines ON broker_invoice_id = broker_invoices.id JOIN entries AS entry ON broker_invoices.entry_id = entry.id WHERE DATE_FORMAT(entry.release_date,'%Y-%m') = `Year / Month` AND charge_type <> 'F' ) AS `Total Freight`,
  (SELECT SUM(charge_amount) FROM broker_invoices JOIN broker_invoice_lines ON broker_invoice_id = broker_invoices.id JOIN entries AS entry ON broker_invoices.entry_id = entry.id WHERE DATE_FORMAT(entry.release_date,'%Y-%m') = `Year / Month` AND charge_type NOT IN ('F','D')) AS `Total Brokerage Fees`
FROM entries

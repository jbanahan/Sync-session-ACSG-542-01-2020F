...
module OpenChain; module Report; class AscenaEntryAuditReport
  ...
  def run run_by, settings
    ...
    wb = create_workbook start_date, end_date, settings['range_field'], run_by.time_zone, self.class.cust_num_to_sys_code(settings['cust_number'])
    workbook_to_tempfile wb, "AscenaEntryAuditReport-", file_name: "#{self.class.cust_nums_to_short_names(settings['cust_number'])} Entry Audit Report.xls"
  end
  ...
  def create_workbook start_date, end_date, range_field, time_zone, importer_system_code
    wb, sheet = XlsMaker.create_workbook_and_sheet "#{self.class.sys_codes_to_short_names(importer_system_code)} Entry Audit Report"
    table_from_query sheet, query(start_date, end_date, range_field, sanitize(importer_system_code), cdefs), conversions(time_zone).merge("Web Link"=>weblink_translation_lambda(CoreModule::ENTRY))
    wb
  end
    ...
    qry = <<-SQL
            ...
            FROM entries e
              ...
              LEFT OUTER JOIN orders o ON o.order_number = CASE ?
                                                             WHEN '#{ASCENA_SYS_CODE}' THEN CONCAT('ASCENA-', cil.product_line, '-', cil.po_number)
                                                             WHEN '#{MAURICES_SYS_CODE}' THEN CONCAT('ASCENA-MAU-', cil.po_number)
                                                             WHEN '#{ANN_SYS_CODE}' THEN CONCAT('ATAYLOR-', cil.po_number)
                                                           END
            ...
            WHERE e.#{range_field} >= ? AND e.#{range_field} < ?
              AND e.customer_number = ?
                SQL

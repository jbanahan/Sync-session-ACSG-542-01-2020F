module OpenChain; module WorkflowTester; class SurveyCompleteWorkflowTest
  ...
  def self.pass? workflow_task
    ...
    return !workflow_task.object_to_test.survey_responses.includes(:survey).where('survey_responses.rating = ?',payload['survey_rating']).where('surveys.system_code = ?',payload['survey_code']).empty?
  end
end; end; end;
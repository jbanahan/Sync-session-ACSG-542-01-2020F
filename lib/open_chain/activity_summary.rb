...
module OpenChain; class ActivitySummary
  ...
  def self.generator_for_country iso
    case iso.upcase
    when 'US'
      USEntrySummaryGenerator.new
    when 'CA'
      CAEntrySummaryGenerator.new
    else
  ...
  module DutyDetail
    ...
    def create_linked_digests(current_user, company)
      company.linked_companies.select(&:importer?).map { |co| create_digest(current_user, co) }.compact
    end
    ...
    def build_digest(entries)
      co = nil
      entries.each do |ent|
        co ||= {company_name: ent.company_name,
                company_report: {date_hsh: {}, company_total_duty: 0, company_total_fees: 0,
                                 company_total_duty_and_fees: 0, company_entry_count: 0}}

        company_ptr = co[:company_report]

        date_ptr = company_ptr[:date_hsh][ent.duty_due_date] ||= {port_hsh: {}, date_total_duty: 0, date_total_fees: 0,
                                                                  date_total_duty_and_fees: 0, date_entry_count: 0}

        port_ptr = date_ptr[:port_hsh][ent.port_name] ||= {port_total_duty: 0, port_total_fees: 0, port_total_duty_and_fees: 0,
                                                           port_entry_count: 0, entries: []}
        ...
        port_ptr[:entries] << {ent_id: ent.entry_id, ent_entry_number: ent.entry_number, ent_entry_type: ent.entry_type,
                               ent_port_name: ent.port_name, release_date_mf.uid => ent.send(release_date_mf.field_name),
                               ent_customer_references: ent.customer_references, ent_duty_due_date: ent.duty_due_date,
                               ent_total_fees: ent.total_fees, ent_total_duty: ent.total_duty, ent_total_duty_and_fees: ent.total_duty_and_fees}
    ...
    def get_entries(current_user, company)
      if current_user.view_entries? && company.can_view?(current_user)
        Entry.search_secure(current_user, Entry.select("companies.name AS company_name, duty_due_date, ports.name AS port_name, "\
                                                       "entries.id AS entry_id, entry_number, entry_type, customer_references, " \
                                                       "#{ActiveRecord::Base.connection.quote_column_name(release_date_mf.field_name)}, " \
                                                       "total_duty, total_fees, (total_duty + total_fees) AS total_duty_and_fees")
                                               .joins(:us_entry_port)
                                               .joins(:importer)
                                               ...
                                               .where("!(entry_type <=> '06' && first_entry_sent_date IS NULL)")
                                               .order("duty_due_date"))
      else
        []
      end
  ...
  class EntrySummaryGenerator
    ...
    def create_by_release_range_download company_id, range, base_date = Time.zone.now.midnight
      ...
      select_clause = mf_uids.map do |uid|
        mf = ModelField.find_by_uid(uid) # rubocop:disable Rails/DynamicFindBy
        "#{mf.field_name} AS \"#{mf.label}\""
      end.push("id AS \"Link\"").join(", ")
    ...
    def by_release_range_query importer_id, base_date, range
      ...
      Entry.where(date_clause)
           ...
           .where(exclude_ftz_clause)
           .order(%(IFNULL(entries.#{ActiveRecord::Base.connection.quote_column_name(release_date_mf.field_name)},"2999-01-01") DESC))
    end
    ...
    def generate_port_breakouts importer_id, base_date_utc
      generate_breakout_hash(->(imp, dc) { generate_port_breakout_line(imp, dc) }, importer_id, base_date_utc)
    end
    ...
    def generate_port_breakout_line importer_id, date_clause
      ...
      AND #{tracking_open_clause} AND #{country_clause} AND #{exclude_ftz_clause}
    ...
    def generate_hts_breakouts importer_id, base_date_utc
      generate_breakout_hash(->(imp, dc) { generate_hts_breakout_line(imp, dc)}, importer_id, base_date_utc)
    end

    def generate_hts_breakout_line importer_id, date_clause
      sql = <<-SQL
        SELECT left(commercial_invoice_tariffs.hts_code, 2), count(*)
        FROM entries
    ...
    def generate_top_vendors importer_id, base_date_utc
      sql = <<-SQL
        SELECT #{vendor_field}, sum(commercial_invoice_tariffs.entered_value)
        FROM entries
          INNER JOIN commercial_invoices ON commercial_invoices.entry_id = entries.id
          INNER JOIN commercial_invoice_lines ON commercial_invoice_lines.commercial_invoice_id = commercial_invoices.id
          INNER JOIN commercial_invoice_tariffs ON commercial_invoice_tariffs.commercial_invoice_line_id = commercial_invoice_lines.id
        WHERE (#{Entry.search_where_by_company_id importer_id})
          AND (#{ytd_clause(base_date_utc)}) AND (#{tracking_open_clause})
          AND #{country_clause} AND #{exclude_ftz_clause}
    ...
    def week_clause base_date_utc
      ActiveRecord::Base.sanitize_sql_array(["(#{ActiveRecord::Base.connection.quote_column_name(release_date_mf.field_name)} > DATE_ADD(?, INTERVAL -1 WEEK) AND #{ActiveRecord::Base.connection.quote_column_name(release_date_mf.field_name)} < ?)", base_date_utc, end_of_day(base_date_utc)]) # rubocop:disable Layout/LineLength
    end

    # generate a where clause for the previous 4 weeks
    def four_week_clause base_date_utc
      ActiveRecord::Base.sanitize_sql_array(["(#{ActiveRecord::Base.connection.quote_column_name(release_date_mf.field_name)} > DATE_ADD(?, INTERVAL -4 WEEK) AND #{ActiveRecord::Base.connection.quote_column_name(release_date_mf.field_name)} < ?)", base_date_utc, end_of_day(base_date_utc)]) # rubocop:disable Layout/LineLength
    end
    ...
    def exclude_ftz_clause
      "!(entry_type <=> '06' && first_entry_sent_date IS NULL)"
    end
  ...
  class CAEntrySummaryGenerator < EntrySummaryGenerator
    def release_date_mf
      @release_date_mf ||= ModelField.find_by_uid :ent_release_date # rubocop:disable Rails/DynamicFindBy
    end
    ...
    def generate_hash company_id, base_date
      @country_id = Country.find_by(iso_code: 'CA').id
    ...
    def generate_summary_line importer_id, date_clause, more_where_clauses = []
      ...
      sql = <<-SQL
               ...
               FROM entries
               WHERE (#{date_clause}) AND (#{w}) AND #{tracking_open_clause} AND #{country_clause} AND #{exclude_ftz_clause}
  ...
  class USEntrySummaryGenerator < EntrySummaryGenerator
    ...
    def release_date_mf
      @release_date_mf ||= ModelField.find_by_uid :ent_first_release_received_date # rubocop:disable Rails/DynamicFindBy
    end
    ...
    def linked_companies_unpaid_duty importer, base_date_utc
      importer.linked_companies.select(&:importer).map { |co| single_company_unpaid_duty(co, base_date_utc)}
    end
    ...
    def generate_pms_section importer_id, base_date_utc
      ...
      qry = <<-SQL
        ...
        WHERE monthly_statement_due_date <= DATE(DATE_ADD('#{base_date_utc}', INTERVAL 1 MONTH))
          ...
          AND (#{Entry.search_where_by_company_id importer_id})
          AND #{tracking_open_clause}
          AND #{country_clause}
          AND #{exclude_ftz_clause}
  ...
  class EntrySummaryDownload
    ...
    class ReportEmailer
      def self.email importer, iso_code, addresses, subject = nil, body = nil, user_id = nil, mailing_list = nil
        ...
        begin
          report = gen.run
          to = []
          to << addresses if addresses.present?
      ...
      def self.update_args generator, addresses, subject, body, user_id
        ...
        subject = default_subject if subject.blank?
        body = "#{default_subject} is attached." if body.blank?
      ...
      def self.update_args_with_user addresses, body, user_id
        ...
        if addresses.blank?
          ...
          fname, lname, email = [user.first_name, user.last_name, user.email].map { |x| CGI.escapeHTML x }
          body = "<p>#{fname} #{lname} (#{email}) has sent you a report.</p><br>".html_safe + body # rubocop:disable Rails/OutputSafety
        end
        [addresses, body]
      end
    end
    ...
    def self.find_company settings
      if settings['system_code']
        ...
        if settings["cargowise_customer_number"].blank?
          Company.by_system_code("Customs Management", settings["alliance_customer_number"])
        else
          Company.by_system_code("Cargowise", settings["cargowise_customer_number"])
        end
      else
        Company.by_system_code("Fenix", settings["fenix_customer_number"])
      end
    end
    ...
    def write_unpaid_duty board, rows, row_num
      ...
      rows.each { |r| board.insert_row (self.row_num += 1), fst_col, [r['customer_name'].upcase, r['total_duty'], r['total_fees'], r['total_duty_and_fees']], [nil] + Array.new(3, :default_currency) } # rubocop:disable Layout/LineLength
    end

module OpenChain; module ModelFieldGenerator; module CountryHtsGenerator
  ...
  def import_lambda product_lambda, hts_number, country
    lambda do |obj, d|
      ...
      if hts.blank?
        ...
        unless OfficialTariff.where(country_id: country.id, hts_code: hts).present? || OfficialTariff.where(country_id: country.id).empty?
      ...
      cls ||= p.classifications.build(country_id: country.id)
      tr = cls.tariff_records.min {|a, b| a.line_number <=> b.line_number}
      tr ||= cls.tariff_records.build
      tr.send("hts_#{hts_number}=".to_sym, hts)
      "#{country.iso_code} HTS #{hts_number} set to #{hts&.hts_format || '[blank]'}"
    end
  end
  ...
  def make_country_hts_fields core_module, join_table_name: core_module.table_name, read_only: true, model_field_suffix: core_module.table_name, hts_numbers: 1..1, label_prefix: "Product - ", starting_index: nil, product_lambda: # rubocop:disable Layout/LineLength
    model_fields = []
      ...
      hts_numbers.each_with_index do |i, x|
        ...
        mf = ModelField.new((starting_index + x), field_name, core_module, field_name,
                            {label_override: "#{label_prefix}First HTS #{i} (#{c.iso_code})", data_type: :string, history_ignore: true, read_only: read_only,
                             ...
                             process_query_result_lambda: ->(r) { r.nil? ? nil : r.hts_format }})
        model_fields << mf
      end
    end
    model_fields
